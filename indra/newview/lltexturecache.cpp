...
const F32 TEXTURE_CACHE_PURGE_AMOUNT = 0.20f; // % amount to reduce the cache by when it exceeds its limit the value is altered inside the functions by the ramcache divisor!
...
const F32 TEXTURE_LAZY_PURGE_TIME_LIMIT = .006f; // S24 PERF: 6ms - balanced for continuous background operation
...
size_t LLTextureCache::update(F32 max_time_ms)
{
    ...
    // S24 PERF: Continuous background decay - trickle-drain RAM to disk for smooth equilibrium
    ...
    if (decay_timer.getElapsedTimeF32() > DECAY_INTERVAL)
    {
        ...
        // Check if we're above target ratio
        S64 target_size = (S64)(sCacheMaxTexturesSize * RAM_TARGET_RATIO);
        if (mTexturesSizeTotal > target_size)
        {
            // Trigger gentle continuous purge
            if (!mDoPurge)
            {
                mDoPurge = true;
            }
            // Process a small batch for smooth background drain
            purgeTexturesLazy(DECAY_TIME_BUDGET);
...
bool LLTextureCache::updateEntry(S32& idx, Entry& entry, S32 new_image_size, S32 new_data_size)
{
    ...
    else
    {
        ...
        // S24 PERF: Progressive purge trigger - start evicting at 95% instead of 100%
        // Reduces bursty behavior and maintains smoother equilibrium
        constexpr F32 PROGRESSIVE_PURGE_THRESHOLD = 0.95f;
        if (mTexturesSizeTotal > (S64)(sCacheMaxTexturesSize * PROGRESSIVE_PURGE_THRESHOLD))
        {
            purge = true;
        }
...
U32 LLTextureCache::openAndReadEntries(std::vector<Entry>& entries)
{
    ...
    return num_entries;
        ...
        {
    entries.clear();
    LL_WARNS() << "Bad alloc reading texture entries from cache, mFreeList: " << (S32)mFreeList.size()
...
void LLTextureCache::purgeTexturesLazy(F32 time_limit_sec)
{
    ...
    F32 RamCacheDivisor = sUseRamCache() ? 0.25f : 1.f;
    ...
    if (mPurgeEntryList.empty())
    {
        ...
        S64 purged_cache_size = (llmax(cache_size, sCacheMaxTexturesSize) * (S64)((1.f - TEXTURE_CACHE_PURGE_AMOUNT * RamCacheDivisor) * 100)) / 100;
