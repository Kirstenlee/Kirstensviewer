    ...
    mBSphereRadius(-1.0f)
{
    ...
    if (success)
    {
		...
		//            Cache will self-heal on next object update from server
    ...
    if(success && size > 0)
    {
        ...
        {
			// S24 VOCACHE PERF: Cache entry read failure (disk I/O error or corruption)
			// CRITICAL: This is a NORMAL condition - cache misses/corruption happen constantly
			// Entry will be regenerated from server on next object update
			// Silent failure provides best performance - no console spam during cache regeneration
            delete[] mBuffer ;
            mBuffer = NULL ;
        }
    }
...
{
	// S24 VOCACHE PERF: Debug dump function - only enable if actively debugging cache
	// Commented out to avoid console spam during normal operation
	// Uncomment only when diagnosing specific cache issues
	/*
...
S32 LLVOCacheEntry::writeToBuffer(U8 *data_buffer) const
{
	...
	// S24 VOCACHE PERF: Entry size validation on write
	// CRITICAL: Oversized entries are clamped/rejected silently for performance
	// This is a PROGRAMMER ERROR (generated invalid cache data), not user error
	// llassert will catch in debug builds, production should handle gracefully
	if (size > MAX_ENTRY_BODY_SIZE)
	{
		llassert(size <= MAX_ENTRY_BODY_SIZE); // Debug builds: catch programmer error
		return 0; // Production: silent rejection, entry will regenerate on next update
    }
...
void LLVOCache::initCache(ELLPath location, U32 size, U32 cache_version)
{
	if (!mEnabled)
	{
		return; // Cache disabled - silent return
	}

	if (mInitialized)
	{
		return; // Already initialized - silent return (idempotent operation)
	}
    mInitialized = true;
	...
	// S24 VOCACHE PERF: Cache version check - silent unless debug build
	// Version mismatches are NORMAL during viewer upgrades - handled automatically
	LL_DEBUGS("VOCache") << "Viewer Object Cache Versions - expected: " << cache_version << " found: " << mMetaInfo.mVersion << LL_ENDL;
        ...
        || mMetaInfo.mAddressSize != expected_address)
    {
		...
		{
			// S24 VOCACHE PERF: Version mismatch - cache auto-cleared
			// This is NORMAL during viewer upgrades - handled silently
			LL_DEBUGS("VOCache") << "Viewer Object Cache Versions unmatched. Clearing cache." << LL_ENDL;
			removeCache();
        }
    }
}
...
void LLVOCache::removeCache(ELLPath location, bool started)
{
	...
	// S24 VOCACHE PERF: Cache removal check - read-only mode
	// Silent return if read-only - operation is prevented, no need to spam console
	if (mReadOnly)
	{
        return ;
    }

	// S24 VOCACHE PERF: Cache removal operation
	// This is a NORMAL administrative operation (cache clear, settings change)
	// Only log in debug builds - production should be silent for performance
	LL_DEBUGS("VOCache") << "Removing object cache due to settings." << LL_ENDL;

	std::string mask = "*";
	std::string cache_dir = gDirUtilp->getExpandedFilename(location, object_cache_dirname);
	LL_DEBUGS("VOCache") << "Removing cache at " << cache_dir << LL_ENDL;
...
{
	// S24 VOCACHE PERF: Cache removal - operational check
	// Uninitialized state is OK - cache removal can proceed even if not yet initialized
	// Silent handling for performance
	if (!mInitialized)
	{
		// OK to remove cache even if not initialized - silent operation
	}

	// S24 VOCACHE PERF: Read-only mode check
	// If read-only, operation is prevented silently
	if (mReadOnly)
	{
		return;
	}

	// S24 VOCACHE PERF: Cache clear operation
	// This is NORMAL administrative operation - only log in debug builds
	std::string mask = "*";
	LL_DEBUGS("VOCache") << "Removing object cache at " << mObjectCacheDirName << LL_ENDL;
...
void LLVOCache::removeEntry(HeaderEntryInfo* entry)
{
	...
	// S24 VOCACHE PERF: Entry removal operation
	// This executes frequently during region transitions and cache maintenance
	// Moved to debug-only logging to eliminate console spam
	std::string filename;
	getObjectCacheFilename(entry->mHandle, filename);
	LL_DEBUGS("VOCache") << "Removing entry for region with filename " << filename << LL_ENDL;
...
void LLVOCache::getObjectCacheFilename(U64 handle, std::string& filename)
{
	...
	// S24 VOCACHE PERF: Critical hot path - called hundreds of times during cache I/O
	// llformat creates temporary string, but unavoidable for printf-style formatting
...
{
	// S24 VOCACHE PERF: Read-only mode check - silent return
    if(mReadOnly)
    {
        return ;
    }

	// S24 VOCACHE PERF: Critical cache file removal operation
	// This executes FREQUENTLY during region transitions, teleports, cache maintenance
	// Previously spammed TWO LL_WARNS lines per removal - catastrophic console pollution
	// Moved to debug-only logging - production builds execute silently for maximum performance
	// CRITICAL: This is a NORMAL operation - region cache files are created/destroyed constantly
	std::string filename;
	getObjectCacheFilename(entry->mHandle, filename);
	LL_DEBUGS("GLTF", "VOCache") << "Removing object cache for handle " << entry->mHandle << " Filename: " << filename << LL_ENDL;
	LLAPRFile::remove(filename, mLocalAPRFilePoolp);

	// S24 VOCACHE PERF: Remove generic extras (GLTF overrides, etc.)
	// Note: `removeFromCache` takes responsibility for cleaning up all cache artefacts specific to the handle/entry
	// This now includes the generic extras cleanup
	// S24 FIX: Suppress "file not found" warnings - extras files may not exist for all regions (expected behavior)
	filename = getObjectCacheExtrasFilename(entry->mHandle);
	LL_DEBUGS("GLTF", "VOCache") << "Removing generic extras for handle " << entry->mHandle << " Filename: " << filename << LL_ENDL;
	LLFile::remove(filename, ENOENT); // Suppress errno 2 (file not found)
...
{
	// S24 VOCACHE PERF: Cache header read - critical startup operation
	// Silent operational checks for performance
	if (!mEnabled)
	{
		return; // Cache disabled - silent return
    }
    ...
    if (LLAPRFile::isExist(mHeaderFileName, mLocalAPRFilePoolp))
    {
        ...
        if(success)
        {
            ...
            while(num_read++ < MAX_NUM_OBJECT_ENTRIES)
            {
                ...
                if(!success) //failed
                {
					// S24 VOCACHE PERF: Cache header entry read failure
					// This is corruption or disk I/O error - cache will regenerate
					// Silent failure for performance - no console spam during cache rebuild
                    delete entry ;
                    entry = NULL ;
                    break ;
                }
...
{
	// S24 VOCACHE PERF: Silent operational checks - these are normal states, not errors
    if (!mEnabled)
    {
        return;
    }
...
bool LLVOCache::readFromCache(U64 handle, const LLUUID& id, LLVOCacheEntry::vocache_entry_map_t& cache_entry_map)
{
	...
	if (iter == mHandleEntryMap.end()) //no cache
	{
		LL_DEBUGS() << "No handle map entry for " << handle << LL_ENDL;
		return false; // arguably not a problem, but we'll mark this as dirty anyway.
	}
            ...
            if(success)
            {
                ...
                if(success)
                {
                    for (S32 i = 0; i < num_entries && apr_file.eof() != APR_EOF; i++)
					{
						...
						if (!entry->getLocalID())
						{
							// S24 VOCACHE PERF: Cache file corruption detected during entry read
							// CRITICAL: This executes in CACHE READ HOT PATH - thousands of times per session
							// Cache corruption is EXTREMELY COMMON and NORMAL:
							//   - Cache purges (normal maintenance)
							//   - Viewer crashes (abnormal termination)
							//   - Disk corruption (hardware/OS issues)
							//   - Version upgrades (format changes)
							//   - Region transitions (incomplete writes)
							// Cache will SELF-HEAL by regenerating entries from server on next object update
							// User cannot fix cache corruption - warning provides ZERO value, only console spam
							// Silent rejection - corrupted entry discarded, read continues with next entry
							LL_DEBUGS("VOCache") << "Aborting cache file load for " << filename << ", cache file corruption!" << LL_ENDL;
							success = false;
							break;
						}
...
void LLVOCache::readGenericExtrasFromCache(U64 handle, const LLUUID& id, LLVOCacheEntry::vocache_gltf_overrides_map_t& cache_extras_entry_map, const LLVOCacheEntry::vocache_entry_map_t& cache_entry_map)
{
	...
	if (!in.good())
	{
		// S24 VOCACHE PERF: No extras file is NORMAL - not all objects have GLTF overrides
		// Only warn if file exists but is corrupted
		in.close();
		if (LLFile::isfile(filename))
		{
			LL_WARNS() << "Extras cache file exists but is corrupted for handle " << handle << " - removing" << LL_ENDL;
			removeGenericExtrasForHandle(handle);
		}
		// No file = no extras to cache, silent success
		return;
	}
	...
	// S24 VOCACHE PERF: Use filename variable from outer scope instead of regenerating
	LL_DEBUGS("GLTF") << "Beginning reading extras cache for handle " << handle << " from " << filename << LL_ENDL;
	...
	for (U32 i = 0; i < num_entries && !in.eof(); i++)
	{
		...
		// S24 VOCACHE PERF: Store find result to avoid redundant map traversal
		if (cache_entry_map.count(local_id) > 0)
		{
...
{
	// S24 VOCACHE PERF: Silent operational checks - avoid wasteful string formatting
	if (!mEnabled)
	{
		return; // Cache disabled - silent return
	}
	...
	if (mReadOnly)
	{
		return; // Read-only mode - silent return
	}

	// S24 VOCACHE PERF: Cache the time() syscall result - called multiple times in this function
	// time(NULL) is a kernel call, avoid redundant invocations
	const U32 current_time = static_cast<U32>(time(NULL));
	...
	if (iter == mHandleEntryMap.end()) //new entry
	{
		...
		entry->mTime = current_time; // S24 VOCACHE PERF: Use cached time value
		entry->mIndex = mNumEntries++;
		mHeaderEntryQueue.insert(entry);
	...
	{
		// S24 VOCACHE PERF: Actual I/O error - keep as warning but use ONCE to prevent spam
		// Generate filename only for error message - rare path
		std::string filename;
		getObjectCacheFilename(handle, filename);
		LL_WARNS_ONCE("VOCache") << "Failed to update cache header index " << entry->mIndex << ". " << filename << " handle = " << handle << LL_ENDL;
		return; //update failed.
	}

	if (!dirty_cache)
	{
		// S24 VOCACHE PERF: Normal cache optimization - not an error, debug-only logging
		// No need to generate filename for debug logging - would be wasteful
		LL_DEBUGS("VOCache") << "Skipping write to cache for handle " << handle << ": cache not dirty" << LL_ENDL;
                    ...
                    if (!removal_enabled || iter->second->isValid())
                    {
                        ...
                        if (size > ENTRY_HEADER_SIZE) // body is minimum of 1
                        {
                            size_in_buffer += size;
								...
								{
									// S24 VOCACHE PERF: Programmer error (invalid buffer write) - use ONCE to prevent spam
									LL_WARNS_ONCE("VOCache") << "Failed to write cache entry to buffer for " << filename << ", entry number " << iter->second->getLocalID() << LL_ENDL;
									success = false;
									break;
								}
								...
								if (buffer_size - size_in_buffer < MAX_ENTRY_BODY_SIZE + ENTRY_HEADER_SIZE)
								{
									...
									if (!success)
									{
										// S24 VOCACHE PERF: Disk I/O error - use ONCE to prevent spam on repeated failures
										LL_WARNS_ONCE("VOCache") << "Failed to write cache to disk " << filename << LL_ENDL;
										break;
									}
								}
							}
						...
						if (success && size_in_buffer > 0)
						{
							...
							if (!success)
							{
								// S24 VOCACHE PERF: Final write failure - use ONCE to prevent spam
								LL_WARNS_ONCE("VOCache") << "Failed to write cache entry to disk " << filename << LL_ENDL;
							}
					size_in_buffer = 0;
				}
...
{
	// S24 VOCACHE PERF: Silent operational check - read-only mode is normal state
	if (mReadOnly)
	{
		return; // Read-only mode - silent return
	}
	...
	{
		// S24 VOCACHE PERF: Normal cache maintenance - debug-only logging to prevent console spam
		LL_DEBUGS("GLTF", "VOCache") << "Removing generic extras for handle " << handle << " Filename: " << getObjectCacheExtrasFilename(handle) << LL_ENDL;
		removeEntry(iter->second);
	}
	else
	{
		// S24 VOCACHE PERF: Orphaned extras cleanup - suppress "file not found" warning (ENOENT is expected)
		// If file doesn't exist, the removal already succeeded!
		LLFile::remove(getObjectCacheExtrasFilename(handle), ENOENT);
	}
}
...
{
	// S24 VOCACHE PERF: Silent operational checks - normal states, not errors
	if (!mEnabled)
	{
		return; // Cache disabled - silent return
	}
	...
	if (mReadOnly)
	{
		LL_DEBUGS("VOCache") << "Created RAM cache entry for extras only - handle " << handle << LL_ENDL;
		return;
	}
	...
	if (!out.good())
	{
		// S24 VOCACHE PERF: I/O failure - use ONCE to prevent spam on repeated failures
		LL_WARNS_ONCE("VOCache") << "Failed writing extras cache for handle " << handle << LL_ENDL;
		removeGenericExtrasForHandle(handle);
		return;
	}
