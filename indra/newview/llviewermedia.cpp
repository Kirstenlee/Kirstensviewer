...
#include <boost/bind/bind.hpp>   // for SkinFolder listener
...
LLSD LLViewerMedia::getHeaders()
{
	...
	headers[std::string(HTTP_OUT_HEADER_CONTENT_TYPE)] = std::string(HTTP_CONTENT_XML);
	headers[std::string(HTTP_OUT_HEADER_COOKIE)] = mOpenIDCookie;
	headers[std::string(HTTP_OUT_HEADER_USER_AGENT)] = getCurrentUserAgent();
...
LLCore::HttpHeaders::ptr_t LLViewerMedia::getHttpHeaders()
{
	...
	headers->append(std::string{HTTP_OUT_HEADER_ACCEPT}, "*/*");
	headers->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, std::string{HTTP_CONTENT_XML});
	headers->append(std::string{HTTP_OUT_HEADER_COOKIE}, mOpenIDCookie);
	headers->append(std::string{HTTP_OUT_HEADER_USER_AGENT}, getCurrentUserAgent());
...
void LLViewerMedia::getOpenIDCookieCoro(std::string url)
{
	...
	httpHeaders->append(std::string{HTTP_OUT_HEADER_ACCEPT}, "*/*");
	httpHeaders->append(std::string{HTTP_OUT_HEADER_COOKIE}, inst->mOpenIDCookie);
	httpHeaders->append(std::string{HTTP_OUT_HEADER_USER_AGENT}, inst->getCurrentUserAgent());
...
void LLViewerMedia::openIDSetupCoro(std::string openidUrl, std::string openidToken)
{
	...
	httpHeaders->append(std::string{HTTP_OUT_HEADER_ACCEPT}, "*/*");
	httpHeaders->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, "application/x-www-form-urlencoded");
...
void LLViewerMedia::createSpareBrowserMediaSource()
{
	...
	if (!mSpareBrowserMediaSource && !gSavedSettings.getBOOL("PluginAttachDebuggerToPlugins"))
	{
		...
		mSpareBrowserMediaSource = LLViewerMediaImpl::newSourceFromMediaType(std::string{HTTP_CONTENT_TEXT_HTML}, NULL, 0, 0, 1.0);
	}
}
...
void LLViewerMediaImpl::loadURI()
{
	if (mMediaSource)
	{
		...
		std::string uri = LLURI::escapePathAndData(mMediaURL);
		{
				...
				LL_DEBUGS("Media") << "loadURI: " << sanitized_uri << LL_ENDL;
			}
...
void LLViewerMediaImpl::navigateTo(const std::string& url, const std::string& mime_type, bool rediscover_type, bool server_request, bool clean_browser)
{
	...
	if (mPriority == LLPluginClassMedia::PRIORITY_UNLOADED)
	{
		// Helpful to have media urls in log file. Shouldn't be spammy.
		{
			...
			LL_DEBUGS("PluginPriority") << "Media unloaded (PRIORITY_UNLOADED): id=" << mTextureId << " url=" << sanitized_url << LL_ENDL;
		}
...
void LLViewerMediaImpl::navigateInternal(bool should_log)
{
	// Helpful to have media urls in log file. Shouldn't be spammy.
	{
		...
		if (should_log)
		{
			LL_DEBUGS("Media") << "Navigate: id=" << mTextureId << " url=" << sanitized_url << " mime=" << mMimeType << LL_ENDL;
	}
		else
        {
	...
	if (mNavigateSuspended)
	{
		...
		{
		LL_DEBUGS("Media") << "Navigate suspended, deferring." << LL_ENDL;
		}
	...
	if (!mMimeProbe.expired())
	{
		...
		LL_DEBUGS("Media") << "MIME probe in progress, deferring navigate." << LL_ENDL;
		}
	return;
	}
	...
	if (mNavigateRediscoverType)
	{
		...
		else if ("data" == scheme || "file" == scheme || "about" == scheme)
		{
			...
			if (initializeMedia(std::string{HTTP_CONTENT_TEXT_HTML}))
			{
				loadURI();
			}
		}
...
void LLViewerMediaImpl::mimeDiscoveryCoro(std::string url)
{
	...
	httpHeaders->append(std::string{HTTP_OUT_HEADER_ACCEPT}, "*/*");
	httpHeaders->append(std::string{HTTP_OUT_HEADER_COOKIE}, "");
	...
	if (!status)
	{
		LL_DEBUGS("Media") << "HTTP probe failed (expected for file:// URLs): " << status.toString() << LL_ENDL;
	}
	...
	{
		LL_DEBUGS("Media") << "Media impl ref count=1, releasing." << LL_ENDL;
	}
...
void LLViewerMediaImpl::handleMediaEvent(LLPluginClassMedia* plugin, LLPluginClassMediaOwner::EMediaEvent event)
{
	...
	case LLViewerMediaObserver::MEDIA_EVENT_CLOSE_REQUEST:
	{
		...
		LL_DEBUGS("Media") << "MEDIA_EVENT_CLOSE_REQUEST uuid=" << uuid << LL_ENDL;
	...
	case LLViewerMediaObserver::MEDIA_EVENT_GEOMETRY_CHANGE:
	{
		...
		LL_DEBUGS("Media") << "MEDIA_EVENT_GEOMETRY_CHANGE uuid=" << uuid << LL_ENDL;
