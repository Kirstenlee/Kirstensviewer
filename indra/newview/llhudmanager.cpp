...
void LLHUDManager::sendEffects()
{
    ...
    for (i = 0; i < mHUDEffects.size(); i++)
    {
        ...
        if (hep->isDead())
        {
            // S24: PERFORMANCE - Removed pointless warning log
            // Dead effects are gracefully skipped (not sent to sim)
            // This is normal operation during effect lifecycle cleanup
            // cleanupEffects() properly removes them later
            // Logging here just wastes CPU cycles in the HUD rendering hot path
            continue;
        }
        if (hep->mType < LLHUDObject::LL_HUD_EFFECT_BEAM)
        {
            // S24: PERFORMANCE - Removed pointless warning log
            // Non-effect HUD objects are gracefully skipped (not sent to sim)
            // This is normal - some HUD objects shouldn't be in this list but are handled correctly
            // Logging here just wastes CPU cycles in the HUD rendering hot path
            continue;
        }
