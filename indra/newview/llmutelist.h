...
public:
    ...
    // S24: Track if mute list was loaded from cache fallback (server timeout)
    // This allows UI to show degraded state if needed, and helps debugging
	...
	// S24: Apply object derender using the SAME logic as updateAdd() that works per-session.
	// This is called after mute list loads to apply derender to all muted objects.
	void applyObjectDerender();

	// S24: Enforce derender for muted objects in a specific region after createVisibleObjects.
	// Called from LLViewerRegion::idleUpdate() to catch teleport/region-cross visibility.
	void enforceRegionDerender(LLViewerRegion* region);

   private:
    void clearCachedMutes();
    bool loadFromFile(const std::string& filename, EMuteListSource source);
    bool saveToFile(const std::string& filename);
    // LL standard cache fallback function (called from updateLoadState)
