...
public:
	...
	void setThumbnailPlaceholderRect(const LLRect& rect)
	{
		// S24 Snapshot revamp: invalidate the cached thumbnail when the
		// placeholder is resized so the preview regenerates at the new size
		// without a manual Refresh. Use a small threshold so per-frame layout
		// jitter (1px) does NOT cause continuous regeneration (fps killer).
		const S32 dw = llabs(rect.getWidth() - mThumbnailPlaceholderRect.getWidth());
		const S32 dh = llabs(rect.getHeight() - mThumbnailPlaceholderRect.getHeight());
		if (dw > 2 || dh > 2)
		{
			mThumbnailUpToDate = false;
			mBigThumbnailUpToDate = false;
		}
		mThumbnailPlaceholderRect = rect;
	}
	...
	// Composition guides overlay (S24 Snapshot revamp - Phase 2).
	// Draws non-destructive framing aids inside the given absolute screen rect
	// (left, bottom, width, height). The caller is responsible for passing the
	// actual on-screen pixels of the image being shown.
	void drawGuides(S32 left, S32 bottom, S32 width, S32 height);
