...
void LLMediaCtrl::navigateToLocalPage( const std::string& subdir, const std::string& filename_in )
{
    ...
    if (ensureMediaSourceExists())
    {
        ...
        mMediaSource->setSize(mTextureWidth, mTextureHeight);
        mMediaSource->navigateTo(expanded_filename, std::string{HTTP_CONTENT_TEXT_HTML}, false);
    }
}
...
void LLMediaCtrl::draw()
{
    ...
    if(draw_media)
    {
        gDX.pushUIMatrix();
        {
            ...
            gDX.getTexUnit(0)->bind(media_texture);
            LLColor4 media_color = LLColor4::white % alpha;
            gDX.color4fv( media_color.mV );
            ...
            if (! media_plugin->getTextureCoordsOpenGL())
            {
                ...
                gDX.texCoord2f( max_u, 0.f );
                gDX.vertex2i( x_offset + width, y_offset + height );

                gDX.texCoord2f( 0.f, 0.f );
                gDX.vertex2i( x_offset, y_offset + height );

                gDX.texCoord2f( 0.f, max_v );
                gDX.vertex2i( x_offset, y_offset );
                ...
                gDX.texCoord2f( max_u, max_v );
                gDX.vertex2i( x_offset + width, y_offset );
            }
            else
            {
                // render using web browser reported width and height, instead of trying to invert GL scale
...
void LLMediaCtrl::handleMediaEvent(LLPluginClassMedia* self, EMediaEvent event)
{
    switch(event)
    {
        ...
        case MEDIA_EVENT_NAVIGATE_ERROR_PAGE:
        {
            ...
            {
                navigateTo(mErrorPageURL, std::string{HTTP_CONTENT_TEXT_HTML});
            };
        };
        break;
