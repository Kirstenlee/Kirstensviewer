    ...
    if (rv.isNull() && root_id.notNull())
    {
        ...
        if (isInventoryUsable())
        {
                ...
                [preferred_type](const LLUUID &new_cat_id)
            {
                    ...
                    {
                        // S24: Changed to LL_DEBUGS - creation failure already handled by caller
                        // This is a callback notification, not a primary error site
                        LL_DEBUGS("Inventory")
                    ...
                    {
                        // S24: Changed to LL_DEBUGS - successful folder creation is not a warning
                        // This was logging success as LL_WARNS (incorrect log level)
        ...
        {
            // S24: Changed to LL_DEBUGS - inventory not usable during init is normal
            // This fires during startup before inventory fully loads from server
            LL_DEBUGS("Inventory") << "Can't create requested folder, type " << preferred_type
                << " because inventory is not usable yet" << LL_ENDL;
        }
    }
    else
    {
    ...
    const LLUUID& root_id) const
{
    ...
    {
        // S24: Changed to LL_DEBUGS - folder not existing yet is normal during initialization
        // if it does not exists, it should either be added
        // to createCommonSystemCategories or server should have set it
        // This fires during startup before folders are created - expected behavior
        // The llassert below confirms this is only expected when inventory not ready
        llassert(!isInventoryUsable());
        LL_DEBUGS("Inventory") << "Folder type " << preferred_type
                                  << " does not exist yet (will be created)" << LL_ENDL;
    }
    return rv;
}
                                           ...
                                           const LLUUID& thumbnail_id)
{
    ...
    if (!isInventoryUsable())
    {
        // S24: Changed to LL_DEBUGS - inventory not usable during creation is normal at startup
        // User/code attempting to create folder before inventory loads from server
        LL_DEBUGS(LOG_INV) << "Inventory is not usable yet; can't create requested category of type "
    ...
    {
        // S24: Changed to LL_DEBUGS - creating system folders is normal operation, not a warning
        // Ultimately this should only be done for non-singleton
        // types. Requires back-end changes to guarantee that others already exist.
        LL_DEBUGS(LOG_INV) << "Creating new system folder, type " << preferred_type << LL_ENDL;
    }
...
void LLInventoryModel::initHttpRequest()
{
    if (! mHttpRequestFG)
    {
        ...
        mHttpHeaders->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, std::string{HTTP_CONTENT_LLSD_XML});
        mHttpHeaders->append(std::string{HTTP_OUT_HEADER_ACCEPT}, std::string{HTTP_CONTENT_LLSD_XML});
