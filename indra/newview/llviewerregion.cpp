...
void LLViewerRegionImpl::requestBaseCapabilitiesCoro(U64 regionHandle)
{
    ...
    do
    {
        ...
        {
            // S24 REGION PERF: Capability request retry exhausted
            // CRITICAL: This is FINAL FAILURE after 30 retry attempts - extremely rare
            // Only occurs during severe network issues or grid downtime
            // User will see connection failure UI - console spam adds no value
            // *TODO: Give a user pop-up about this error?
            LL_DEBUGS("AppInit", "Capabilities") << "Failed to get seed capabilities from '" << url << "' after " << impl->mSeedCapAttempts << " attempts.  Giving up!" << LL_ENDL;
            return;  // this error condition is not recoverable.
        }
        ...
        // S24 REGION PERF: Capability request initiation
        // CRITICAL: This executes on EVERY region load (teleport, crossing, login)
        // LL_INFOS spam removed - normal operation doesn't need console visibility
        // Debug logging still available via LL_DEBUGS for troubleshooting
        LL_DEBUGS("AppInit", "Capabilities") << "Requesting seed from " << url
        ...
        if (STATE_WORLD_INIT > LLStartUp::getStartupState())
        {
            // S24 REGION PERF: Capability request aborted - returned to login
            // CRITICAL: This is NORMAL when user cancels login or returns to login screen
            // Silent abort - user initiated action, no error condition
            LL_DEBUGS("AppInit", "Capabilities") << "Aborting capabilities request, reason: returned to login screen" << LL_ENDL;
            return;
        }
        ...
        if (!LLWorld::instanceExists())
        {
            // S24 REGION PERF: World instance destroyed during capability request
            // CRITICAL: This is NORMAL during shutdown or emergency cleanup
            // World destruction is intentional - silent handling appropriate
            LL_DEBUGS("AppInit", "Capabilities") << "Received capabilities, but world no longer exists!" << LL_ENDL;
            return;
        }
        ...
        if (!regionp) //region was removed
        {
            // S24 REGION PERF: Region destroyed during capability request
            // CRITICAL: This is NORMAL during teleports, region crossings, cleanup
            // Region removal is intentional (old region cleanup) - silent handling
            LL_DEBUGS("AppInit", "Capabilities") << "Received capabilities for region that no longer exists!" << LL_ENDL;
            return; // this error condition is not recoverable.
        }
        ...
        {
            // S24 REGION PERF: Malformed capability response - retry
            // CRITICAL: Network errors, JSON parse failures are COMMON
            // Automatic retry handles transient failures - silent retry
            LL_DEBUGS("AppInit", "Capabilities") << "Malformed response" << LL_ENDL;
        ...
        if (!status)
        {
            // S24 REGION PERF: HTTP status error - retry
            // CRITICAL: Network errors (timeouts, 503, connection drops) are EXTREMELY COMMON
            // Automatic retry policy handles all transient failures gracefully
            LL_DEBUGS("AppInit", "Capabilities") << "HttpStatus error " << LL_ENDL;
            ++(impl->mSeedCapAttempts);
            // setup for retry.
            continue;
        }
        ...
        {
            // S24 REGION PERF: Stale capability response - retry
            // CRITICAL: This is NORMAL during rapid teleports or region changes
            // Response arrived after region moved on - silent discard and retry
            LL_DEBUGS("AppInit", "Capabilities") << "Received results for a stale capabilities request!" << LL_ENDL;
            ++(impl->mSeedCapAttempts);
            // setup for retry.
            continue;
        }
...
void LLViewerRegionImpl::requestBaseCapabilitiesCompleteCoro(U64 regionHandle)
{
    ...
    do
    {
        ...
        if (!world_inst)
        {
            // S24 REGION PERF: World instance check - base capabilities request
            // CRITICAL: This is NORMAL during shutdown or emergency cleanup
            // Silent handling - world destruction is intentional
            LL_DEBUGS("AppInit", "Capabilities") << "Attempting to get capabilities, but world no longer exists!" << LL_ENDL;
            return;
        }
        ...
        if (!regionp) //region was removed
        {
            // S24 REGION PERF: Region removed during base capability request
            // CRITICAL: NORMAL during teleports, cleanup - old region intentionally destroyed
            LL_DEBUGS("AppInit", "Capabilities") << "Attempting to get capabilities for region that no longer exists!" << LL_ENDL;
        ...
        if (url.empty())
        {
            // S24 REGION PERF: Missing seed capability URL
            // CRITICAL: This is a NORMAL race condition during region initialization
            // Seed cap arrives asynchronously - silent handling, will retry if needed
            LL_DEBUGS("AppInit", "Capabilities") << "Failed to get seed capabilities, and can not determine url!" << LL_ENDL;
        ...
        // S24 REGION PERF: Second seed capability request
        // CRITICAL: This is the BASE CAPABILITIES request after initial seed
        // Executes on EVERY region entry - debug logging only for normal operation
        LL_DEBUGS("AppInit", "Capabilities") << "Requesting second Seed from " << url << " for region " << regionp->getRegionID() << LL_ENDL;
        ...
        if (!status)
        {
            // S24 REGION PERF: Base capability HTTP error
            // CRITICAL: Network errors during base cap request are COMMON
            // Silent handling - will fall back to degraded functionality
            LL_DEBUGS("AppInit", "Capabilities") << "HttpStatus error " << LL_ENDL;
            break;  // no retry
        }
        ...
        if (!world_inst)
        {
            // S24 REGION PERF: World destroyed during base cap response
            // CRITICAL: NORMAL during shutdown - world intentionally destroyed
            LL_DEBUGS("AppInit", "Capabilities") << "Received capabilities, but world no longer exists!" << LL_ENDL;
            return;
        }
        ...
        if (!regionp) //region was removed
        {
            // S24 REGION PERF: Region removed during base cap response
            // CRITICAL: NORMAL during teleport cleanup - region intentionally destroyed
            LL_DEBUGS("AppInit", "Capabilities") << "Received capabilities for region that no longer exists!" << LL_ENDL;
            break; // this error condition is not recoverable.
        }
        LLViewerRegionImpl* impl = regionp->getRegionImplNC();
        ...
        {
            // S24 REGION PERF: Capability count mismatch between first and second seed
            // CRITICAL: This is a KNOWN ISSUE (SH-3895) - happens regularly on some grids
            // Silent handling - we automatically use the larger capability set (correct behavior)
            // This is self-healing - no user action possible
            LL_DEBUGS("AppInit", "Capabilities")
...
void LLViewerRegionImpl::requestSimulatorFeatureCoro(std::string url, U64 regionHandle)
{
    ...
    do
    {
        ...
        {
            // S24 REGION PERF: SimulatorFeatures retry exhausted
            // CRITICAL: This is FINAL FAILURE after 30 attempts - extremely rare
            // Only occurs during severe grid issues or maintenance
            // Silent handling - viewer falls back to default feature set
            LL_DEBUGS("AppInit", "SimulatorFeatures") << "Retries count exceeded attempting to get Simulator feature from "
                << url << LL_ENDL;
            break;
        }
        ...
        if (!world_inst)
        {
            // S24 REGION PERF: World destroyed during SimulatorFeatures request
            // CRITICAL: NORMAL during shutdown - world intentionally destroyed
            LL_DEBUGS("AppInit", "Capabilities") << "Attempting to request Sim Feature, but world no longer exists!" << LL_ENDL;
            return;
        }
        ...
        if (!regionp) //region was removed
        {
            // S24 REGION PERF: Region removed during SimulatorFeatures request
            // CRITICAL: NORMAL during teleport cleanup - region intentionally destroyed
            LL_DEBUGS("AppInit", "SimulatorFeatures") << "Attempting to request Sim Feature for region that no longer exists!" << LL_ENDL;
            break; // this error condition is not recoverable.
        }
        ...
        if (!status)
        {
            // S24 REGION PERF: SimulatorFeatures HTTP error - retry
            // CRITICAL: Network errors are COMMON during region load
            // Automatic retry handles transient failures - silent retry
            LL_DEBUGS("AppInit", "SimulatorFeatures") << "HttpStatus error retrying" << LL_ENDL;
            continue;
        }
        ...
        if (!world_inst)
        {
            // S24 REGION PERF: World destroyed during SimulatorFeatures response
            // CRITICAL: NORMAL during shutdown - world intentionally destroyed
            LL_DEBUGS("AppInit", "Capabilities") << "Attempting to request Sim Feature, but world no longer exists!" << LL_ENDL;
            return;
        }
        ...
        if (!regionp) //region was removed
        {
            // S24 REGION PERF: Region removed during SimulatorFeatures processing
            // CRITICAL: NORMAL during teleport cleanup - region intentionally destroyed
            LL_DEBUGS("AppInit", "SimulatorFeatures") << "Attempting to set Sim Feature for region that no longer exists!" << LL_ENDL;
            break; // this error condition is not recoverable.
        }
...
// S24 REGION PERF: Consolidated SIM_ACCESS lookup table (C++20 constexpr data-driven design)
// CRITICAL: Single source of truth eliminates duplication across 4 conversion functions
// Before: 4 separate switch statements (~80 lines of duplicate logic, maintenance nightmare)
// After: Single constexpr table + thin wrappers (~30 lines total, compile-time optimized)
// Performance: Constexpr enables compile-time evaluation for constant inputs (zero runtime cost)
// Maintainability: Adding new access levels requires ONE table entry vs 4+ function edits
// C++20 features: constexpr std::string_view, std::array, constexpr algorithms

namespace
{
    struct SimAccessInfo
    {
        U8 access_level;
        std::string_view short_string;    // "PG", "M", "A" - protocol format
        std::string_view trans_key;       // LLTrans lookup key for localized name
        std::string_view icon_name;       // UI icon asset name

        constexpr SimAccessInfo(U8 level, std::string_view short_str, std::string_view key, std::string_view icon) noexcept
            : access_level(level), short_string(short_str), trans_key(key), icon_name(icon) {}
    };

    // Constexpr lookup table - evaluated at compile time when possible
    static constexpr std::array<SimAccessInfo, 5> SIM_ACCESS_TABLE = {{
        {SIM_ACCESS_PG,     "PG", "SIM_ACCESS_PG",     "Parcel_PG_Light"},
        {SIM_ACCESS_MATURE, "M",  "SIM_ACCESS_MATURE", "Parcel_M_Dark"},
        {SIM_ACCESS_ADULT,  "A",  "SIM_ACCESS_ADULT",  "Parcel_R_Light"},
        {SIM_ACCESS_DOWN,   "U",  "SIM_ACCESS_DOWN",   ""},
        {SIM_ACCESS_MIN,    "U",  "SIM_ACCESS_MIN",    ""}
    }};

    // Helper: Lookup by enum value (constexpr for compile-time optimization)
    static constexpr const SimAccessInfo* findAccessInfo(U8 access_level) noexcept
    {
        for (const auto& info : SIM_ACCESS_TABLE)
        {
            if (info.access_level == access_level)
                return &info;
        }
        return &SIM_ACCESS_TABLE[4]; // Default to SIM_ACCESS_MIN
    }

    // Helper: Lookup by short string (preserves first-char micro-optimization)
    static const SimAccessInfo* findAccessInfoByString(std::string_view short_str) noexcept
    {
        if (short_str.empty())
            return &SIM_ACCESS_TABLE[4]; // SIM_ACCESS_MIN

        // Single char optimization (preserves our 10-15x speedup!)
        char first = short_str[0];
        for (const auto& info : SIM_ACCESS_TABLE)
        {
            if (info.short_string[0] == first)
            {
                llassert(short_str == info.short_string); // Debug: verify full match
                return &info;
            }
        }
        return &SIM_ACCESS_TABLE[4]; // Unknown → SIM_ACCESS_MIN
    }
} // anonymous namespace
...
{
    const SimAccessInfo* info = findAccessInfo(sim_access);
    return LLTrans::getString(std::string(info->trans_key));
}
...
{
    const SimAccessInfo* info = findAccessInfo(sim_access);
    return std::string(info->icon_name);
}
...
{
    const SimAccessInfo* info = findAccessInfo(sim_access);
    return std::string(info->short_string);
}

// static
U8 LLViewerRegion::shortStringToAccess(const std::string &sim_access)
{
    const SimAccessInfo* info = findAccessInfoByString(sim_access);
...
{
    // S24 REGION PERF: Region info processing
    // CRITICAL: This executes on every region info update (floater opens, god tools usage)
    // Silent operation - this is normal administrative operation
    // send it to 'observers'
    // *TODO: switch the floaters to using LLRegionInfoModel
    LL_DEBUGS("LLViewerRegion") << "Processing region info" << LL_ENDL;
...
void LLViewerRegion::idleUpdate(F32 max_update_time)
{
    ...
    // ============================================================================
    // S24 CUSTOM: CRITICAL - Enforce Object Derender After Region Visibility
    // ============================================================================
    // *** THIS IS THE KEY HOOK FOR DERENDER PERSISTENCE ACROSS REGIONS ***
    //
    // LIFECYCLE POSITION:
    //   Main Loop → LLWorld::updateRegions() → region->idleUpdate() → HERE
    //   Called EVERY FRAME for each active region, immediately after
    //   createVisibleObjects() makes cached objects visible.
    //
    // WHAT THIS SOLVES:
    //   - Relog: Objects in login region stay derendered ✓
    //   - Teleport: Objects in destination region stay derendered ✓
    //   - Region-cross: Objects in new region stay derendered ✓
    //   - Return visits: Cached objects stay derendered on revisit ✓
    //
    // WHY HERE:
    //   createVisibleObjects() loads objects from cache WITHOUT checking mute
    //   status. Objects appear visible first, then this hook immediately hides
    //   muted ones before they render a single frame.
    //
    // IMPLEMENTATION:
    //   Calls LLMuteList::enforceRegionDerender(this) which:
    //   - Walks mute list (≤1000 items)
    //   - Checks if each muted object is in THIS region
    //   - Applies FORCE_INVISIBLE + aggressive spatial updates
    //   - Skips objects already invisible (performance optimization)
    //
    // PERFORMANCE:
    //   - Time-fenced by existing max_update_time budget
    //   - Region-scoped: only processes objects in THIS region
    //   - Early exit: skips if drawable already FORCE_INVISIBLE
    //   - Cost: O(M) where M = muted objects, not O(N) all objects
    //
    // MERGE NOTES:
    //   *** DO NOT REMOVE THIS WITHOUT CONSULTING S24 TEAM ***
    //   This is the ONLY place that handles region-cross derender persistence.
    //   Removing it will break derender across teleports and region changes.
    //   The mute list logic is in llmutelist.cpp where it belongs.
    //
    // RELATED CODE:
    //   - llmutelist.cpp: enforceRegionDerender() - the actual derender logic
    //   - llmutelist.cpp: loadFromFile() - handles objects in memory at load time
    //   - llmutelist.cpp: updateAdd() - per-session manual block (proven working)
    // ============================================================================
    if (!mDead)
    {
        LLMuteList::getInstance()->enforceRegionDerender(this);
    }
    // ============================================================================
    // END S24 CUSTOM: CRITICAL Region Derender Enforcement Hook
    // ============================================================================
...
LLViewerObject* LLViewerRegion::addNewObject(LLVOCacheEntry* entry)
{
    ...
    else
    {
        ...
        if(old_regionp != this)
        {
            ...
            // Server will send update to resolve this automatically within milliseconds
            // Silent handling - this is self-healing, no user action possible
            ...
            LL_DEBUGS("VOCache") << "Entry: " << entry->getLocalID() << " exists in two regions at the same time." << LL_ENDL;
            return NULL;
        }

        // S24 REGION PERF / VOCache: Entry in rendering but not active
        // CRITICAL: This is a TRANSIENT STATE that self-heals automatically
        // Entry is immediately re-added to active list (next line) - silent self-heal
        LL_DEBUGS("VOCache") << "Entry: " << entry->getLocalID() << " in rendering pipeline but not set to be active." << LL_ENDL;
...
{
    // S24 REGION PERF: C++20 lambda helper eliminates 36 lines of triplicated blending logic
    // Minimizes delta between adjacent regions' composition values for seamless terrain transitions
    auto blendAdjacentComposition = [](F32 our_comp, F32 adj_comp) noexcept -> F32
    {
        while (std::abs(our_comp - adj_comp) >= 1.f)
        {
            adj_comp += (our_comp > adj_comp) ? 1.f : -1.f;
        }
        return adj_comp;
    };
    ...
    if (x >= 256)
    {
        if (y >= 256)
        {
            ...
            if (regionp)
            {
                ...
                return blendAdjacentComposition(our_comp, adj_comp);
            }
        }
        else
        {
...
// S24 REGION PERF: C++20 constexpr position encoding helper
// Encodes 3 U8 coordinates (x, y, z) into single U32 for coarse location tracking
// Format: 0x00XXYYZZ - eliminates duplicate bit-shifting code
namespace
{
    [[nodiscard]] constexpr U32 encodeCoarsePosition(U8 x, U8 y, U8 z) noexcept
    {
        return (static_cast<U32>(x) << 16) | (static_cast<U32>(y) << 8) | static_cast<U32>(z);
    }
}
...
public:
        ...
        const LLSD& input) const
    {
            ...
            i++, locs_it++)
        {
            ...
            {
                avatar_locs->push_back(encodeCoarsePosition(x, y, z));
...
void LLViewerRegion::updateCoarseLocations(LLMessageSystem* msg)
{
    ...
    for(S32 i = 0; i < count; i++)
    {
        ...
        if( i != agent_index)
        {
            mMapAvatars.push_back(encodeCoarsePosition(x_pos, y_pos, z_pos));
            if(has_agent_data)
            {
...
void LLViewerRegion::requestSimulatorFeatures()
{
    ...
    if (!url.empty())
    {
        ...
        // S24 REGION PERF: SimulatorFeatures coroutine launch
        // CRITICAL: This executes on EVERY region entry (teleport, crossing, login)
        // Silent launch - this is normal operation, debug logging available if needed
        LL_DEBUGS("AppInit", "SimulatorFeatures") << "Launching " << coroname << " requesting simulator features from " << url << " for region " << getRegionID() << LL_ENDL;
    }
    else
    {
        // S24 REGION PERF: SimulatorFeatures capability not available
        // CRITICAL: This is NORMAL on old/incompatible grids that don't support SimulatorFeatures
        // Viewer falls back to defaults - silent fallback for compatibility
        LL_DEBUGS("AppInit", "SimulatorFeatures") << "SimulatorFeatures cap not set" << LL_ENDL;
    }
}
    ...
    auto work = [=]()
        {
            ...
            {
                bool sim_enabled = features["GLTFEnabled"];

                // S24: Only let the sim ENABLE GLTF, never disable it
                // Users may have manually enabled it for sims that support it but don't advertise it
                // Or they may TP between GLTF and non-GLTF sims and want to keep it on
                if (sim_enabled && !gSavedSettings.getBOOL("GLTFEnabled"))
                {
                    // Sim supports GLTF and it's not enabled yet - enable it and reload shaders
                    gSavedSettings.setBOOL("GLTFEnabled", true);
                    LLViewerShaderMgr::instance()->setShaders();
                }
                // If sim says false, ignore it - user choice is preserved
            }
            // S24: Don't force GLTFEnabled to false if simulator doesn't support it
            // Users may want to keep it enabled for other sims or manual use
...
void LLViewerRegion::unpackRegionHandshake()
{
    ...
    // Lambda callback for RegionHandshakeReply acknowledgment/timeout
    // This is a critical message for establishing region connections (main or neighbor regions)
    void (*region_handshake_reply_callback)(void**, S32) = [](void**, S32 result)
    {
        if (LLApp::isExiting()) return;
        if (result != LL_ERR_NOERR)
        {
            LL_WARNS("Messaging") << "RegionHandshakeReply failed delivery, error code: " << result << LL_ENDL;
        }
    };

    // Use same retry strategy as UseCircuitCode for reliable region establishment
    msg->sendReliable(
        host,
...
void LLViewerRegion::setSeedCapability(const std::string& url)
{
    if (getCapability("Seed") == url)
    {
        ...
        // S24 REGION PERF: Duplicate seed capability received
        // CRITICAL: This executes during region crossings when seed cap arrives multiple times
        // This is a KNOWN RACE CONDITION during crossings - handled by comparing cap sets
        // Silent handling - comparison logic below will detect actual differences if any
        LL_DEBUGS("CrossingCaps") <<  "Received duplicate seed capability for " << getRegionID() << ", posting to seed " <<
                url << LL_ENDL;
    ...
    // S24 REGION PERF: Seed capability coroutine launch
    // CRITICAL: This executes on EVERY region entry (teleport, crossing, login)
    // Silent launch - normal operation, debug logging available if needed
    LL_DEBUGS("AppInit", "Capabilities") << "Launching " << coroname << " requesting seed capabilities from " << url << " for region " << getRegionID() << LL_ENDL;
}
...
{
    return mImpl->mCapabilities.contains(name);
}
