...
void LLWebProfile::uploadImageCoro(LLPointer<LLImageFormatted> image, std::string caption, bool addLocation)
{
    ...
    httpHeaders->append(std::string(HTTP_OUT_HEADER_COOKIE), getAuthCookie());
    ...
    httpHeaders->append(std::string(HTTP_OUT_HEADER_COOKIE), getAuthCookie());
    httpHeaders->remove(std::string(HTTP_OUT_HEADER_CONTENT_TYPE));
    httpHeaders->append(std::string(HTTP_OUT_HEADER_CONTENT_TYPE), "multipart/form-data; boundary=" + boundary);
