...
#include "lldir.h" // S24
#include "lldiriterator.h" // S24
#include "lldirpicker.h"
#include "llxmlnode.h" // S24
#include "lleventtimer.h"
#include "llfeaturemanager.h"
#include "llfile.h" // S24
    ...
    mSearchDataDirty(true)
{
	...
	// S24
    mCommitCallbackRegistrar.add("Pref.KVTweaks",               boost::bind(&LLFloaterPreference::onCLickKVTweaks, this));
...
// S24 - Modified to use combo box for dynamic skin selection
void LLFloaterPreference::onSelectSkin()
{
    LLComboBox* skin_combo = getChild<LLComboBox>("skin_selection");
    if (!skin_combo) return;

    std::string skin_selection = skin_combo->getValue().asString();
    gSavedSettings.setString("SkinCurrent", skin_selection);
}

// S24 - Dynamically populate skin list from skin_registry.xml
// Add new skins by editing skins/skin_registry.xml - no C++ recompile needed!
void LLFloaterPreference::populateSkinList(LLComboBox* skin_combo)
{
    if (!skin_combo) return;

    skin_combo->clearRows();

    // Load the skin registry XML file
    std::string registry_path = gDirUtilp->getExpandedFilename(LL_PATH_SKINS, "skin_registry.xml");

    LLXMLNodePtr root;
    if (!LLXMLNode::parseFile(registry_path, root, NULL))
    {
        LL_WARNS("Preferences") << "Failed to parse skin registry: " << registry_path << LL_ENDL;
        return;
    }

    if (!root->hasName("skin_registry"))
    {
        LL_WARNS("Preferences") << "Invalid skin registry format" << LL_ENDL;
        return;
    }

    // Iterate through <skin> entries
    for (LLXMLNode* skin_node = root->getFirstChild(); skin_node != NULL; skin_node = skin_node->getNextSibling())
    {
        if (skin_node->hasName("skin"))
        {
            std::string folder_name;
            std::string display_name;

            skin_node->getAttributeString("folder", folder_name);
            skin_node->getAttributeString("name", display_name);

            if (!folder_name.empty() && !display_name.empty())
            {
                // Verify the folder actually exists
                std::string skin_path = gDirUtilp->add(gDirUtilp->getExpandedFilename(LL_PATH_SKINS, ""), folder_name);
                if (LLFile::isdir(skin_path))
                {
                    // Add to combo box: display name shown to user, folder name as value
                    skin_combo->add(display_name, folder_name);
                }
            }
        }
    }
    ...
    skin_combo->sortByName();
}

// S24 - Modified to use combo box and populate skin list dynamically
void LLFloaterPreference::refreshSkin(void* data)
{
    LLPanel* self = (LLPanel*)data;
    if (!self) return;

    // S24 - Get the combo box and populate it directly
    LLComboBox* skin_combo = self->getChild<LLComboBox>("skin_selection", true);
    if (skin_combo)
    {
        populateSkinList(skin_combo);
    }

    sSkin = gSavedSettings.getString("SkinCurrent");
    if (skin_combo)
    {
        skin_combo->setValue(sSkin);
    }
}
...
void LLFloaterPreference::onCLickKVTweaks()
{
    LLFloaterReg::showInstance("prefs_kvtweaks");  
}
...
bool LLPanelPreference::postBuild()
{
    ...
    if (hasChild("skin_selection", true))
    {
        ...
        // S24 - Modified to use combo box instead of radio group
        // if skin is set to a skin that no longer exists set back to default
        LLComboBox* skin_combo = getChild<LLComboBox>("skin_selection");
        if (skin_combo && skin_combo->getValue().asString().empty())
        {
