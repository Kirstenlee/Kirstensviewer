...
void LLVOAvatarSelf::updateRegion(LLViewerRegion *regionp)
{
    ...
    if (!regionp || (regionp->getHandle() != mLastRegionHandle))
    {
        if (mLastRegionHandle != 0)
        {
            ...
            // Diagnostics - If so disable unless explicit diag req KL
...
{
    // S24: BEAM SUBSYSTEM - UNCONDITIONAL TRACE - FIRES EVERY FRAME
    // This is only done for yourself (maybe it should be in the agent?)
    if (!needsRenderBeam() || !isBuilt())
    {
        if (mBeam)
        {
            // Normal cleanup - beam destroyed when not needed or avatar not ready (login/region crossing).
            LL_DEBUGS("Beam") << "S24 BEAM: Destroying beam (not needed or avatar not ready)" << LL_ENDL;
        }
        mBeam = NULL;
    }
    else if (!mBeam || mBeam->isDead())
    {
        ...
        // S24: BEAM SUBSYSTEM - Create beam with unified EffectColor
        mBeam = (LLHUDEffectSpiral *)LLHUDManager::getInstance()->createViewerEffect(LLHUDObject::LL_HUD_EFFECT_BEAM);

        if (!mBeam)
        {
            LL_WARNS() << "S24 BEAM: Failed to create beam effect!" << LL_ENDL;
            return;
        }

        // Get unified effect color (set in Preferences or KVtweaks experimental tab)
        LLColor4 beam_color = gAgent.getEffectColor();
        beam_color.mV[VALPHA] = 1.0f; // Ensure full opacity for visibility

        // Convert to unsigned byte color and apply to beam
        mBeam->setColor(LLColor4U(beam_color));
...
{
    // S24: Check dedicated EnableSelectionBeam setting (independent of selection hints popups)
    static LLCachedControl<bool> enable_selection_beam(gSavedSettings, "EnableSelectionBeam", true);
    if (!enable_selection_beam)
    {
        return false;
    }
