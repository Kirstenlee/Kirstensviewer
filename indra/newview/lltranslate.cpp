...
{
    headers->append(std::string{HTTP_OUT_HEADER_ACCEPT}, std::string{HTTP_CONTENT_JSON});
    headers->append(std::string{HTTP_OUT_HEADER_USER_AGENT}, user_agent);
}
...
void LLAzureTranslationHandler::initHttpHeader(
    ...
    const LLSD &key) const
{
    headers->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, std::string{HTTP_CONTENT_JSON});
    headers->append(std::string{HTTP_OUT_HEADER_USER_AGENT}, user_agent);
...
void LLDeepLTranslationHandler::initHttpHeader(
    ...
    const LLSD& key) const
{
    headers->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, std::string{"application/x-www-form-urlencoded"});
    headers->append(std::string{HTTP_OUT_HEADER_USER_AGENT}, user_agent);

    if (key.has("id"))
    {
        std::string authkey = "DeepL-Auth-Key " + key["id"].asString();
        headers->append(std::string{HTTP_OUT_HEADER_AUTHORIZATION}, authkey);
    }
}
