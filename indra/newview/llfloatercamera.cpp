...
void LLPanelCameraZoom::onZoomPlusHeldDown()
{
    ...
    // Camera zoom rate is progressive for smooth control
    F32 rate = getOrbitRate(time);
    gAgentCamera.unlockView();
    gAgentCamera.setOrbitInKey(rate);
    // Note: slider will update automatically in draw() from camera zoom fraction
}
...
void LLPanelCameraZoom::onZoomMinusHeldDown()
{
    ...
    F32 rate = getOrbitRate(time);
    gAgentCamera.unlockView();
    gAgentCamera.setOrbitOutKey(rate);
    // Note: slider will update automatically in draw() from camera zoom fraction
}
...
// static
bool LLFloaterCamera::isCameraRecorderPlaying()
{
    LLFloaterCamera* floater_camera = LLFloaterCamera::findInstance();
    if (floater_camera && floater_camera->mRecorderState == RECORDER_PLAYING)
    {
        return true;
    }
    return false;
...
// static - Update camera recorder independent of UI visibility
void LLFloaterCamera::updateCameraRecorder(F32 dt)
{
    LLFloaterCamera* floater_camera = LLFloaterCamera::findInstance();
    if (floater_camera)
    {
        floater_camera->updateRecorder(dt);
    }
}
...
{
    // Note: updateRecorder is now called from main loop via updateCameraRecorder()
    // to ensure playback continues even when UI is hidden
    LLFloater::draw();
}
...
:   LLFloater(val),
    ...
    mRecordStartTime(0.0f),
    mPlaybackStartTime(0.0f),
    mPlaybackCurrentTime(0.0f),
    mPlaybackIndex(0)
{
    ...
    mCommitCallbackRegistrar.add("CameraRecorder.Record", boost::bind(&LLFloaterCamera::onCameraRecord, this));
    mCommitCallbackRegistrar.add("CameraRecorder.Stop", boost::bind(&LLFloaterCamera::onCameraStop, this));
    mCommitCallbackRegistrar.add("CameraRecorder.Clear", boost::bind(&LLFloaterCamera::onCameraClear, this));
    mCommitCallbackRegistrar.add("CameraRecorder.Play", boost::bind(&LLFloaterCamera::onCameraPlay, this));
    mCommitCallbackRegistrar.add("CameraRecorder.Pause", boost::bind(&LLFloaterCamera::onCameraPause, this));
}
...
bool LLFloaterCamera::postBuild()
{
    ...
    mRecorderTimeText = getChild<LLTextBox>("recorder_time_text");

    // Ensure playback speed defaults to 1.0x (normal speed)
    if (gSavedSettings.getF32("CameraRecorderPlaybackSpeed") != 1.0f)
    {
        gSavedSettings.setF32("CameraRecorderPlaybackSpeed", 1.0f);
    }
    LLComboBox* speed_combo = getChild<LLComboBox>("playback_speed_combo");
    if (speed_combo)
    {
        speed_combo->selectByValue(LLSD(1.0f));
    }
...
void LLFloaterCamera::onCameraRecord()
{
    if (mRecorderState == RECORDER_RECORDING)
    {
        return;
    }
    ...
    mRecordStartTime = LLTimer::getElapsedSeconds();
...
void LLFloaterCamera::onCameraStop()
{
    if (mRecorderState == RECORDER_STOPPED)
    {
        return;
    }
...
void LLFloaterCamera::onCameraClear()
{
    mCameraKeyframes.clear();
    mRecorderState = RECORDER_STOPPED;
    mRecordStartTime = 0.0f;
...
void LLFloaterCamera::onCameraPlay()
{
    if (mCameraKeyframes.empty())
    {
        return;
    }

    if (mRecorderState == RECORDER_PLAYING)
    {
        return;
    }

    mRecorderState = RECORDER_PLAYING;
    mPlaybackStartTime = LLTimer::getElapsedSeconds();
    mPlaybackCurrentTime = 0.0f;
    mPlaybackIndex = 0;

    // Unlock camera from avatar tracking
    gAgentCamera.unlockView();
    gAgentCamera.setFocusOnAvatar(false, false);
    // Don't lock movement - avatar can walk independently (keyboard or double-click)

    updateRecorderUI();
}

void LLFloaterCamera::onCameraPause()
{
    if (mRecorderState == RECORDER_PLAYING)
    {
        mRecorderState = RECORDER_PAUSED;
        updateRecorderUI();
    }
}

void LLFloaterCamera::updateRecorder(F32 dt)
{
    F32 current_time = LLTimer::getElapsedSeconds();
    F32 interval = gSavedSettings.getF32("CameraRecorderInterval");
    F32 max_duration = gSavedSettings.getF32("CameraRecorderMaxDuration");

    if (mRecorderState == RECORDER_RECORDING)
    {
        F32 elapsed = current_time - mRecordStartTime;
        ...
        // Fixed 60fps capture for consistent smooth playback
        if (mCameraKeyframes.empty() || (elapsed - mCameraKeyframes.back().timestamp) >= interval)
        {
            CameraKeyframe keyframe;
            keyframe.timestamp = elapsed;
            keyframe.position = LLViewerCamera::getInstance()->getOrigin();
            keyframe.rotation = LLViewerCamera::getInstance()->getQuaternion();
            keyframe.fov = LLViewerCamera::getInstance()->getView();

            mCameraKeyframes.push_back(keyframe);
        }
    }
    else if (mRecorderState == RECORDER_PLAYING)
    {
        if (mCameraKeyframes.empty())
        {
            onCameraStop();
        ...
        F32 playback_speed = gSavedSettings.getF32("CameraRecorderPlaybackSpeed");
        F32 elapsed = (current_time - mPlaybackStartTime) * playback_speed;
        mPlaybackCurrentTime = elapsed;

        F32 total_duration = mCameraKeyframes.back().timestamp;

        if (elapsed >= total_duration)
        {
            bool loop = gSavedSettings.getBOOL("CameraRecorderLoop");
            if (loop)
            {
                mPlaybackStartTime = current_time;
                mPlaybackCurrentTime = 0.0f;
                mPlaybackIndex = 0;
                elapsed = 0.0f;
            }
        ...
        // Optimized search - start from last known index instead of linear search
        while (mPlaybackIndex < mCameraKeyframes.size() - 1 && 
               mCameraKeyframes[mPlaybackIndex + 1].timestamp < elapsed)
        {
            mPlaybackIndex++;
        }

        if (mPlaybackIndex < mCameraKeyframes.size() - 1)
        {
            const CameraKeyframe& k1 = mCameraKeyframes[mPlaybackIndex];
            const CameraKeyframe& k2 = mCameraKeyframes[mPlaybackIndex + 1];

            F32 t = (elapsed - k1.timestamp) / (k2.timestamp - k1.timestamp);
            t = llclamp(t, 0.0f, 1.0f); // Prevent overshooting

            bool smooth = gSavedSettings.getBOOL("CameraRecorderSmoothInterpolation");
            if (smooth)
            {
                t = smoothstep(t);
            }

            LLVector3 pos = interpolatePosition(k1, k2, t);
            LLQuaternion rot = interpolateRotation(k1, k2, t);
            F32 fov = lerp(k1.fov, k2.fov, t);

            // Directly set viewer camera, bypassing agent camera system
            LLViewerCamera* camera = LLViewerCamera::getInstance();
            camera->setOrigin(pos);
            camera->setView(fov);

            LLMatrix3 rotation_matrix(rot);
            LLVector3 at = LLVector3::x_axis * rotation_matrix;
            LLVector3 left = LLVector3::y_axis * rotation_matrix;
            LLVector3 up = LLVector3::z_axis * rotation_matrix;
            camera->setAxes(at, left, up);
        }
    }
...
void LLFloaterCamera::updateRecorderUI()
{
    if (!mRecorderStatusText || !mRecorderTimeText)
    {
        return;
    }
    ...
    case RECORDER_STOPPED:
        mRecorderStatusText->setText(std::string("Ready"));
        mRecorderStatusText->setColor(LLColor4::grey);
        if (mCameraKeyframes.empty())
        {
            mRecorderTimeText->setText(std::string("0:00 / 0:00"));
        }
        else
        {
            F32 total = mCameraKeyframes.back().timestamp;
            mRecorderTimeText->setText(std::string("0:00 / ") + formatTime(total));
        }
        break;

    case RECORDER_RECORDING:
        {
            mRecorderStatusText->setText(std::string("Recording"));
            mRecorderStatusText->setColor(LLColor4::red);
            F32 elapsed = LLTimer::getElapsedSeconds() - mRecordStartTime;
            F32 max_duration = gSavedSettings.getF32("CameraRecorderMaxDuration");
            mRecorderTimeText->setText(formatTime(elapsed) + std::string(" / ") + formatTime(max_duration));
        }
        break;

    case RECORDER_PLAYING:
        {
            mRecorderStatusText->setText(std::string("Playing"));
            mRecorderStatusText->setColor(LLColor4::green);
            F32 total = mCameraKeyframes.empty() ? 0.0f : mCameraKeyframes.back().timestamp;
            mRecorderTimeText->setText(formatTime(mPlaybackCurrentTime) + std::string(" / ") + formatTime(total));
        }
        break;

    case RECORDER_PAUSED:
        {
            mRecorderStatusText->setText(std::string("Paused"));
            mRecorderStatusText->setColor(LLColor4::yellow);
            F32 total = mCameraKeyframes.empty() ? 0.0f : mCameraKeyframes.back().timestamp;
            mRecorderTimeText->setText(formatTime(mPlaybackCurrentTime) + std::string(" / ") + formatTime(total));
        }
        break;
...
std::string LLFloaterCamera::formatTime(F32 seconds)
{
    S32 minutes = (S32)(seconds / 60.0f);
    S32 secs = (S32)seconds % 60;
    return llformat("%d:%02d", minutes, secs);
}

LLVector3 LLFloaterCamera::interpolatePosition(const CameraKeyframe& k1, const CameraKeyframe& k2, F32 t)
{
    return lerp(k1.position, k2.position, t);
}

LLQuaternion LLFloaterCamera::interpolateRotation(const CameraKeyframe& k1, const CameraKeyframe& k2, F32 t)
{
    return slerp(t, k1.rotation, k2.rotation);
}

F32 LLFloaterCamera::smoothstep(F32 t)
{
