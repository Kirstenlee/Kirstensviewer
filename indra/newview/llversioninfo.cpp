...
{
    ViewerMaturity maturity = TEST_VIEWER;  // default fallback

    std::string channel = getChannel();
    std::transform(channel.begin(), channel.end(), channel.begin(), ::tolower);

    if (channel.find("kirstens") != std::string::npos)
    {
        maturity = RELEASE_VIEWER;
    }
    else if (channel.find("release") != std::string::npos)
    {
        maturity = RELEASE_VIEWER;
    }
    else if (channel.find("beta") != std::string::npos ||
        channel.find("develop") != std::string::npos)
    {
        maturity = BETA_VIEWER;
    }
    else if (channel.find("project") != std::string::npos)
    {
        maturity = PROJECT_VIEWER;
    }
    else if (channel.find("test") != std::string::npos)
    {
        maturity = TEST_VIEWER;
    }
    else
    {
        LL_WARNS() << "Channel '" << getChannel()
