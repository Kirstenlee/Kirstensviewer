...
{
	// S24 : Rationale: Reduce excessive console logging (DEBUG/INFO) that floods the client console.
	// S24 : Technical: Removed per-agent debug/info messages and simplified control flow.
	// S24 : Benefit: Improves performance and signal-to-noise in logs; only WARN on HTTP failures
	// S24 :          or malformed agent entries. Keeps necessary behavior (update reported complexity
	// S24 :          and notification region limits) intact.
	...
	// Perform the request and handle results with minimal logging to avoid console spam.
	...
	if (!status)
	{
		LL_WARNS("AvatarRenderInfo") << "HTTP request failed: " << status.toTerseString() << LL_ENDL;
		return;
	}

	if (result.has(KEY_AGENTS) && result[KEY_AGENTS].isMap())
	{
			...
			++agent_iter)
		{
			...
			if (!vobjp) continue;
			LLVOAvatar* avatarp = vobjp->asAvatar();
			if (!avatarp || avatarp->isControlAvatar()) continue;

			const LLSD& agent_info_map = agent_iter->second;
			if (!agent_info_map.isMap())
			{
				LL_WARNS("AvatarRenderInfo") << "Invalid agent entry for " << target_agent_id << LL_ENDL;
				continue;
			}
	...
	if (result.has(KEY_REPORTING_COMPLEXITY_LIMIT) && result.has(KEY_OVER_COMPLEXITY_LIMIT))
	{
	...
	{
		// Only warn when expected limit fields are missing to indicate a server-side issue
		LL_WARNS("AvatarRenderInfo") << "Missing complexity limit fields in avatar render info response" << LL_ENDL;
	}
}
...
{
	// S24 : Rationale: Reduce noisy logging and simplify control flow to avoid console spam.
	// S24 : Technical: Remove per-avatar DEBUG logs and the initial DEBUG send message.
	// S24 : Technical: Keep all existing early-outs and preserve behavior for timers, POST body,
	// S24 :            and error handling. This reduces noise while maintaining functionality.
	...
	{
		auto* avatar = static_cast<LLVOAvatar*>(character);
		if (!avatar) continue;
		if (avatar->isDead() || avatar->isControlAvatar()) continue;
		if (avatar->getRezzedStatus() < 2) continue;
		if (avatar->getObjectHost() != regionp->getHost()) continue;

		U32 avatar_complexity = avatar->getVisualComplexity();
		if (avatar_complexity == 0) continue;
		...
		S32 weight = (S32)(avatar_complexity < S32_MAX ? avatar_complexity : S32_MAX);
		LLSD info = LLSD::emptyMap();
		info[KEY_WEIGHT] = weight;
	...
	// Clear local references before suspending the coroutine
	regionp = NULL;
	world_inst = NULL;
	LLSD result = httpAdapter->postAndSuspend(httpRequest, url, report, httpOpts);

	// Re-resolve world/region after resume
	...
	if (result.isMap())
	{
		...
		// Intentionally avoid noisy DEBUG logging of successful POST result
	}
	else
	{
