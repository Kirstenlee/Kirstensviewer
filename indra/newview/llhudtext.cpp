    ...
    const LLFontGL* font)
{
    ...
    if (!wline.empty())
    {
        ...
        std::vector<LLWString> lines;
        LLWString::size_type start = 0;
        LLWString::size_type pos = 0;

        // Use wchar_t* to construct LLWString delimiter properly
        const LLWString delimiters = utf8str_to_wstring("\r\n");

        while ((pos = wline.find_first_of(delimiters, start)) != LLWString::npos)
        {
            if (pos > start)
            {
                lines.push_back(wline.substr(start, pos - start));
            }
            start = pos + 1;
        }
        ...
        if (start < wline.size())
        {
            lines.push_back(wline.substr(start));
        }
