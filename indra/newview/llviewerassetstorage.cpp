    ...
    void *user_data)
{
    ...
    if (!gAgent.getRegion())
    {
        if (STATE_WORLD_INIT <= LLStartUp::getStartupState())
        {
            ...
            // S24: PERFORMANCE - Changed to LL_DEBUGS - waiting for region is normal during startup/teleports
            // Not an error or warning - just coroutine suspension while region loads
            // Fires frequently during teleports when asset requests queue up
            LL_DEBUGS("ViewerAsset") << "Waiting for agent region to be set" << LL_ENDL;
    ...
    {
        // S24: PERFORMANCE - Changed to LL_DEBUGS - waiting for caps is normal during region init
        // Not an error or warning - just coroutine suspension while caps arrive
        // Fires on every asset request during region initialization (common during teleport/login)
        ...
        // S24: PERFORMANCE - Changed to LL_DEBUGS to eliminate hot-path logging spam
        // These status messages fire on EVERY asset request after caps are received
        // Asset requests are extremely frequent (textures, sounds, animations, etc.)
        // Not an error condition - just status info that wastes CPU cycles in production
        // Each log wastes: string formatting + region pointer formatting + URL formatting + I/O
