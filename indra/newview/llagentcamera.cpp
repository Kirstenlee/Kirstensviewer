...
#include "aoengine.h"           // ## S24 Animation Overrider
#include "llagent.h"
...
LLAgentCamera::LLAgentCamera() :
    ...
    // S24: Initialize smooth movement values
    mSmoothedAtKey(0.f),
    mSmoothedLeftKey(0.f),
    mSmoothedUpKey(0.f),
    mSmoothedYawKey(0.f),
...
void LLAgentCamera::updateCamera()
{
    ...
    // Move the camera
    // Skip agent-driven camera positioning during recorder playback
    if (LLFloaterCamera::isCameraRecorderPlaying())
    {
        return; // Camera is being controlled by recorder, skip agent updates
    }
...
// S24: Smooth movement system - ease in/ease out for keyboard controls
void LLAgentCamera::updateSmoothMovement(F32 dt)
{
    ...
    static LLCachedControl<F32> ground_accel_time(gSavedSettings, "SmoothMovementAccelTime");
    static LLCachedControl<F32> ground_decel_time(gSavedSettings, "SmoothMovementDecelTime");
    static LLCachedControl<F32> flight_accel_time(gSavedSettings, "SmoothFlightAccelTime");
    static LLCachedControl<F32> flight_decel_time(gSavedSettings, "SmoothFlightDecelTime");

    // Check if we're flying to determine which settings to use
    bool is_flying = gAgent.getFlying();
    bool use_smoothing = is_flying ? smooth_flight_enabled : smooth_movement_enabled;

    if (!use_smoothing || dt <= 0.f)
    {
        // Smooth movement disabled - use instant response
        mSmoothedAtKey = (F32)mAtKey;
        mSmoothedLeftKey = (F32)mLeftKey;
        mSmoothedUpKey = (F32)mUpKey;
        mSmoothedYawKey = mYawKey;
        return;
    }

    // Select appropriate acceleration/deceleration times based on movement mode
    F32 accel_t, decel_t;
    if (is_flying)
    {
        accel_t = llmax((F32)flight_accel_time, 0.001f);
        decel_t = llmax((F32)flight_decel_time, 0.001f);
    }
    else
    {
        accel_t = llmax((F32)ground_accel_time, 0.001f);
        decel_t = llmax((F32)ground_decel_time, 0.001f);
    }

    // Lambda for smooth interpolation with different accel/decel rates
    auto smoothLerp = [](F32 current, F32 target, F32 dt, F32 accel_time, F32 decel_time) -> F32
    {
        ...
        // Choose acceleration or deceleration time based on direction
        bool accelerating = (fabs(target) > fabs(current));
        F32 blend_time = accelerating ? accel_time : decel_time;
        ...
        // Calculate lerp factor based on time (exponential smoothing)
        F32 lerp_factor = 1.f - expf(-dt / blend_time);
        lerp_factor = llclamp(lerp_factor, 0.f, 1.f);

        return current + (target - current) * lerp_factor;
    };

    // Smooth each movement axis
    mSmoothedAtKey = smoothLerp(mSmoothedAtKey, (F32)mAtKey, dt, accel_t, decel_t);
    mSmoothedLeftKey = smoothLerp(mSmoothedLeftKey, (F32)mLeftKey, dt, accel_t, decel_t);
    mSmoothedUpKey = smoothLerp(mSmoothedUpKey, (F32)mUpKey, dt, accel_t, decel_t);
    mSmoothedYawKey = smoothLerp(mSmoothedYawKey, mYawKey, dt, accel_t, decel_t);
}
