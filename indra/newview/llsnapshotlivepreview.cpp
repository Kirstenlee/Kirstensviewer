...
void LLSnapshotLivePreview::draw()
{
		...
		getSnapshotUpToDate())
	{
		...
		{
			const LLRect& irect = getImageRect();
			drawGuides(irect.mLeft, irect.mBottom + TOP_PANEL_HEIGHT,
...
// S24 Snapshot revamp - Phase 2: composition guides overlay.
// Draws non-destructive framing aids inside the given absolute screen rect.
void LLSnapshotLivePreview::drawGuides(S32 left, S32 bottom, S32 width, S32 height)
{
	static LLCachedControl<bool> g_thirds(gSavedSettings, "SnapshotGuideThirds", false);
	static LLCachedControl<bool> g_golden(gSavedSettings, "SnapshotGuideGolden", false);
	static LLCachedControl<bool> g_diag(gSavedSettings, "SnapshotGuideDiagonals", false);
	static LLCachedControl<bool> g_center(gSavedSettings, "SnapshotGuideCenter", false);
	static LLCachedControl<bool> g_safe169(gSavedSettings, "SnapshotGuideSafe169", false);
	static LLCachedControl<bool> g_safe43(gSavedSettings, "SnapshotGuideSafe43", false);
	static LLCachedControl<bool> g_safe11(gSavedSettings, "SnapshotGuideSafe11", false);
	static LLCachedControl<F32>  guide_opacity(gSavedSettings, "SnapshotGuideOpacity", 0.6f);
	static LLCachedControl<LLColor4> guide_color(gSavedSettings, "SnapshotGuideColor", LLColor4(1.f, 1.f, 1.f, 1.f));

	if (!(g_thirds || g_golden || g_diag || g_center || g_safe169 || g_safe43 || g_safe11))
	{
		return;
	}
	if (width <= 0 || height <= 0)
	...
	LLColor4 col = guide_color();
	col.mV[VW] = llclamp((F32)guide_opacity(), 0.f, 1.f);
	...
	auto draw_safe = [&](F32 aspect)
		{
			F32 cur_aspect = (F32)w / (F32)h;
			S32 sw = w;
			S32 sh = h;
