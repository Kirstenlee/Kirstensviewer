...
//-----------------------------------------------------------------------
// S24 - Metadata Support
//-----------------------------------------------------------------------
bool LLStreamingAudio_MediaPlugins::getMetadata(std::string& title, std::string& artist)
{
	if (!mMediaPlugin)
	{
		title = mLastTitle;
	...
	// Get metadata from plugin via public accessors
	title = mMediaPlugin->getMetadataTitle();
	artist = mMediaPlugin->getMetadataArtist();
...
//-----------------------------------------------------------------------
// S24 - 10-Band Equalizer Support
//-----------------------------------------------------------------------
bool LLStreamingAudio_MediaPlugins::initEqualizer()
{
	if (!mMediaPlugin || mEQInitialized)
		return false;

	mMediaPlugin->initEqualizer();
	mEQInitialized = true;
	LL_INFOS("MediaPlugins") << "10-band equalizer initialized" << LL_ENDL;
	return true;
}

void LLStreamingAudio_MediaPlugins::cleanupEqualizer()
{
	if (mMediaPlugin && mEQInitialized)
	{
		mMediaPlugin->cleanupEqualizer();
		mEQInitialized = false;
		LL_INFOS("MediaPlugins") << "10-band equalizer cleaned up" << LL_ENDL;
	}
}

void LLStreamingAudio_MediaPlugins::updateEqualizer(bool enabled, F32 preamp, const F32 bands[10])
{
	if (!mMediaPlugin)
		return;

	// Initialize EQ on first use
	if (!mEQInitialized && enabled)
	{
		initEqualizer();
	}

	// Send EQ settings to VLC plugin
	mMediaPlugin->updateEqualizer(enabled, preamp, bands);
}
