...
void LLControlAvatar::updateAnimations()
{
    if (!mRootVolp)
    {
        // During shutdown/cleanup, mRootVolp is intentionally set to NULL in markForDeath() and markDead().
        // This is expected behavior - no point warning about something the user cannot control.
        // If you're debugging actual animation issues, enable AnimatedObjectsNotify debug logging.
        if (!mMarkedForDeath && !isDead())
        {
