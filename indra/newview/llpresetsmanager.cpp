...
void LLPresetsManager::createMissingDefault(const std::string& subdirectory)
{
    ...
    if (!gDirUtilp->fileExists(default_file))
    {
        ...
        // S24 - For water presets, copy ALL shipped presets on first run
        if (PRESETS_WATER == subdirectory)
        {
            std::string shipped_presets_dir = gDirUtilp->getExpandedFilename(LL_PATH_APP_SETTINGS, PRESETS_DIR, subdirectory);
            std::string user_presets_dir = gDirUtilp->getExpandedFilename(LL_PATH_PER_SL_ACCOUNT, PRESETS_DIR, subdirectory);

            LL_INFOS() << "Copying shipped water presets from " << shipped_presets_dir << " to " << user_presets_dir << LL_ENDL;

            // Ensure user preset directory exists
            LLFile::mkdir(user_presets_dir);

            // Copy all .xml files from shipped presets
            bool copied_any = false;
            LLDirIterator shipped_iter(shipped_presets_dir, "*.xml");
            bool found = true;
            while (found)
            {
                std::string filename;
                found = shipped_iter.next(filename);
                if (found)
                {
                    std::string source = gDirUtilp->add(shipped_presets_dir, filename);
                    std::string dest = gDirUtilp->add(user_presets_dir, filename);

                    if (LLFile::copy(source, dest))
                    {
                        LL_INFOS() << "Copied water preset: " << filename << LL_ENDL;
                        copied_any = true;
                    }
                    else
                    {
                        LL_WARNS() << "Failed to copy water preset: " << filename << LL_ENDL;
                    }
                }
            }
            ...
            if (copied_any)
            {
                return; // Successfully copied shipped presets
            }
            else
            {
                LL_WARNS() << "No shipped water presets found or failed to copy, falling back to current settings" << LL_ENDL;
            }
        }
...
void LLPresetsManager::loadPresetNamesFromDir(const std::string& subdirectory, preset_name_list_t& presets, EDefaultOptions default_option)
{
    ...
    bool IS_WATER = (PRESETS_WATER == subdirectory);
...
void LLPresetsManager::getWaterControlNames(std::vector<std::string>& names)
{
    const std::vector<std::string> water_controls = {
        // S24 - KV Tweaks Water tab controls
        "RenderWaterMetallic",
...
bool LLPresetsManager::savePreset(const std::string& subdirectory, std::string name, bool createDefault)
{
    ...
    {
        name_list.clear();
        getWaterControlNames(name_list);
        name_list.push_back("PresetWaterActive");
    }
    else
    {
 ...
 // If the active preset name is the only thing in the list, don't save the list
    if (paramsData.size() > 1)
    {
        ...
        if (presetsXML.is_open())
        {
            ...
            if (IS_WATER)
            {
                gSavedSettings.setString("PresetWaterActive", name);
                // signal interested parties
                triggerChangeSignal();
            }
        }
...
bool LLPresetsManager::deletePreset(const std::string& subdirectory, std::string name)
{
    ...
    // S24 - Handle water preset deletion
    if(PRESETS_WATER == subdirectory)
    {
        if (gSavedSettings.getString("PresetWaterActive") == name)
        {
            // Reset to Default preset when deleting the active water preset
            gSavedSettings.setString("PresetWaterActive", PRESETS_DEFAULT);
            loadPreset(PRESETS_WATER, PRESETS_DEFAULT);
        }
        // signal interested parties
        triggerChangeSignal();
    }
