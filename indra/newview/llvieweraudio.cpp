...
#include "llviewermedia_streamingaudio.h" // S24 - MediaPlugins streaming audio
...
void audio_update_stream_metadata()
{
	static LLCachedControl<bool> show_stream_metadata(gSavedSettings, "ShowStreamMetadata");
	static std::string last_title;
	static std::string last_artist;

	if (!show_stream_metadata || !gAudiop)
		return;
	...
	if (stream->getMetadata(title, artist))
	{
		...
		if (title != last_title || artist != last_artist)
		{
			...
			// Format notification with labels
			std::string notification_text = "♪ Now Playing\n";
			notification_text += "Artist: " + (artist.empty() ? "Unknown" : artist) + "\n";
			notification_text += "Title: " + (title.empty() ? "Unknown" : title);

			LLSD args;
			args["MESSAGE"] = notification_text;
			LLNotificationsUtil::add("SystemMessageTip", args);
		}
	}
}

void audio_update_equalizer()
{
#ifdef LL_FMODSTUDIO
	if (!gAudiop)
		return;

	// FMOD 3-band equalizer
	LLAudioEngine_FMODSTUDIO* fmod_engine = dynamic_cast<LLAudioEngine_FMODSTUDIO*>(gAudiop);
	if (fmod_engine)
	{
		static LLCachedControl<bool> eq_enabled(gSavedSettings, "AudioEqualizerEnabled", false);
		static LLCachedControl<F32> low_gain(gSavedSettings, "AudioEqualizerLowGain", 0.0f);
		static LLCachedControl<F32> mid_gain(gSavedSettings, "AudioEqualizerMidGain", 0.0f);
		static LLCachedControl<F32> high_gain(gSavedSettings, "AudioEqualizerHighGain", 0.0f);

		fmod_engine->updateEqualizer(eq_enabled, low_gain, mid_gain, high_gain);
	}
#endif

	// S24 - MediaPlugins/VLC 10-band equalizer
	if (!gAudiop)
		return;

	LLStreamingAudio_MediaPlugins* vlc_stream = 
		dynamic_cast<LLStreamingAudio_MediaPlugins*>(gAudiop->getStreamingAudioImpl());

	if (vlc_stream)
	{
		static LLCachedControl<bool> vlc_eq_enabled(gSavedSettings, "VLCAudioEQEnabled", false);
		static LLCachedControl<F32> vlc_preamp(gSavedSettings, "VLCAudioEQPreamp", 0.0f);

		F32 bands[10];
		for (int i = 0; i < 10; ++i)
		{
			std::string setting_name = llformat("VLCAudioEQBand%d", i);
			bands[i] = gSavedSettings.getF32(setting_name);
		}

		vlc_stream->updateEqualizer(vlc_eq_enabled, vlc_preamp, bands);
	}
}
