...
bool LLFloaterWindowSize::postBuild()
{
    ...
    // S24: Check if borderless mode is active
    if (gViewerWindow && gViewerWindow->getWindow() && gViewerWindow->getWindow()->getBorderless())
    {
        // Show warning text
        getChild<LLUICtrl>("borderless_warning")->setVisible(true);
        // Disable controls since they won't work in borderless mode
        getChild<LLUICtrl>("window_size_combo")->setEnabled(false);
        getChild<LLUICtrl>("set_btn")->setEnabled(false);
    }
