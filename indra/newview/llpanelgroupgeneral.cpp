...
bool LLPanelGroupGeneral::apply(std::string& mesg)
{
    ...
    // Handle group chat mute setting
    if (mCtrlReceiveChat)
    {
        bool receive_chat = mCtrlReceiveChat->get();
        LLGroupChatMute::instance().setGroupChatMuted(mGroupID, !receive_chat);
    }
...
void LLPanelGroupGeneral::update(LLGroupChange gc)
{
    ...
    if (mCtrlReceiveChat)
    {
        ...
        if (is_member)
        {
            bool receive_chat = !LLGroupChatMute::instance().isGroupChatMuted(mGroupID);
