...
bool LLViewerObjectList::removeFromLocalIDTable(LLViewerObject* objectp)
{
	...
	// Remove valid entry and reset region index
	...
	bool from_cache)
{
	...
	if (!from_cache)
	{
		msg = gMessageSystem; // Only assign if not from cache
	}
	...
	// Handle selection for newly created objects
    if (just_created
...
LLViewerObject* LLViewerObjectList::processObjectUpdateFromCache(LLVOCacheEntry* entry, LLViewerRegion* regionp)
{
    ...
    if (objectp)
	{
            ...
            objectp->getRegion() != regionp))
		{
			...
			{
				objectp->mLocalID = entry->getLocalID(); // Update local ID
			}

            if (objectp->getRegion() != regionp)
			{
				objectp->updateRegion(regionp); // Update region
			}
		}
        else
        {
...
void LLViewerObjectList::update(LLAgent& agent)
{
	...
	static LLCachedControl<F32> interpolation_phase_out(gSavedSettings, "InterpolationPhaseOut");
	static LLCachedControl<F32> region_crossing_interpolation(gSavedSettings, "RegionCrossingInterpolationTime");
	static LLCachedControl<bool> animate_textures(gSavedSettings, "AnimateTextures");
	...
	F32 region_interp_time = llclamp((F32)region_crossing_interpolation, 0.5f, 5.f);
	if (interp_time < 0.0 ||
		phase_out_time < 0.0 ||
...
void LLViewerObjectList::fetchObjectCostsCoro(std::string url)
{
	...
	if (!status || result.has("error"))
	{
		if (result.has("error"))
		{
			...
			{
				LL_WARNS("ObjectCost") << "Object cost fetch error (future errors suppressed). Message: " << result["error"]["message"].asString()
					<< ", identifier: " << result["error"]["identifier"].asString()
					<< ", requested: " << idList.size() << " objects. Batching enabled." << LL_ENDL;
...
void LLViewerObjectList::cleanDeadObjects(bool use_timer)
{
    ...
    for ( ; iter != mObjects.end(); )
	{
        ...
        if (objectp == NULL)
		{
			break; // Exit once we've reached the dead tail
		}
	...
	// Clean remaining dead list and reset counter
	mDeadObjects.clear();
	mNumDeadObjects = 0;
}
...
void LLViewerObjectList::updateActive(LLViewerObject* objectp)
{
	...
	if (objectp->isDead())
	{
		return; // Dead objects are not updated
	}
	...
	else // Removing object from active list
	{
...
void LLViewerObjectList::orphanize(LLViewerObject* childp, U32 parent_id, U32 ip, U32 port)
{
	...
	// Add parent to orphan parents list if not already present
...
void LLViewerObjectList::findOrphans(LLViewerObject* objectp, U32 ip, U32 port)
{
	...
	// Search object cache for orphans if the object has a region
	if (objectp->getRegion())
	{
