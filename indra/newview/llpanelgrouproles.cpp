                       ...
                       const LLUUID& role_id)
{
    ...
    if (!gdatap)
    {
        // S24: Changed to LL_DEBUGS - group data not loaded yet is normal during initialization
        // Fires when checking permissions before group data arrives (async loading)
        // This is expected behavior during UI setup, not an error
        // Security: Group UUIDs reveal user's group memberships
        LL_DEBUGS("Groups") << "agentCanAddToRole: Group data not available for group " << group_id << LL_ENDL;
        return false;
    }
...
void LLPanelGroupRoles::update(LLGroupChange gc)
{
    ...
    {
        // S24: Changed to LL_DEBUGS - subtab not ready during initialization (normal)
        // Fires when update() called before tabs finish building
        LL_DEBUGS("Groups") << "LLPanelGroupRoles::update() -- No subtab to update!" << LL_ENDL;
    }
...
bool LLPanelGroupMembersSubTab::apply(std::string& mesg)
{
    ...
    if (!gdatap)
    {
        // S24: Changed to LL_DEBUGS - group data not loaded when applying changes (normal race condition)
        // User clicked apply before group data finished loading from server
        // Security: Group UUIDs reveal user's group memberships
        LL_DEBUGS("Groups") << "apply: Group data not available for group " << mGroupID << LL_ENDL;
...
void LLPanelGroupMembersSubTab::applyMemberChanges()
{
    ...
    if (!gdatap)
    {
        // S24: Changed to LL_DEBUGS - group data not loaded when applying member changes (async race)
        // User triggered changes before server data arrived
        // Security: Group UUIDs reveal user's group memberships
        LL_DEBUGS("Groups") << "applyMemberChanges: Group data not available for group " << mGroupID << LL_ENDL;
        return;
    }
...
void LLPanelGroupBanListSubTab::populateBanList()
{
    ...
    if(!gdatap)
    {
        // S24: Changed to LL_DEBUGS - group data not available yet is normal during initialization
        // Fires when: (1) no group selected (mGroupID is null), (2) group data not loaded yet, (3) user not in any groups
        // This function is called from postBuildSubTab() before group data loads - expected behavior
        // Potential gain: Eliminate spam during UI initialization + reduce UUID exposure in client logs
        // Security: Group UUIDs reveal user's group memberships (privacy concern)
        LL_DEBUGS("Groups") << "Group data not available yet for group " << mGroupID << LL_ENDL;
        return;
    }
