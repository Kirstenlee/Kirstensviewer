...
#ifdef DX_RENDER
    // LL::GLTFSceneManager's own render()/renderOpaque() (imported .glb/
    // .gltf SCENE-FILE assets, separate from ordinary PBR-materialed prims/
    // mesh) call glBindBufferBase(), never assigned under DX_RENDER -
    // skipped here, same as dxdrawpoolsimple.cpp's other GLTFSceneManager
    // call site. Ordinary PBR-materialed geometry (pushGLTFBatches/
    // pushGLTFBatch, via LLFetchedGLTFMaterial::bind() + drawRange()) is
    // unaffected.
    //
    // pushRiggedGLTFBatches() is safe to call unconditionally - pure shared
    // code (uploadMatrixPalette() + the same drawRange() chokepoint every
    // converted pool uses). mRenderType+1 follows the established "next enum
    // value is the RIGGED variant" convention (PASS_BUMP_RIGGED etc).
...
void LLDrawPoolGLTFPBR::renderPostDeferred(S32 pass)
{
    ...
    else if (mRenderType == LLPipeline::RENDER_TYPE_PASS_GLTF_PBR) // HACK -- don't render glow except for the non-alpha masked implementation
    {
        ...
        // 3D masking here causes issues with alpha-containing textures; avoid for now.
