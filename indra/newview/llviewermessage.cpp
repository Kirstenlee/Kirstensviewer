...
void send_complete_agent_movement(const LLHost& sim_host)
{
    ...
    // Lambda callback for CompleteAgentMovement acknowledgment/timeout
    // This is a critical message that MUST arrive for successful login/teleport/region-crossing
    void (*complete_agent_movement_callback)(void**, S32) = [](void**, S32 result)
    {
        if (LLApp::isExiting()) return;
        if (result != LL_ERR_NOERR)
        {
            LL_WARNS("Messaging") << "CompleteAgentMovement failed delivery, error code: " << result << LL_ENDL;
        }
    };

    // Use same retry strategy as UseCircuitCode - crucial for connection reliability
    msg->sendReliable(
        sim_host,
