...
bool LLToolBarView::handleDropTool( void* cargo_data, EDragAndDropType cargo_type, S32 x, S32 y, LLToolBar* toolbar)
{
    ...
    if (type == LLAssetType::AT_WIDGET)
    {
        ...
        // S24 early out to avoid wasted time and un nesting of logic
        if (!command)
        {
        ...
        if (old_toolbar_loc != LLToolBarEnums::TOOLBAR_NONE)
        {
            ...
            LLToolBar* old_toolbar = gToolBarView->mDragToolbarButton->getParentByType<LLToolBar>();
        ...
        // Convert the (x, y) position into rank in toolbar
