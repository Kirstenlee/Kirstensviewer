...
void LLPanelEnvironmentInfo::onEnvironmentReceived(S32 parcel_id, LLEnvironment::EnvironmentInfo::ptr_t envifo)
{
    ...
    {
        // S24: This is expected behavior - async environment responses can arrive for 
        // old/invalid parcels when parcel changes quickly or during initialization
        // The defensive check correctly discards mismatched data - downgraded to DEBUG
        LL_DEBUGS("ENVPANEL") << "Have environment for parcel " << parcel_id << " expecting " << getParcelId() << ". Discarding." << LL_ENDL;
            return;
    }
