...
namespace LLMarketplaceImport
{
    ...
    void marketplacePostCoro(std::string url)
    {
        ...
        httpHeaders->append(std::string{HTTP_OUT_HEADER_CONNECTION}, "Keep-Alive");
        httpHeaders->append(std::string{HTTP_OUT_HEADER_COOKIE}, sMarketplaceCookie);
        httpHeaders->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, std::string{HTTP_CONTENT_XML});
        httpHeaders->append(std::string{HTTP_OUT_HEADER_USER_AGENT}, LLViewerMedia::getInstance()->getCurrentUserAgent());
    ...
    void marketplaceGetCoro(std::string url, bool buildHeaders)
    {
        ...
        if (buildHeaders)
        {
            ...
            httpHeaders->append(std::string{HTTP_OUT_HEADER_ACCEPT}, "*/*");
            httpHeaders->append(std::string{HTTP_OUT_HEADER_COOKIE}, sMarketplaceCookie);
            httpHeaders->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, std::string{HTTP_CONTENT_LLSD_XML});
            httpHeaders->append(std::string{HTTP_OUT_HEADER_USER_AGENT}, LLViewerMedia::getInstance()->getCurrentUserAgent());
        }
        else
        {
