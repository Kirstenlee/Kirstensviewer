...
void LLIMProcessing::requestOfflineMessagesCoro(std::string url)
{
    ...
    if (!contents.size())
    {
        // S24: No contents is not an error - it just means no offline messages
        // Absence is not newsworthy; don't spam logs when there's nothing to process
...
{
    // S24: Don't log requests - only log when messages are actually received
    // User's right to log chat; system shouldn't spam about checking for messages
    LL_DEBUGS("Messaging") << "Requesting offline messages (Legacy)." << LL_ENDL;
