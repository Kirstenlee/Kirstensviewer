...
static bool handleSetShaderChanged(const LLSD& newvalue)
{
    ...
    // Defer shader reload to next frame to avoid blocking main thread during settings change
...
{
       bool perfTestEnabled = newvalue.asBoolean();
       if (perfTestEnabled)
       {
               // Performance test mode: disable UI/HUD/sky/water, KEEP world geometry
       ...
       {
               // Normal operation: enable all render types
...
bool toggle_freeze_animation(const LLSD& newvalue)
{
    if (isAgentAvatarValid())
    {
...
void handleGetBeamColor(LLUICtrl* ctrl, const LLSD& param)
{
    ...
    if (color_swatch)
    {
        // Read color from settings (0-1 range)
        LLColor4 beam_color = gSavedSettings.getColor4("SelectionBeamColor");
        color_swatch->set(beam_color, TRUE);
    }
}
...
void handleSetBeamColor(LLUICtrl* ctrl, const LLSD& param)
{
    ...
    if (color_swatch)
    {
        // Get color from swatch and save to settings
        LLColor4 beam_color = color_swatch->get();
        gSavedSettings.setColor4("SelectionBeamColor", beam_color);
    }
}
...
void settings_setup_listeners()
{
    ...
    setting_setup_signal_listener(gSavedSettings, "FreezeAnimation", toggle_freeze_animation);
