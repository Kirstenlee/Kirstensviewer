...
#include "message.h"
#include "llsys.h"          // S24: For LLMemoryInfo memory pressure monitoring
#include "llframetimer.h"   // S24: For memory check throttling
...
#include "llsdutil.h"
#include "llstartup.h"
#include "kvramcache.h"  // S24 STAGE 3: RAM Cache integration
...
static const S32 HTTP_NONPIPE_REQUESTS_LOW_WATER = 40;      // Aggressive (was 25 conservative, orig 20) - Non-pipelined refill
...
bool LLTextureFetchWorker::doWork(S32 param)
{
	...
	if (mState == LOAD_FROM_TEXTURE_CACHE)
	{
		if (mCacheReadHandle == LLTextureCache::nullHandle())
		{
			...
			// S24 STAGE 3 INJECTION POINT 1: RAM Cache Read (Hot Path - Before Disk I/O)
			// Cached booleans for performance - eliminates repeated gSavedSettings lookups
			static LLCachedControl<bool> sKVRAMCacheEnabled(gSavedSettings, "KVRAMCacheEnabled", false);
			static LLCachedControl<bool> sKVRAMDebugLog(gSavedSettings, "KVRAMCacheDebugLogging", false);

			// CRITICAL: Single boolean check + instance existence before expensive operations
			if (sKVRAMCacheEnabled && KVRAMCache::instanceExists())
			{
				// Fast hash lookup - O(1) operation
				if (KVRAMCache::getInstance()->hasTexture(mID))
				{
					...
					// Retrieve J2C compressed data from RAM cache
					if (KVRAMCache::getInstance()->getTexture(
						mID, texture_data, data_size, discard_level, width, height, components))
					{
						// RAM CACHE HIT - Load J2C data into mFormattedImage
						// Must copy data since cache retains ownership

						// Allocate aligned memory for J2C data (LLImageFormatted expects aligned allocation)
						U8* data_copy = (U8*)ll_aligned_malloc_16(data_size);
						if (!data_copy)
						{
							// CRITICAL FAILURE: System out of memory
							LL_WARNS("KVRAMCache") << "Failed to allocate memory for texture " << mID << LL_ENDL;
							// Fall through to normal disk cache path
						}
						else
						{
							// Copy J2C data from cache
							memcpy(data_copy, texture_data, data_size);

									// Create J2C image container and give it the data
									// setData() takes ownership of the pointer
									if (mFormattedImage.isNull())
									{
										mFormattedImage = LLImageFormatted::createFromType(IMG_CODEC_J2C);
									}
									mFormattedImage->setData(data_copy, data_size);

									// Set state exactly as disk cache callback would
									mLoadedDiscard = discard_level;
									mLoaded = true;
									mFileSize = data_size;
									...
									// mHaveAllData should ONLY be true if:
									//   1. We have the complete file (mFileSize check), AND
									//   2. The cached discard level is good enough for current request
									// If cached discard is worse than desired, system will fetch better quality
									if (mFileSize != 0 && mFormattedImage->getDataSize() >= mFileSize)
									{
										// We have complete data for this discard level
										// Now check if it's good enough quality
										if (discard_level <= mDesiredDiscard)
										{
											// Cached quality meets or exceeds request
											mHaveAllData = true;
										}
										// else: we have complete low-res data, but need better quality - will fetch from network
									}
									...
									if (sKVRAMDebugLog)
									{
										LL_WARNS("KVRAMCache") << "RAM HIT: " << mID 
											<< " Size: " << data_size 
											<< " " << width << "x" << height << "x" << (S32)components
											<< " Discard: " << discard_level 
											<< " Desired: " << mDesiredDiscard
											<< (discard_level <= mDesiredDiscard ? " (satisfied)" : " (will upgrade)")
											<< LL_ENDL;
									}

									// Don't call doWork recursively or change state
									// Just fall through to the mLoaded check below which will proceed to CACHE_POST
								}
							}
				}
			}
		...
		if (mLoaded)
		{
			// RAM cache hit path: if handle is null, data is already loaded
			if (mCacheReadHandle == LLTextureCache::nullHandle())
			{
				// RAM cache provided data directly - proceed to CACHE_POST
				setState(CACHE_POST);
				mCacheReadTime = 0.0f;  // Instant "read" from RAM
				// fall through
			}
			// Make sure request is complete. *TODO: make this auto-complete
			else if (mFetcher->mTextureCache->readComplete(mCacheReadHandle, false))
			{
				mCacheReadHandle = LLTextureCache::nullHandle();
				setState(CACHE_POST);
				add(LLTextureFetch::sCacheHit, 1.0);
	...
	if (mState == CACHE_POST)
	{
		...
		if ((mCachedSize >= mDesiredSize) || mHaveAllData)
		{
			...
			mInCache = true;
			mWriteToCacheState = NOT_WRITE;
			// S24 RAM CACHE: Log cache hit to detect potential re-decode loops
			LL_DEBUGS("TextureCache") << "RAM Cache HIT: " << mID << " Size: " << mFormattedImage->getDataSize() 
									  << " Discard: " << mLoadedDiscard << " (write-back disabled)" << LL_ENDL;
	...
	if (mState == LOAD_FROM_NETWORK)
	{
		...
		if (use_http && mCanUseHTTP && mUrl.empty())//get http url.
		{
			...
			if (region)
			{
				...
				else
				{
					...
					else
					{
						...
						LL_DEBUGS(LOG_TXT) << "Texture not available via HTTP: empty URL." << LL_ENDL; // S24 Debug due to console spam
					}
				}
			}
			else
			{
				...
				LL_DEBUGS(LOG_TXT) << "Texture not available via HTTP: no region " << mUrl << LL_ENDL;  // S24 Debug due to console spam
			}
		}
	...
	if (mState == WAIT_HTTP_REQ)
	{
		...
		if (mLoaded)
		{
			...
			if (mRequestedSize < 0)
			{
				...
				setState(DONE);
				releaseHttpSemaphore();
				LL_DEBUGS(LOG_TXT) << mID << " abort: fail harder" << LL_ENDL; // S24 world map causes massive SPAM stop it except for test builds!
				return true; // failed
			}
	...
	if (mState == WRITE_TO_CACHE)
	{
		...
		{
			// S24 RAM CACHE: Detect if we're skipping write because texture was from cache
			if (mInCache)
			{
				LL_DEBUGS("TextureCache") << "RAM Cache: Skipping redundant write for " << mID 
										  << " (already in cache)" << LL_ENDL;
			}
		...
		// S24 RAM CACHE: Warn if we're writing back a texture that was marked as cached
		if (mInCache)
		{
			LL_WARNS("TextureCache") << "**** RAM CACHE WRITE-BACK LOOP DETECTED ****" << LL_ENDL;
			LL_WARNS("TextureCache") << "Texture: " << mID << LL_ENDL;
			LL_WARNS("TextureCache") << "This texture was loaded from cache but is being written back!" << LL_ENDL;
			LL_WARNS("TextureCache") << "ACTION: Report to developer - potential cache thrashing issue" << LL_ENDL;
		}
		...
		// S24 STAGE 3 INJECTION POINT 2: RAM Cache Write (Exclusive Write Path)
		// When RAM cache is ACTIVE, decode goes EXCLUSIVELY to RAM
		// RAM cache handles disk writes via its decay algorithm
		// This maximizes RAM usage and minimizes disk activity
		// Cached booleans for performance
		static LLCachedControl<bool> sKVRAMCacheEnabled(gSavedSettings, "KVRAMCacheEnabled", false);
		static LLCachedControl<bool> sKVRAMDebugLog(gSavedSettings, "KVRAMCacheDebugLogging", false);

		bool ram_cache_accepted = false;

		if (sKVRAMCacheEnabled && KVRAMCache::instanceExists() && mRawImage.notNull())
		{
			// Offer J2C compressed data to RAM cache
				// Cache will make internal copy - we retain ownership
				ram_cache_accepted = KVRAMCache::getInstance()->acceptEviction(
					mID,
					mFormattedImage->getData(),  // J2C compressed data
					datasize,
					mDecodedDiscard,
					mRawImage->getWidth(),
					mRawImage->getHeight(),
			...
			// CRITICAL: Only write to disk if RAM cache is disabled OR rejected the texture
			// When RAM cache is active and accepts, it becomes responsible for disk writes via decay
			if (!ram_cache_accepted)
		{
		...
		{
			// RAM cache accepted - skip disk write entirely
			// Set state to WAIT_ON_WRITE with null handle so writeToCacheComplete() returns true immediately
			mCacheWriteTimer.reset();
			mCacheWriteHandle = LLTextureCache::nullHandle();
			mWritten = true;
			setState(WAIT_ON_WRITE);
...
void LLTextureFetchWorker::onCompleted(LLCore::HttpHandle handle, LLCore::HttpResponse* response)
{
	...
	if (!status)
	{
		...
		{
            LL_INFOS(LOG_TXT) << "CURL GET FAILED, status: " << status.toTerseString()
	...
	mTextureInfoMainThread(false)
{
	...
	mHttpHeaders->append(std::string{ HTTP_OUT_HEADER_ACCEPT }, std::string{ HTTP_CONTENT_IMAGE_X_J2C });
	mHttpPolicyClass = app_core_http.getPolicy(LLAppCoreHttp::AP_TEXTURE);
	mHttpMetricsHeaders = std::make_shared<LLCore::HttpHeaders>();
	mHttpMetricsHeaders->append(std::string{ HTTP_OUT_HEADER_CONTENT_TYPE }, std::string{ HTTP_CONTENT_LLSD_XML });
...
// Threads:  T*
//
// Remvoves the request id from the HTTP queue.  This is called when
// a request is completed (successfully or not).
//
...
{
	// Fast path: nothing to do if we're already at or above the low water mark.
    // to avoid a lock.
    ...
    for (worker_list_t::iterator iter2(tids2.begin()); tids2.end() != iter2; ++iter2)
    {
		...
		{
			// No slots available; release worker lock and stop trying.
			worker->unlockWorkMutex();                                  // -Mw
			break;
        }
