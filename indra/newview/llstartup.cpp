...
bool idle_startup()
{
	...
	if (STATE_FIRST == LLStartUp::getStartupState())
	{
		...
		if (false == gSavedSettings.getBOOL("NoAudio"))
		{
...
#ifdef LL_FMODSTUDIO // S24 Do NOT remove this otherwise FMOD we use breaks!
			...
			if (gAudiop)
			{
				...
				{
					// Choose streaming implementation based on user preference
					// 0 = Media Plugins (libVLC, better codec support)
					// 1 = FMOD (metadata support)
					S32 streaming_engine = gSavedSettings.getS32("StreamingAudioEngine");

					if (streaming_engine == 0)
					{
						// Use Media Plugins (libVLC) for better codec support
						LL_INFOS("AppInit") << "Using Media Plugins (libVLC) for streaming audio" << LL_ENDL;
						gAudiop->setStreamingAudioImpl(new LLStreamingAudio_MediaPlugins());
					}
					else
					{
						// Use FMOD's default streaming implementation (supports metadata)
						LLStreamingAudioInterface* stream_impl = gAudiop->createDefaultStreamingAudioImpl();
						if (stream_impl)
						{
							LL_INFOS("AppInit") << "Using FMOD for streaming audio (metadata supported)" << LL_ENDL;
							gAudiop->setStreamingAudioImpl(stream_impl);
						}
					}
		...
		LL_WARNS("AppInit") << "Audio Engine Initialized." << LL_ENDL;
	...
	if (STATE_LOGIN_PROCESS_RESPONSE == LLStartUp::getStartupState())
	{
		...
		if (LLLoginInstance::getInstance()->authFailure())
		{
				...
				|| reason_response == "optional")
			{
                ...
                // S24 Not Applicable - LL_INFOS("LLStartUp") << "Forcing a quit due to update." << LL_ENDL;
	...
	if (STATE_WORLD_INIT == LLStartUp::getStartupState())
	{
		...
		update_tech_status(); // S24: Update tech panel
		do_startup_frame();
	...
	if (STATE_SEED_CAP_GRANTED == LLStartUp::getStartupState())
	{
		...
		// S24: SECURITY - Removed server IP/port/circuit code from client logs
		// Information disclosure vulnerability: Server infrastructure details should not be advertised in user-accessible logs
		// Previous: Logged gFirstSim (IP:port) and circuit code, creating attack surface map
		// KL : "the aggregation of excessive information is extremely bad"
		// While attackers can discover this through network analysis, we shouldn't advertise it
		// Aggregation of IPs + ports + circuit codes + UUIDs + assets creates complete infrastructure blueprint
		// Complies with: Operational security best practices, data minimization principles (GDPR/CCPA), defense in depth
		LL_DEBUGS("AppInit") << "viewer: UserLoginLocationReply() circuit enabled" << LL_ENDL;
	...
	if (STATE_MISC == LLStartUp::getStartupState())
	{
		...
		LLPresetsManager::getInstance()->createMissingDefault(PRESETS_WATER); // S24 - Water shader presets
	...
	if (STATE_CLEANUP == LLStartUp::getStartupState())
	{
		...
		// ============================================================================
		// S24 CUSTOM: Apply Object Derender at Startup Complete
		// ============================================================================
		// RATIONALE: Use the SAME logic that works perfectly per-session (updateAdd)
		//            and apply it to ALL muted objects after login.
		// WHY:       Per-session derender works flawlessly, so just replicate it!
		// BENEFIT:   - Uses proven working code
		//            - No complex timing/flags
		//            - Same reliable behavior as manual block
		// MERGE:     Keep this - it's the simple working solution.
		// ============================================================================
		LLMuteList::getInstance()->applyObjectDerender();
		// ============================================================================
		// END S24 CUSTOM: Apply Object Derender at Startup Complete
		// ============================================================================
...
void validate_release_notes_coro(const std::string url)
{
        ...
        && ll_regex_search(url, version_regex)) // has any version
    {
        ...
        // S24 Updater only provides notes for a most recent version, if it is not
        // the current one (which it wont be), fall back to the hardcoded URL. (Our default!)
...
void show_release_notes_if_required()
{
	...
	// S24: All builds use internal browser for release notes
	if (!release_notes_shown &&
		LLVersionInfo::instance().getChannelAndVersion() != gLastRunVersion &&
		// Removed TEST_VIEWER filter to always show notes
...
// S24: Update tech status display with system info and features
void update_tech_status()
{
	static bool gpu_info_set = false;
	static std::string last_status_text = "⚙ Initializing Graphics...";

	// GPU info (show once detected)
	if (gGLManager.mVRAM > 0 && !gpu_info_set)
	{
		std::string gpu_name = gGLManager.mGLRenderer;
		// Trim long GPU names for display
		if (gpu_name.length() > 35)
		{
			gpu_name = gpu_name.substr(0, 32) + "...";
		}

		// S24: Multi-line format using text_editor (no line limits!)
		if (gGLManager.mVRAM >= 4096)
		{
			last_status_text = llformat("✓ GPU: %s\n✓ VRAM: %dMB\n⚡ Ultra Graphics • PBR • Shadows • Water", 
				gpu_name.c_str(), gGLManager.mVRAM);
		}
		else
		{
			last_status_text = llformat("✓ GPU: %s\n✓ VRAM: %dMB\n⚡ S24 Rendering • PBR • Dynamic Effects", 
				gpu_name.c_str(), gGLManager.mVRAM);
		}
		gpu_info_set = true;
	}
	else if (!gpu_info_set)
	{
		last_status_text = "⚙ Initializing Graphics...";
	}
	// else: gpu_info_set is true, keep last_status_text (already has GPU info)

	gViewerWindow->setProgressTechStatus(last_status_text);
}
...
bool process_login_success_response()
{
	...
	LLAppViewer::instance()->recordSessionToMarker(); // S24 testing
