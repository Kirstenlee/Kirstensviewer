...
// S24_IMPROVEMENT: Optimized LOD triangle counting - single iteration
void LLViewerObject::recursiveGetLODTriangleCount(S32 &high_lod, S32 &medium_lod, S32 &low_lod, S32 &lowest_lod) const
{
    // Initialize with root object's counts
    ...
    // S24_OPTIMIZATION: Single child iteration for all LOD levels (LL original called getLODTriangleCount 4x per child)
    const LLViewerObject::const_child_list_t& child_list = getChildren();
    for (LLViewerObject::const_child_list_t::const_iterator iter = child_list.begin();
        iter != child_list.end(); ++iter)
    {
        const LLViewerObject* childp = *iter;
        if (childp)  // S24_DEFENSIVE: const correctness, null check
        {
...
bool LLViewerObject::isOwnerInMuteList(LLUUID id) const
{
    ...
    else
    {
        ...
        // S24: Const-cast to update cache - safe because cache is mutable concept
        const_cast<LLViewerObject*>(this)->mCachedMuteListUpdateTime = now + SECONDS_BETWEEN_MUTE_UPDATES;
        const_cast<LLViewerObject*>(this)->mCachedOwnerInMuteList = muted;
    }
    return muted;
}
...
void LLViewerObject::updateFlags(bool physics_changed)
{
    ...
    gMessageSystem->addBOOL("CastsShadows", false ); // S24 I have seen failures with THIS named when setting properties on object in build mode! ???
...
//  if (setit)
    {
    updateFlags(); // Always push update due to viewer/simulator sync bug
    }
    return setit;
}
