    ...
    mAlertNotif(false)
{
    ...
    if (!start_slurl.isSpatial())  // No start location via command line or NextLoginLocation
    {
...
void LLPanelLogin::setFields(LLPointer<LLCredential> credential)
{
    ...
    // Populate password field with placeholder stars if authenticator exists
                             ...
                             bool& remember_psswrd)
{
    ...
    // Determine if username is in "first last" format
    size_t separator_index = username.find_first_of(' ');
    {
        ...
        if (last.find_first_of(' ') == last.npos)
        {
            ...
            // Check if username is in traditional "firstname lastname" format
...
{
    // #5607: Workaround for CEF black screen after resize
