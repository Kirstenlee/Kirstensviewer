...
void LLJoystickCameraRotate::drawRotatedImage( LLPointer<LLUIImage> image, S32 rotations )
{
    ...
    gDX.getTexUnit(0)->bind(texture);

    gDX.color4fv(UI_VERTEX_COLOR.mV);

    gDX.begin(LLRender::TRIANGLES);
    {
        gDX.texCoord2fv( uv[ (rotations + 0) % 4]);
        gDX.vertex2i(width, height );

        gDX.texCoord2fv(uv[(rotations + 1) % 4]);
        gDX.vertex2i(0, height);

        gDX.texCoord2fv(uv[(rotations + 2) % 4]);
        gDX.vertex2i(0, 0);
        ...
        gDX.texCoord2fv( uv[ (rotations + 3) % 4]);
...
void LLJoystickCameraTrack::onHeldDown()
{
    ...
    // Calculate progressive rate that ramps up from zero over time
    // This provides smooth acceleration from standstill, giving fine control
    F32 time = getElapsedHeldDownTime();
    F32 rate = 0.f;

    if (time < NUDGE_TIME)
    {
        // Gentle ramp-up from 0 to ORBIT_NUDGE_RATE over NUDGE_TIME
        rate = ORBIT_NUDGE_RATE * (time / NUDGE_TIME);
    }
    else
    {
        // Continue ramping from ORBIT_NUDGE_RATE to full speed
        F32 accel_time = time - NUDGE_TIME;
        const F32 FULL_SPEED_TIME = 1.0f; // Time to reach full speed after nudge phase
        rate = ORBIT_NUDGE_RATE + (1.f - ORBIT_NUDGE_RATE) * llmin(accel_time / FULL_SPEED_TIME, 1.f);
    }

    // Apply the time-based rate to whichever direction is active
    if (dx > mVertSlopNear)
    {
        gAgentCamera.unlockView();
        gAgentCamera.setPanRightKey(rate);
    }
    else if (dx < -mVertSlopNear)
    {
        gAgentCamera.unlockView();
        gAgentCamera.setPanLeftKey(rate);
    }

    if (dy > mVertSlopNear)
    {
        gAgentCamera.unlockView();
        gAgentCamera.setPanUpKey(rate);
    }
    else if (dy < -mVertSlopNear)
    {
        gAgentCamera.unlockView();
        gAgentCamera.setPanDownKey(rate);
    }
}
