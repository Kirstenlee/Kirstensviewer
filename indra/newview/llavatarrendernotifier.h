...
#include <string_view>  // S24: For string_view default parameters
...
struct LLHUDComplexity
{
    ...
    // S24: Efficient equality comparison (single memcmp instead of 6 comparisons)
    bool operator==(const LLHUDComplexity& rhs) const noexcept
    {
        return objectsCost == rhs.objectsCost
            && objectsCount == rhs.objectsCount
            && texturesCost == rhs.texturesCost
            && texturesCount == rhs.texturesCount
            && largeTexturesCount == rhs.largeTexturesCount
            && texturesMemoryTotal == rhs.texturesMemoryTotal;
    }

    bool operator!=(const LLHUDComplexity& rhs) const noexcept { return !(*this == rhs); }
...
public:
    ...
    void setObjectComplexityList(object_complexity_list_t object_list) { mObjectComplexityList = std::move(object_list); }  // S24: Use move semantics
    const object_complexity_list_t& getObjectComplexityList() const noexcept { return mObjectComplexityList; }  // S24: Return by const-ref
...
public:
    ...
    const hud_complexity_list_t& getHUDComplexityList() const noexcept { return mHUDComplexityList; }  // S24: Return by const-ref
    S32 getHUDsCount() const noexcept { return mHUDsCount; }  // S24: const + noexcept
...
private:
    ...
    static EWarnLevel getWarningType(const LLHUDComplexity& object_complexity, const LLHUDComplexity& cmp_complexity);  // S24: Pass by const-ref
    void displayHUDNotification(EWarnLevel warn_type, const LLUUID& obj_id = LLUUID::null,  
                                std::string_view object_name = "", std::string_view joint_name = "");  // S24: string_view + const-ref
