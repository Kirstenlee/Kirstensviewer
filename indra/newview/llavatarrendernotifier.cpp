...
{
    // S24: Cache translated strings - avoid repeated allocations
    static const std::string everyone_now = LLTrans::getString("av_render_everyone_now");
    static const std::string not_everyone = LLTrans::getString("av_render_not_everyone");
    static const std::string over_half = LLTrans::getString("av_render_over_half");
    static const std::string most = LLTrans::getString("av_render_most_of");
    static const std::string anyone = LLTrans::getString("av_render_anyone");

    if ( mLatestOverLimitPct >= 99.0f )
    {
        return anyone;
    }
    else if ( mLatestOverLimitPct >= 75.0f )
    {
        return most;
    }
    else if ( mLatestOverLimitPct >= 50.0f )
    {
        return over_half;
    }
    else if ( mLatestOverLimitPct > 10.0f )
    {
        return not_everyone;
    }
    else
...
void LLAvatarRenderNotifier::displayNotification(bool show_over_limit)
{
    ...
    // S24: Changed to LL_DEBUGS to avoid console spam (was LL_WARNS unconditional)
...
void LLAvatarRenderNotifier::updateNotificationRegion(U32 agentcount, U32 overLimit)
{
    ...
    // S24: Precompute percentage (avoid division on every call)
    constexpr float TO_PERCENTAGE = 100.0f;
    mLatestOverLimitPct = mLatestAgentsCount != 0 
        ? (static_cast<float>(overLimit) / static_cast<float>(mLatestAgentsCount)) * TO_PERCENTAGE 
        : 0.0f;
...
void LLHUDRenderNotifier::updateNotificationHUD(hud_complexity_list_t complexity)
{
    ...
    mHUDComplexityList = std::move(complexity);  // S24: Use move semantics (avoid deep copy)
    ...
    // S24: Use const-ref in range-based for loop (avoid struct copies)
    for (const auto& object_complexity : mHUDComplexityList)
    {
    ...
    // S24: Use operator!= instead of 6 separate field comparisons (more efficient)
    if (mLatestHUDComplexity != new_total_complexity)
    {
...
// private static
LLHUDRenderNotifier::EWarnLevel LLHUDRenderNotifier::getWarningType(const LLHUDComplexity& object_complexity, const LLHUDComplexity& cmp_complexity)  // S24: const-ref
{
...
void LLHUDRenderNotifier::displayHUDNotification(EWarnLevel warn_type, const LLUUID& obj_id, std::string_view obj_name, std::string_view joint_name)  // S24: const-ref + string_view
{
    static LLCachedControl<U32> pop_up_delay(gSavedSettings, "ComplexityChangesPopUpDelay", 300);
    static LLCachedControl<U32> expire_delay(gSavedSettings, "ShowMyComplexityChanges", 20);
    LLDate expire_date(LLDate::now().secondsSinceEpoch() + expire_delay);
    ...
    else
    {
        ...
        if (joint_name.empty())
        {
            // S24: Explicit string_view conversion required (string_view doesn't implicitly convert to avoid copies)
            std::string verb = "select?name=" + LLURI::escape(std::string(obj_name));
            reason_args["HUD_DETAILS"] = LLSLURL("inventory", obj_id, verb).getSLURLString();  // Pass verb directly, not .c_str()
        }
        else
        {
            LLSD object_args;
            std::string verb = "select?name=" + LLURI::escape(std::string(obj_name));
            object_args["OBJ_NAME"] = LLSLURL("inventory", obj_id, verb).getSLURLString();  // Pass verb directly
            object_args["JNT_NAME"] = LLTrans::getString(std::string(joint_name));
