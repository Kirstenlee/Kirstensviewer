...
void LLFastTimerView::exportCharts(const std::string& base, const std::string& target)
{
    ...
    gDX.loadIdentity();
    gDX.ortho(-0.05f, 1.05f, -0.05f, 1.05f, -1.0f, 1.0f);
    ...
    for (std::set<std::string>::iterator iter = chart_names.begin(); iter != chart_names.end(); ++iter)
    {
        ...
        gDX.color3fv(base_col.mV);
        for (U32 i = 0; i < base_times.size(); ++i)
        {
            gDX.begin(LLRender::TRIANGLE_STRIP);
            gDX.vertex3fv(last_p.mV);
            gDX.vertex3f(last_p.mV[0], 0.f, 0.f);
...
void LLFastTimerView::drawLineGraph()
{
    ...
    //highlight visible range
    {
        ...
        if (mHoverBarIndex > 0)
        {
            ...
            gDX.color4f(0.5f,0.5f,0.5f,1);

            gDX.begin(LLRender::LINES);
            gDX.vertex2i((S32)bar, mGraphRect.mBottom);
            gDX.vertex2i((S32)bar, mGraphRect.mTop);
            gDX.end();
        }
    }
        ...
        ++it)
    {
        ...
        gDX.color4f(col[0], col[1], col[2], alpha);
        gDX.begin(LLRender::TRIANGLE_STRIP);
...
{
    // S24: Improved fast timer legend with better typography and percentages
    S32 dx;
...

    {
        ...
        // S24: Get frame time for percentage calculations
        F32Milliseconds frame_time = mRecording.getPeriodMean(FTM_FRAME, RUNNING_AVERAGE_WIDTH);
        if (frame_time.value() < 0.001f) frame_time = F32Milliseconds(16.0f); // Avoid divide by zero
            ...
            ++it)
        {
            ...
            {
                scroll_offset += IMPROVED_TEXT_HEIGHT;
                if (idp->getTreeNode().mCollapsed)
                {
                    it.skipDescendants();
                }
            ...
            // S24: Draw color square with slight glow on hover
            x = MARGIN;
            LLRect bar_rect(x, y, x + TEXT_HEIGHT, y - TEXT_HEIGHT);
            S32 scale_offset = 0;
            if (idp == mHoverID)
            {
                scale_offset = llfloor(sinf(mHighlightTimer.getElapsedTimeF32() * 6.f) * 3.f); // S24: More pronounced highlight
            }
            ...
            // S24: Calculate percentage of frame time
            F32 percentage = (ms.value() / frame_time.value()) * 100.0f;

            // S24: Build label with percentage
            std::string timer_label;
            switch(mDisplayType)
            {
            case DISPLAY_TIME:
                // S24: Show percentage alongside milliseconds for better context
                if (percentage >= 0.1f) // Only show percentage if meaningful
                {
                    timer_label = llformat("%s  %.1fms (%.1f%%)", idp->getName().c_str(), ms.value(), percentage);
                }
                else
                {
                    timer_label = llformat("%s  %.1fms", idp->getName().c_str(), ms.value());
                }
                break;
            case DISPLAY_CALLS:
                ...
                break;
            case DISPLAY_HZ:
                timer_label = llformat("%.1f Hz", ms.value() ? (1.f / ms.value()) : 0.f);
                break;
            }
            dx = (TEXT_HEIGHT+4) + get_depth(idp)*8;

            // S24: Color code by performance impact
            LLColor4 color;
            if (percentage > 50.0f)
            {
                color = LLColor4(1.0f, 0.3f, 0.3f, 1.0f); // Red - Major bottleneck
            }
            else if (percentage > 25.0f)
            {
                color = LLColor4(1.0f, 0.8f, 0.3f, 1.0f); // Yellow - Significant
            }
            else if (percentage > 10.0f)
            {
                color = LLColor4(1.0f, 1.0f, 0.6f, 1.0f); // Light yellow - Notable
            }
            else
            {
                color = LLColor4(0.9f, 0.9f, 0.9f, 1.0f); // White - Normal
            }

            // S24: Draw tree structure lines
            if (get_depth(idp) > 0)
            {
            ...
            // S24: Use SansSerif font and bold for hovered items
...
void LLFastTimerView::drawBars()
{
    ...
    gDX.getTexUnit(0)->bind(bar_image->getImage());
    {
