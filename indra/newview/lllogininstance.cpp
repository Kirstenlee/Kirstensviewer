    ...
    mDispatcher("LLLoginInstance", "change")
{
    ...
    mDispatcher.add("authenticating", "", boost::bind(&LLLoginInstance::handleAuthenticating, this, _1));
    mDispatcher.add("downloading", "", boost::bind(&LLLoginInstance::handleDownloading, this, _1));
}
...
void LLLoginInstance::handleAuthenticating(const LLSD& event)
{
    // Informational state - authentication in progress
    LL_DEBUGS("LLLogin") << "LLLoginInstance::handleAuthenticating" << LL_ENDL;
}

void LLLoginInstance::handleDownloading(const LLSD& event)
{
    // Informational state - downloading login response data
    LL_DEBUGS("LLLogin") << "LLLoginInstance::handleDownloading" << LL_ENDL;
}
...
void LLLoginInstance::saveMFAHash(LLSD const& response)
{
    ...
    // related to SL-17223 consider building a better interface that sync's automatically
