...
void LLAgent::moveAt(S32 direction, bool reset)
{
    ...
    // S24: Apply smoothing to flight controls
    static LLCachedControl<bool> smooth_flight_enabled(gSavedSettings, "SmoothFlightMovement");
    static LLCachedControl<F32> min_threshold(gSavedSettings, "SmoothFlightMinThreshold");
    static LLCachedControl<F32> fast_threshold(gSavedSettings, "SmoothFlightFastThreshold");

    if (direction > 0)
    {
        if (smooth_flight_enabled && getFlying())
        {
            // Use smoothed values for flight
            F32 smoothed_at = gAgentCamera.getSmoothedAtKey();

            // S24: Only send movement if smoothed value exceeds minimum threshold (creates dead zone for damping)
            if (smoothed_at >= min_threshold)
            {
                setControlFlags(AGENT_CONTROL_AT_POS);

                // Only add FAST flag when smoothed value exceeds fast threshold
                if (smoothed_at >= fast_threshold)
                {
                    setControlFlags(AGENT_CONTROL_FAST_AT);
                }
            }
        }
        else
        {
            // Standard instant response for ground movement
    ...
    else if (direction < 0)
    {
        if (smooth_flight_enabled && getFlying())
        {
            ...
            // S24: Only send movement if smoothed value exceeds minimum threshold (creates dead zone for damping)
            if (fabs(smoothed_at) >= min_threshold)
            {
                setControlFlags(AGENT_CONTROL_AT_NEG);

                if (fabs(smoothed_at) >= fast_threshold)
                {
                    setControlFlags(AGENT_CONTROL_FAST_AT);
                }
            }
...
void LLAgent::moveLeft(S32 direction)
{
    ...
    if (direction > 0)
    {
        if (smooth_flight_enabled && getFlying())
        {
            F32 smoothed_left = gAgentCamera.getSmoothedLeftKey();

            // S24: Only send movement if smoothed value exceeds minimum threshold (creates dead zone for damping)
            if (smoothed_left >= min_threshold)
            {
                setControlFlags(AGENT_CONTROL_LEFT_POS);

                if (smoothed_left >= fast_threshold)
                {
                    setControlFlags(AGENT_CONTROL_FAST_LEFT);
                }
            }
        }
        else
    ...
    else if (direction < 0)
    {
        if (smooth_flight_enabled && getFlying())
        {
            ...
            // S24: Only send movement if smoothed value exceeds minimum threshold (creates dead zone for damping)
            if (fabs(smoothed_left) >= min_threshold)
            {
                setControlFlags(AGENT_CONTROL_LEFT_NEG);

                if (fabs(smoothed_left) >= fast_threshold)
                {
                    setControlFlags(AGENT_CONTROL_FAST_LEFT);
                }
            }
...
void LLAgent::moveUp(S32 direction)
{
    ...
    if (direction > 0)
    {
        ...
        if (smooth_flight_enabled && getFlying())
        {
            F32 smoothed_up = gAgentCamera.getSmoothedUpKey();

            // S24: Only send movement if smoothed value exceeds minimum threshold (creates dead zone for damping)
            if (smoothed_up >= min_threshold)
            {
                setControlFlags(AGENT_CONTROL_UP_POS);

                // Add FAST flag when smoothed value exceeds fast threshold
                if (smoothed_up >= fast_threshold)
                {
                    setControlFlags(AGENT_CONTROL_FAST_UP);
                }
            }
        }
        else
        {
            // Standard instant response for jumping and ground movement
    ...
    else if (direction < 0)
    {
        if (smooth_flight_enabled && getFlying())
        {
            ...
            // S24: Only send movement if smoothed value exceeds minimum threshold (creates dead zone for damping)
            if (fabs(smoothed_up) >= min_threshold)
            {
                setControlFlags(AGENT_CONTROL_UP_NEG);

                // Add FAST flag when smoothed value exceeds fast threshold
                if (fabs(smoothed_up) >= fast_threshold)
                {
                    setControlFlags(AGENT_CONTROL_FAST_UP);
                }
            }
...
{
    // S24: Update smooth movement interpolation first
    gAgentCamera.updateSmoothMovement(dt);
    ...
    if (fabs(dt) > 1e-6)
    {
        // S24: Use smoothed yaw key for rotation
        static LLCachedControl<bool> smooth_movement_enabled(gSavedSettings, "SmoothKeyboardMovement");
        F32 yaw_key = smooth_movement_enabled ? gAgentCamera.getSmoothedYawKey() : gAgentCamera.getYawKey();

        if (fabs(yaw_key) > 1e-6)
        {
            static const F32 YAW_RATE = 90.f * DEG_TO_RAD;   // radians per second
            yaw(YAW_RATE * yaw_key * dt);
    }
...
void LLAgent::teleportCancel()
{
    if (!hasPendingTeleportRequest())
    {
        ...
        {
            // S24 ROBUSTNESS: If we're canceling a teleport but have no region,
            // we're already disconnected. Force immediate state cleanup to prevent
            // getting stuck in limbo waiting for a reply that will never come.
            LL_WARNS("Teleport") << "TeleportCancel called with no region - forcing immediate state cleanup" << LL_ENDL;
        }
        mTeleportCanceled = mTeleportRequest;
    }
    // S24 ROBUSTNESS: Always clear teleport state immediately, even if region is gone.
    // Prevents viewer from being stuck in TELEPORT_REQUESTED state after cancel.
    clearTeleportRequest();
...
void LLAgent::renderAutoPilotTarget()
{
    if (mAutoPilot)
    {
        ...
        // lovely green
        gDX.color4f(0.f, 1.f, 1.f, 1.f);

        target_global = mAutoPilotTargetGlobal;

        gDX.translatef((F32)(target_global.mdV[VX]), (F32)(target_global.mdV[VY]), (F32)(target_global.mdV[VZ]));

        height_meters = 1.f;

        gDX.scalef(height_meters, height_meters, height_meters);
