                                                   ...
                                                   const LLUUID& force_id)
{
        ...
        filename == "Uploading..." || filename == "(Uploading...)")
    {
        ...
        LL_WARNS("TextureList") << "Filename: '" << filename << "'" << LL_ENDL;
...
void LLViewerTextureList::updateImageDecodePriority(LLViewerFetchedTexture* imagep, bool flush_images)
{
    ...
    if (imagep->getBoostLevel() < LLViewerFetchedTexture::BOOST_HIGH)
    {
        ...
        {
            U32 num_faces = imagep->getNumFaces(channel);
            face_count += num_faces;

            // Detect priority channel inline during existing iteration
            if (num_faces > 0)
            {
                priority_channel = llmin(priority_channel, render_to_priority[channel]);
            }
            ...
            for (S32 fi = 0; fi < (S32)num_faces; ++fi)
            {
                LLFace* face = (*(imagep->getFaceList(channel)))[fi];
                if (!face || !face->getViewerObject())
                {
                    continue;
                }

                // Update the face's pixel area once every 10 frames.
                if ((gFrameCount - face->mLastTextureUpdate) > 10)
                {
                    F32 radius, cos_angle_to_view_dir;
                ...
                // Adjust for texture entry scaling.
                S32 te_offset = face->getTEOffset();
                LLViewerObject* objp = face->getViewerObject();
                const LLTextureEntry* te = (te_offset < 0 || te_offset >= objp->getNumTEs())
                    ? nullptr : objp->getTE(te_offset);
        ...
        // When a texture is used in many places, force it to full resolution.
        if (face_count > max_faces_to_check)
        {
    ...
    // Update spotlight priorities for each volume using this texture.
    ...
    // Lazy flush logic for unused images.
    const auto max_inactive_time = 20.f;
    const auto min_refs = 3;
    const auto lazy_flush_timeout = 30.f;
...
F32 LLViewerTextureList::updateImagesCreateTextures(F32 max_time)
{
    ...
    while (!mCreateTextureList.empty())
    {
        ...
        // Cache values to avoid repeated virtual calls.
        const bool has_gl = imagep->hasGLTexture();
        const S8 discard = imagep->getDiscardLevel();
        const S8 desired = imagep->getDesiredDiscardLevel();

        // Collapse redundant-load logic into a single predictable branch.
        ...
        // Scale-down check using cached values.
        if (has_gl && discard < desired && desired <= MAX_DISCARD_LEVEL)
            imagep->scaleDown();
    }
...
F32 LLViewerTextureList::updateImagesLoadingFastCache(F32 max_time)
{
    ...
    LLMutex* mutex = LLAppViewer::getTextureCache()->getFastCacheMutex();

    // --- Stage 1: Fast trylock ---
    {
        LLMutexTrylock try_lock(mutex);
        if (!try_lock.isLocked())
        {
            // --- Stage 2: Bounded retry window (game-engine) ---
            LLTimer spin_timer;
            const F32 spin_limit = 0.00005f; // ~50 microseconds

            bool locked = false;
            while (!locked)
            {
                LLMutexTrylock retry(mutex);
                if (retry.isLocked())
                {
                    locked = true;
                    break;
                ...
                if (spin_timer.getElapsedTimeF32() > spin_limit)
                    return 0.f; // Avoid stutter, skip this frame
            }
        }
    }

    // --- Stage 3: We now hold the lock ---
    LLMutexLock lock(mutex);

    auto iter = mFastCacheList.begin();
    const auto end = mFastCacheList.end();

    while (iter != end)
    {
        LLViewerFetchedTexture* imagep = iter->get();

        // Optional: prefetch next pointer
        auto next = iter;
        ++next;
    ...
    mFastCacheList.erase(mFastCacheList.begin(), iter);
...
void LLViewerTextureList::decodeAllImages(F32 max_time)
{
    ...
    // Fast cache loading
    F32 cache_time = updateImagesLoadingFastCache(max_time);
    if (enable_timing) max_time -= cache_time;
    ...
    while (true)
    {
        LLAppViewer::instance()->getTextureCache()->update(1);
        LLAppViewer::instance()->getImageDecodeThread()->update(1);
        fetch_pending = LLAppViewer::instance()->getTextureFetch()->update(1);
        ...
        if (fetch_pending == 0 || (enable_timing && timer.getElapsedTimeF32() > max_time))
        {
            break;
        }
    }
    ...
    if (enable_timing)
    {
        elapsed_time = timer.getElapsedTimeF32();
        max_time = llmax(max_time - elapsed_time, 0.001f);
    }
