...
vec3 tapRefMap(vec3 pos, vec3 dir, out float w, out float dw, float lod, vec3 c, int i)
{
    ...
    vec4 ret = textureLod(reflectionProbes, vec4(v.xyz, refIndex[i].x), adjusted_lod) * refParams[i].y;
...
vec3 sampleProbes(vec3 pos, vec3 dir, float lod)
{
    ...
    if (probe_saturation != 1.0)
    {
        ...
        result = mix(vec3(luma), result, probe_saturation);
    }
    ...
    if (probe_contrast != 1.0)
    {
        ...
        result = max(result, vec3(0.0)); // Clamp to prevent negative values
    }
