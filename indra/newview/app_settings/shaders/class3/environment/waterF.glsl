...
void calculateFresnelFactors(out vec3 df3, out vec2 df2, vec3 viewVec, vec3 wave1, vec3 wave2, vec3 wave3, vec3 wavef)
{
    ...
    df3 = pow(df3, vec3(waterFresnelPower)); // Was: df3 *= df3 (hardcoded power of 2)
...
void main()
{
    ...
    vec3 punctual = clamp(nl * (diffPunc + specPunc * waterSpecularIntensity), vec3(0), vec3(10)) * sunlit_linear * shadow * atten;
    ...
    vec3 tintedWater = fb.rgb * waterColorTint;
    vec3 untintedWater = fb.rgb;
    vec3 finalWater = mix(untintedWater, tintedWater, waterColorTintAlpha);
    color = mix(finalWater, radiance, min(1, df2.x)) + punctual.rgb;
