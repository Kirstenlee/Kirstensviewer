...
static LLPanelInjector<KVPanelProfileMetadata> t_panel_metadata("kv_panel_profile_metadata");
...
static const std::string PANEL_METADATA     = "panel_profile_metadata";
...
LLUUID post_profile_image(std::string cap_url, const LLSD &first_data, std::string path_to_image, LLHandle<LLPanel> *handle)
{
    ...
    uploaderhttpHeaders->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, std::string{"application/jp2"}); // optional
    uploaderhttpHeaders->append(std::string{HTTP_OUT_HEADER_CONTENT_LENGTH}, llformat("%d", length)); // required!
...
void LLPanelProfileWeb::updateData()
{
    ...
    if (!getStarted() && avatar_id.notNull() && !mURLWebProfile.empty())
    {
        ...
        mWebBrowser->navigateTo(mURLWebProfile, std::string{HTTP_CONTENT_TEXT_HTML});
    }
}
...
void LLPanelProfileWeb::onCommitLoad(LLUICtrl* ctrl)
{
    if (!mURLHome.empty())
    {
        ...
        if (valstr.empty())
        {
            mWebBrowser->setVisible(true);
            mPerformanceTimer.start();
            mWebBrowser->navigateTo( mURLHome, std::string{HTTP_CONTENT_TEXT_HTML} );
        }
...
bool LLPanelProfileFirstLife::postBuild()
{
    ...
    // S24: Add click callback to show expanded First Life picture (same as Second Life picture)
...
// S24: Show expanded First Life picture (same as Second Life picture)
...
void LLPanelProfile::onOpen(const LLSD& key)
{
    ...
    mPanelMetadata      = findChild<KVPanelProfileMetadata>(PANEL_METADATA);
    ...
    if (mPanelMetadata)
    {
        mPanelMetadata->onOpen(avatar_id);
    }
