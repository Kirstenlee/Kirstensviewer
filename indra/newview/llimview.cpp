...
void chatterBoxInvitationCoro(std::string url, LLUUID sessionId, LLIMMgr::EInvitationType invitationType, const LLSD& voiceChannelInfo)
{
    // S24 - Early out if this is a muted group chat (defensive check)
    if (gAgent.isInGroup(sessionId, true) && LLGroupChatMute::instance().isGroupChatMuted(sessionId))
    {
        LL_DEBUGS("ChatHistory") << sessionId << ": Skipping invitation acceptance for muted group chat" << LL_ENDL;
    ...
    if (!status)
    {
        // S24 - Handle 403 "You cannot join chat" gracefully for blocked group chats
        if (status.getType() == 403)
        {
            LLSD error_body = httpResults["error_body"];
            std::string error_msg = error_body.asString();

            // Check if this is a "cannot join chat" error (group chat disabled)
            if (error_msg.find("cannot join chat") != std::string::npos || 
                error_msg.find("You cannot join chat") != std::string::npos)
            {
                LL_DEBUGS("ChatHistory") << sessionId << ": Group chat disabled or access denied (403), skipping invitation" << LL_ENDL;
                gIMMgr->clearPendingAgentListUpdates(sessionId);
                gIMMgr->clearPendingInvitation(sessionId);
                return;
            }
...
{   // if parameters from, message and timestamp have values, they are a message that opened chat
    // S24 - Early out if this is a muted group chat (defensive check)
    if (gAgent.isInGroup(sessionId) && LLGroupChatMute::instance().isGroupChatMuted(sessionId))
    {
        LL_DEBUGS("ChatHistory") << sessionId << ": Skipping chat history fetch for muted group chat" << LL_ENDL;
        return;
    }
    ...
    if (!status)
    {
        ...
        if (status.getType() == 403)
        {
            ...
            if (error_msg.find("cannot join chat") != std::string::npos || 
                error_msg.find("You cannot join chat") != std::string::npos)
            {
                LL_DEBUGS("ChatHistory") << sessionId << ": Group chat disabled or access denied (403), skipping history fetch" << LL_ENDL;
                return;
            }
        }
...
LLIMSpeakerMgr* LLIMModel::getSpeakerManager( const LLUUID& session_id ) const
{
    ...
    if (!session)
    {
        // S24 - Don't warn for muted group chats (expected behavior when chat is disabled for a group)
        if (gAgent.isInGroup(session_id, true) && LLGroupChatMute::instance().isGroupChatMuted(session_id))
        {
            LL_DEBUGS("IM") << session_id << ": Skipping speaker manager access for muted group chat" << LL_ENDL;
        }
        else
        {
            LL_WARNS() << "session " << session_id << " does not exist " << LL_ENDL;
    ...
    std::string_view display_name)
{
    ...
    // Check if this is a muted group chat
    if (gAgent.isInGroup(new_session_id) && LLGroupChatMute::instance().isGroupChatMuted(new_session_id))
    {
        skip_message = true;
    }
    ...
    if (new_session)
    {
        ...
        if (session)
        {
            // Do not skip group chats unless they are muted
            bool is_muted_group_chat = session->isGroupSessionType() && 
                                       gAgent.isInGroup(new_session_id) && 
                                       LLGroupChatMute::instance().isGroupChatMuted(new_session_id);

            if (!is_muted_group_chat)
            {
            ...
            // Fetch group chat history, enabled by default (LL #5690 + S24 selective group muting)
            static LLCachedControl<bool> fetch_chat_history(gSavedPerAccountSettings, "FetchGroupChatHistory", true);
            bool fetch_history = fetch_chat_history;

            // LL #5690: Don't fetch history for peer-to-peer IMs
            if (!session->isP2PSessionType())
            {
                // S24: Only fetch for actual group chats, not ad-hoc conferences
                if (gAgent.isInGroup(new_session_id))
                {
                    // S24: Don't fetch history for muted group chats
                    if (LLGroupChatMute::instance().isGroupChatMuted(new_session_id))
                    {
                        fetch_history = false;
                    }
                }
                ...
                {
                    fetch_history = false; // Not a group chat
                }
            }
            else
            {
                fetch_history = false; // P2P chat - no history to fetch
            }
        ...
        if ( success )
        {
            ...
            if ( im_floater )
            {
                if ( body.has("session_info") )
                {
                    ...
                    // Send request for chat history, if enabled (LL #5690 + S24 selective group muting)
                    static LLCachedControl<bool> fetch_chat_history(gSavedPerAccountSettings, "FetchGroupChatHistory", true);
                    bool fetch_history = fetch_chat_history;

                    // LL #5690: Don't fetch history for peer-to-peer IMs
                    if (!im_floater->isP2PSessionType())
                    {
                        // S24: Only fetch for actual group chats, not ad-hoc conferences
                        if (gAgent.isInGroup(session_id))
                        {
                            // S24: Don't fetch history for muted group chats
                            if (LLGroupChatMute::instance().isGroupChatMuted(session_id))
                            {
                                fetch_history = false;
                            }
                        }
