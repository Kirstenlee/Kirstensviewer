...
// S24: Selection beam color picker callbacks (defined in llviewercontrol.cpp)
void handleGetBeamColor(LLUICtrl* ctrl, const LLSD& param);
void handleSetBeamColor(LLUICtrl* ctrl, const LLSD& param);
...
void init_menus()
{
    ...
    // Choose menu file based on UseMenuIcons setting
    std::string menu_file = gSavedSettings.getBOOL("UseMenuIcons") ? "menu_viewer_icons.xml" : "menu_viewer.xml";

    gMenuBarView = LLUICtrlFactory::getInstance()->createFromFile<LLMenuBarGL>(menu_file, gMenuHolder, LLViewerMenuHolderGL::child_registry_t::instance());
...
U32 render_type_from_string(std::string_view render_type)
{
    static const std::unordered_map<std::string_view, U32> render_type_map = {
        ...
        {"controlAV", LLPipeline::RENDER_TYPE_CONTROL_AV},
    ...
    auto it = render_type_map.find(render_type);
    return (it != render_type_map.end()) ? it->second : 0;
}
...
U32 feature_from_string(std::string_view feature)
{
    ...
    return (it != feature_map.end()) ? it->second : 0;
}
...
U64 info_display_from_string(std::string_view info_display)
{
    ...
    auto it = info_display_map.find(info_display);
    if (it != info_display_map.end())
    {
        return it->second;
    }
...
void show_buy_currency(const char* extra)
{
    ...
    LLNotificationsUtil::add("PromptGoToCurrencyPage", args);
}
...
class LLShowS24EngineGuide : public view_listener_t
{
    bool handleEvent(const LLSD& userdata)
    {
...
void initialize_menus()
{
    ...
    commit.add("Pref.getBeamColor", boost::bind(&handleGetBeamColor, _1, _2));
    commit.add("Pref.setBeamColor", boost::bind(&handleSetBeamColor, _1, _2));
    ...
    view_listener_t::addMenu(new LLShowS24EngineGuide(), "Help.ShowS24EngineGuide");
