...
void LLViewerParcelMgr::processParcelAccessListReply(LLMessageSystem *msg, void **user)
{
    ...
    // S24 Performance: Cache getLocalID() to avoid redundant function call
    // Why: getLocalID() was being called twice - once for comparison, once for logging
    // Impact: Eliminates one function call in error path (though LL_WARNS_ONCE limits frequency) if this were only true!
	// Savings: ~5-10 CPU cycles per invocation (negligible but good practice) - also switch to DEBUGS this warning has no client impact and can be very noisy if it happens frequently
    S32 current_parcel_id = parcel->getLocalID();
    if (parcel_id != current_parcel_id)
    {
        LL_DEBUGS("ParcelMgr") << "processParcelAccessListReply for parcel " << parcel_id
            << " which isn't the selected parcel " << current_parcel_id << LL_ENDL;
        return;
    }
