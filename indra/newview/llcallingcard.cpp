...
void LLAvatarTracker::setBuddyOnline(const LLUUID& id, bool is_online)
{
    ...
    {
        // S24: Changed to LL_DEBUGS - buddy info not loaded yet is a normal race condition during startup
        // Fires when friend status updates arrive before buddy list fully initialized (common at login)
        // Potential gain: Eliminate startup spam + reduce UUID exposure in client logs
        // Security: Friend UUIDs are PII, shouldn't be in production logs accessible to end users
        LL_DEBUGS("Friends") << "!! No buddy info found for " << id
                << ", setting to " << (is_online ? "Online" : "Offline") << LL_ENDL;
        LL_DEBUGS("Friends") << "Did we receive a buddy status update before the buddy info?" << LL_ENDL;
...
void LLAvatarTracker::processNotify(LLMessageSystem* msg, bool online)
{
    ...
    if(count > 0)
    {
        ...
        for(S32 i = 0; i < count; ++i)
        {
            ...
            {
                // S24: Changed to LL_DEBUGS - unknown buddy notifications are normal during buddy list initialization
                // Fires for every friend status change that arrives before buddy list loads (startup race condition)
                // Potential gain: Eliminate spam from frequent friend online/offline events + reduce UUID exposure
                // Security: Friend UUIDs are PII, client logs are accessible to end users (privacy violation)
                LL_DEBUGS("Friends") << "Received online notification for unknown buddy: "
                    << agent_id << " is " << (online ? "ONLINE" : "OFFLINE") << LL_ENDL;
                LL_DEBUGS("Friends") << "Adding buddy to buddy queue." << LL_ENDL;
