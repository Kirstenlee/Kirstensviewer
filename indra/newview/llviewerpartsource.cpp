...
void LLViewerPartSourceBeam::update(const F32 dt)
{
    ...
    if (dt_update > RATE)
    {
        ...
        // ========================================================================
        // S24: SELECTION BEAM PARTICLE CONTROL
        // ========================================================================
        // Check beam style setting - if line-only mode (style=1), skip particle
        // generation entirely. This allows clean GL line rendering without the
        // particle glow overlay.
        //
        // Style modes:
        //   0 = Particles only (this code runs normally)
        //   1 = Line only (skip particle generation, return early)
        //   2 = Hybrid (both GL line and particles render)
        // ========================================================================
        S32 beam_style = gSavedSettings.getS32("SelectionBeamStyle");
        if (beam_style == 1)  // Line only, no particles
        {
            return;
        }
        ...
        // ========================================================================
        // S24: UNIFIED COLOR FROM EFFECTCOLOR
        // ========================================================================
        // Read color from UIColorTable (single source of truth set in Preferences)
        // This ensures line and particles use the same color automatically.
        // ========================================================================
        LLColor4 particle_color = LLUIColorTable::instance().getColor("EffectColor");
        particle_color.mV[VALPHA] = 1.0f; // Force full opacity for visibility

        part->mStartColor = particle_color;
        part->mEndColor = part->mStartColor;
        part->mEndColor.mV[3] = 0.8f;  // S24: Keep high alpha for bright glow (was 0.4f)
        part->mColor = part->mStartColor;

        // ========================================================================
        // S24: PARTICLE SCALE (GLOW INTENSITY)
        // ========================================================================
        // Apply SelectionBeamParticleScale setting to control "glow" intensity.
        // Base scale is 0.1f, multiplied by user setting (0.5x - 3.0x range).
        // Higher values create a more pronounced glow around the GL line core.
        // ========================================================================
        F32 particle_scale = gSavedSettings.getF32("SelectionBeamParticleScale");
        F32 base_scale = 0.1f * particle_scale;
        part->mStartScale = LLVector2(base_scale, base_scale);
        part->mEndScale = LLVector2(base_scale, base_scale);
        part->mScale = part->mStartScale;

        part->mPosAgent = mPosAgent;
        part->mVelocity = mTargetPosAgent - mPosAgent;

        // S24: ADDITIVE BLENDING for bright glowing particles (not standard alpha blend)
        part->mBlendFuncDest = LLRender::BF_ONE;  // Additive!
        part->mBlendFuncSource = LLRender::BF_SOURCE_ALPHA;

        // S24: Add glow component for extra brightness
        part->mStartGlow = 0.3f;
        part->mEndGlow = 0.3f;
        part->mGlow = LLColor4U(255, 255, 255, 255);  // Full white glow
