...
static std::string title_string1a("Tex UUID Area  DDis(Req)  DecodePri(Fetch)");
static std::string title_string1b("Tex UUID Area  DDis(Req)  Fetch(DecodePri)");
static std::string title_string2("State       [download]    pk/max");
static std::string title_string3("");
static std::string title_string4("  W x H (Dis) Mem");

static S32 title_x1 = 0;
static S32 title_x2 = 245;  // State column
static S32 title_x3 = 440;  // pk/max pips (bar_left + bar_width + 10 = 330 + 100 + 10)
static S32 title_x4 = 490;  // W x H column
...
void LLTextureBar::draw()
{
    ...
    {
        color = LLColor4(0.0f, 1.0f, 1.0f, 1.0f); // Bright cyan for debug
    }
    else if (mHilite)
    {
        S32 idx = llclamp(mHilite,1,3);
        if (idx==1) color = LLColor4(1.0f, 0.8f, 0.2f, 1.0f); // Bright orange
        else if (idx==2) color = LLColor4(1.0f, 1.0f, 0.3f, 1.0f); // Bright yellow
        else color = LLColor4(1.0f, 0.5f, 0.8f, 1.0f); // Bright pink
    }
    else if (mImagep->mDontDiscard)
    {
        color = LLColor4(0.4f, 0.9f, 0.4f, 0.95f); // Bright green for important
    }
    else if (mImagep->getMaxVirtualSize() <= 0.0f)
    {
        color = LLColor4(0.5f, 0.5f, 0.5f, 0.6f); // Dimmed grey for inactive
    }
    else
    {
        color = LLColor4(0.9f, 0.9f, 0.9f, 0.85f); // Brighter white for readability
    }
    ...
    struct { const std::string desc; LLColor4 color; } fetch_state_desc[] = {
        { "Invalid", LLColor4(1.0f, 0.3f, 0.3f, 1.0f) },   // INVALID - Bright red
        { "Init", LLColor4(0.7f, 0.7f, 0.7f, 1.0f) }, // INIT - Light grey
        { "DiskRead", LLColor4(0.3f, 0.8f, 1.0f, 1.0f) },  // LOAD_FROM_TEXTURE_CACHE - Bright cyan
        { "DiskPost", LLColor4(0.4f, 0.6f, 1.0f, 1.0f) },  // CACHE_POST - Medium blue
        { "Network", LLColor4(0.3f, 1.0f, 0.3f, 1.0f) }, // LOAD_FROM_NETWORK - Bright green
        { "WaitHttp", LLColor4(0.5f, 0.9f, 0.3f, 1.0f) }, // WAIT_HTTP_RESOURCE - Yellow-green
        { "WaitHttp2", LLColor4(0.5f, 0.9f, 0.3f, 1.0f) }, // WAIT_HTTP_RESOURCE2 - Yellow-green
        { "Request", LLColor4(1.0f, 0.9f, 0.2f, 1.0f) },// SEND_HTTP_REQ - Bright yellow
        { "HttpWait", LLColor4(0.6f, 1.0f, 0.4f, 1.0f) }, // WAIT_HTTP_REQ - Light green
        { "Decode", LLColor4(1.0f, 0.8f, 0.2f, 1.0f) },// DECODE_IMAGE - Orange-yellow
        { "DecUpdate", LLColor4(0.7f, 0.9f, 0.5f, 1.0f) }, // DECODE_IMAGE_UPDATE - Pale green
        { "CacheWrite", LLColor4(0.9f, 0.5f, 1.0f, 1.0f) },// WRITE_TO_CACHE - Bright purple
        { "WriteWait", LLColor4(1.0f, 0.6f, 0.2f, 1.0f) },// WAIT_ON_WRITE - Orange
        { "Done", LLColor4(0.6f, 0.6f, 0.6f, 1.0f) },   // DONE - Grey
#define LAST_STATE 14
        { "Create", LLColor4(1.0f, 0.4f, 0.9f, 1.0f) }, // LAST_STATE+1 - Bright magenta
        { "Complete", LLColor4(0.4f, 1.0f, 0.4f, 1.0f) }, // LAST_STATE+2 - Bright green (complete)
    ...
    S32 bar_width = 100;
    S32 bar_left = 330;  // S24: Moved from 260 to 330 to accommodate longer state names (DiskRead, CacheWrite, etc)
    left = bar_left;
    ...
    {
        // Downloaded bytes - gradient effect
        right = left + llfloor(data_progress * (F32)bar_width);
        right = llclamp(right, left, bar_left + bar_width);
        if (right > left)
        {
            // Use gradient from cyan to blue based on progress
            F32 progress_clamped = llclamp(data_progress, 0.0f, 1.0f);
    ...
    S32 pip_width = 6;
    S32 pip_space = 14;
    S32 pip_x = bar_left + bar_width + 10; // Start pips 10 pixels after progress bar
    ...
    {
        clr = LLColor4(1.0f, 1.0f, 1.0f, 1.0f); // Bright white for recent packets
    }
    else
    {
        ...
        if (last_event < pip_max_time)
        {
            clr = LLColor4(0.4f, 1.0f, 0.4f, 1.0f); // Bright green for request
        }
        else
        {
            ...
            if (last_event < pip_max_time)
            {
                clr = LLColor4(1.0f, 1.0f, 0.3f, 1.0f); // Bright yellow for fetch
            }
        }
    }
    if (last_event < pip_max_time)
    ...
    if (mImagep->hasGLTexture())
    {
        ...
        if (last_event < 1.f)
        {
            clr = mImagep->getMissed() ? 
                LLColor4(1.0f, 0.2f, 0.2f, 1.0f) :  // Bright red for missed
...
void LLGLTexMemBar::draw()
{
   ...
   F64 render_bytes_alloc = LLRenderTarget::sBytesAllocated / 1024.0 / 512.0;
    ...
    LLColor4 header_color(0.5f, 0.9f, 1.0f, 1.0f); // Bright cyan for section headers
    LLColor4 value_color(0.7f, 1.0f, 0.7f, 1.0f);  // Light green for values
    LLColor4 warning_color(1.0f, 0.9f, 0.3f, 1.0f); // Yellow for warnings
    LLColor4 color;
...
void LLGLTexSizeBar::draw()
{
    ...
    LLColor4 value_color(0.7f, 1.0f, 0.7f, 1.0f);  // Light green for values
    LLColor4 warning_color(1.0f, 0.8f, 0.2f, 1.0f); // Yellow-orange for warnings
        std::string text;
...
//              ++debug_count; // for breakpoints
        ...
        static S32 max_count = 100; // S24: Doubled from 50 for 1080p+ displays with aggressive streaming
        S32 count = 0;
        mNumTextureBars = 0 ;
