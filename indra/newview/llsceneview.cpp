...
{
    S32 margin = 10;
    // S24: Smaller window size - 60% instead of 75%
    S32 height = (S32) (gViewerWindow->getWindowRectScaled().getHeight()*0.6f);
    S32 width = (S32) (gViewerWindow->getWindowRectScaled().getWidth() * 0.6f);
    ...
    for (U32 idx = 0; idx < 2; ++idx)
    {
        ...
        { //plot graph of visible/total triangles
            ...
            // S24: Color code by triangle complexity
            LLColor4 tri_color;
            F32 total_ktris = total_triangles[idx]/1024.f;
            if (total_ktris > 5000.0f) tri_color = LLColor4(1.0f, 0.3f, 0.3f, 1.0f); // Red - Very high
            else if (total_ktris > 2000.0f) tri_color = LLColor4(1.0f, 0.8f, 0.3f, 1.0f); // Yellow - High
            else if (total_ktris > 1000.0f) tri_color = LLColor4(1.0f, 1.0f, 0.6f, 1.0f); // Light yellow
            else tri_color = LLColor4(0.9f, 0.9f, 0.9f, 1.0f); // Normal
    ...
    for (U32 idx = 0; idx < 2; ++idx)
    {
        ...
        { //plot graph of streaming cost
            ...
            LLColor4 cost_color;
            if (total_streaming[idx] > 10000.0f) cost_color = LLColor4(1.0f, 0.3f, 0.3f, 1.0f); // Red - Extreme
            else if (total_streaming[idx] > 5000.0f) cost_color = LLColor4(1.0f, 0.8f, 0.3f, 1.0f); // Yellow - High
            else if (total_streaming[idx] > 2000.0f) cost_color = LLColor4(1.0f, 1.0f, 0.6f, 1.0f); // Light yellow
            else cost_color = LLColor4(0.9f, 0.9f, 0.9f, 1.0f); // Normal
    ...
    for (U32 idx = 0; idx < 2; ++idx)
    {
        ...
        { //plot graph of physics cost
            ...
            LLColor4 phys_color;
            if (total_physics[idx] > 1000.0f) phys_color = LLColor4(1.0f, 0.3f, 0.3f, 1.0f); // Red - Extreme
            else if (total_physics[idx] > 500.0f) phys_color = LLColor4(1.0f, 0.8f, 0.3f, 1.0f); // Yellow - High  
            else if (total_physics[idx] > 200.0f) phys_color = LLColor4(1.0f, 1.0f, 0.6f, 1.0f); // Light yellow
            else phys_color = LLColor4(0.9f, 0.9f, 0.9f, 1.0f); // Normal
