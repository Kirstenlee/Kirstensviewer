...
#include "llinventoryfunctions.h"       // for ROOT_KV_FOLDER
...
{
	// S24 Fix motion leak - Clean up accumulated old sets from previous timer-based clears - 2026-04-27
	// When NOT called from timer, it's safe to delete accumulated sets from previous timer calls
	// This prevents the motion/memory leak identified in FIRE-3801 workaround
	if (!fromTimer)
	{
		if (!mOldSets.empty())
		{
			LL_DEBUGS("AOEngine") << "S24: Cleaning up " << mOldSets.size() << " accumulated old AO sets (prevents motion leak)" << LL_ENDL;
			// S24 Modern range-based loop replaces std::for_each(DeletePointer()) - 2026-04-27
			for (AOSet* set : mOldSets)
			{
				delete set;
			}
			mOldSets.clear();
		...
		if (!mOldImportSets.empty())
		{
			LL_DEBUGS("AOEngine") << "S24: Cleaning up " << mOldImportSets.size() << " accumulated old import sets (prevents motion leak)" << LL_ENDL;
			// S24 Modern range-based loop replaces std::for_each(DeletePointer()) - 2026-04-27
			for (AOSet* set : mOldImportSets)
			{
				delete set;
			}
	...
	// Move current sets to old sets (will be cleaned on next non-timer clear)
...
void AOEngine::enable(bool enable)
{
	...
	// S24 Opportunistic cleanup of accumulated old sets - 2026-04-27
	// enable() is always called from UI (non-timer context), safe to clean here
	if (!mOldSets.empty() || !mOldImportSets.empty())
	{
		LL_DEBUGS("AOEngine") << "S24: Opportunistic cleanup during enable() - " 
							  << mOldSets.size() << " old sets, " 
							  << mOldImportSets.size() << " import sets" << LL_ENDL;
	...
	if (mEnabled)
	{
		if (state && !state->mAnimations.empty())
		{
			...
			{
				// S24: Changed to LL_DEBUGS - unhandled motions are normal (custom anims, viewer animations)
				// Not an error condition, just means AO doesn't override this specific motion
				// Potential gain: Eliminate console spam during mixed animation usage
				LL_DEBUGS("AOEngine") << "Unhandled last motion id " << gAnimLibrary.animationName(mLastMotion) << LL_ENDL;
			}
...
LLUUID AOEngine::override(const LLUUID& motion, bool start)
{
	...
	if (motion != ANIM_AGENT_TYPE)
	{
		...
		while ((stateNum = cleanupStates[index]) != AOSet::AOSTATES_MAX)
		{
			...
			if (AOSet::AOState* stateToCheck = mCurrentSet->getState(stateNum); stateToCheck != state)
			{
				...
				{
					// S24: Changed to LL_DEBUGS - state cleanup is normal during transitions, not an error
					// Fires frequently during movement/posture changes
					// Potential gain: Eliminate console I/O overhead in animation hot path
					LL_DEBUGS("AOEngine") << "cleaning up animation in state " << stateToCheck->mName << LL_ENDL;
	...
	else
	{
		...
		{
			// S24: Changed to LL_DEBUGS - motion mismatches are normal during rapid animation changes
			// Happens when user changes movement quickly (walk->run->stand transitions)
			// Potential gain: Eliminate console spam during active gameplay
			LL_DEBUGS("AOEngine") << "trying to stop-override motion " << gAnimLibrary.animationName(motion)
...
void AOEngine::cycleTimeout(const AOSet* set)
{
	...
	if (set != mCurrentSet)
	{
		// S24: Changed to LL_DEBUGS - set mismatches are normal during set switching
		// Timer callbacks can arrive during transitions, not an error condition
		// Potential gain: Eliminate console spam during AO set changes
		LL_DEBUGS("AOEngine") << "cycleTimeout for set " << set->getName() << " but current set is " << mCurrentSet->getName() << LL_ENDL;
		return;
	}
...
void AOEngine::tick()
{
	...
	const LLUUID categoryID = gInventory.findCategoryByName(ROOT_KV_FOLDER);

	if (categoryID.isNull())
	{
		LL_WARNS("AOEngine") << "no " << ROOT_KV_FOLDER << " folder yet. Creating ..." << LL_ENDL;
		gInventory.createNewCategory(gInventory.getRootFolderID(), LLFolderType::FT_NONE, ROOT_KV_FOLDER);
		mAOFolder.setNull();
	}
	else
	{
