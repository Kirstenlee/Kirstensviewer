...
// S24 STAGE 3: KVRAM Cache integration
...
void LLViewerTexture::updateClass()
{
    ...
    if (LLTexturePipelineTester* tester = (LLTexturePipelineTester*)LLMetricPerformanceTesterBasic::getTester(sTesterName)) [[unlikely]]
    {
        tester->update();
    }
...
void LLViewerFetchedTexture::loadFromFastCache()
{
    ...
    if(mRawImage.notNull())
    {
        ...
        if(mFullWidth > MAX_IMAGE_SIZE || mFullHeight > MAX_IMAGE_SIZE)
        {
            ...
            destroyRawImage();
            LL_INFOS() << "oversized, setting as missing" << LL_ENDL;
            setIsMissingAsset();
            mRawDiscardLevel = INVALID_DISCARD_LEVEL;
        }
        else
...
bool LLViewerFetchedTexture::preCreateTexture(S32 usename/*= 0*/)
{
    ...
    if (!size_okay)
    {
        ...
        LL_INFOS() << "!size_ok, setting as missing" << LL_ENDL;
        setIsMissingAsset();
        destroyRawImage();
        return false;
    }
...
bool LLViewerFetchedTexture::processFetchResults(S32& desired_discard, S32 current_discard, S32 fetch_discard, F32 decode_priority)
{
    ...
    if (mRawImage.notNull())
    {
            ...
            (current_discard < 0 || mRawDiscardLevel < current_discard))
        {
            ...
            if (mFullWidth > MAX_IMAGE_SIZE || mFullHeight > MAX_IMAGE_SIZE)
            {
                //discard all oversized textures.
                destroyRawImage();
                LL_INFOS() << "oversize, setting as missing" << LL_ENDL;
                setIsMissingAsset();
                mRawDiscardLevel = INVALID_DISCARD_LEVEL;
                mIsFetching = false;
    ...
    if (!mIsFetching)
    {
            ...
            && mFetchState > 1) // 1 - initial, make sure fetcher did at least something
        {
            ...
            {
                //LL_INFOS() << mID << ": Setting min discard to " << current_discard << LL_ENDL;
