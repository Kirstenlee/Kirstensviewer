...
public:
	...
	// #5607: Force render refresh to work around CEF black screen after resize
	...
	void initEqualizer();
	void cleanupEqualizer();
	void updateEqualizer(bool enabled, float preamp, const float bands[10]);

	// S24 - Streaming audio metadata accessors
	std::string getMetadataTitle() const { return mMetadataTitle; }
	std::string getMetadataArtist() const { return mMetadataArtist; }
...
protected:
	...
	// S24 - Streaming audio metadata
	std::string		mMetadataTitle;
	std::string		mMetadataArtist;
