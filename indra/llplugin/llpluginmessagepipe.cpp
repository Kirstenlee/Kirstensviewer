...
{
	if (LLApp::instance()->isExiting())
	{
		if (mOutput.empty())
			return true; // App exiting, nothing left to write — don't poll input either.
	}
...
{
	if (LLApp::instance()->isExiting() && mOutput.empty())
		return true;
	...
	if(mSocket)
	{
		...
		if(result)
		{
			if(timeout != 0.0f)
			{
				if (LLApp::instance()->isExiting())
				{
					LL_INFOS("PluginSocket") << "Skip sleep: viewer quitting" << LL_ENDL;
					return false;
				}

				S32 sleep_ms = llclamp((S32)(timeout * 1000.0f), 1, 250);  // Cap at quarter second
				ms_sleep(sleep_ms);
				timeout = 0.0f;
			}
		}
...
//              LL_INFOS("Plugin") << "after apr_socket_recv, size = " << size << LL_ENDL;
				...
				{    // S24 this is hit hard and returns 0 for a massive console spam - not useful!
