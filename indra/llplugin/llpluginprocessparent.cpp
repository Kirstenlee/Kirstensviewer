	...
	do
	{
		...
		switch(mState)
		{
			...
			case STATE_INITIALIZED:
			{
                if (LLApp::isExiting())
                {
                    LL_WARNS("PluginTeardown") << "Abort socket init: viewer exiting" << LL_ENDL;
                    killSockets();
                    errorState();
                    break;
                }
            ...
            case STATE_LISTENING:
                ...
                if (LLApp::isExiting())
                {
                    LL_WARNS("PluginTeardown") << "Suppress plugin launch: viewer exiting during LISTENING" << LL_ENDL;
                    errorState();
                    break;
                }
            ...
            case STATE_LAUNCHED:
                if (LLApp::isExiting())
                {
                    LL_WARNS("PluginTeardown") << "Aborting plugin connection wait: viewer exiting during LAUNCHED" << LL_ENDL;
                    errorState();
                    break;
                }
            ...
            case STATE_CONNECTED:
                ...
                if (LLApp::isExiting())
                {
                    LL_WARNS("PluginTeardown") << "Aborting hello wait: viewer exiting during CONNECTED" << LL_ENDL;
                    errorState();
                    break;
                }
            ...
            case STATE_HELLO:
                ...
                if (LLApp::isExiting())
                {
                    LL_WARNS("PluginTeardown") << "Suppressing plugin load: viewer exiting during HELLO" << LL_ENDL;
                    errorState();
                    break;
                }
            ...
            case STATE_LOADING:
                ...
                if (LLApp::isExiting())
                {
                    LL_WARNS("PluginTeardown") << "Aborting plugin wait: viewer exiting during LOADING" << LL_ENDL;
                    errorState();
                    break;
                }
            ...
            case STATE_RUNNING:
                ...
                if (LLApp::isExiting())
                {
                    LL_WARNS("PluginTeardown") << "Viewer exiting during STATE_RUNNING—skipping plugin monitoring" << LL_ENDL;
                    errorState();
                    break;
                }
                if(pluginLockedUpOrQuit())
...
{
    // Early out: read thread unavailable or quitting
    if (!sUseReadThread || !sReadThread || sReadThread->isQuitting())
        return;

    // Early out: no instances mutex
    if (!sInstancesMutex)
        return;

    LLCoros::LockType lock(*sInstancesMutex);

    // Count pollable instances
    int pollable_count = 0;
    for (auto& [id, instance] : sInstances)
    {
        instance->mPolledInput = false;
        if (instance->wantsPolling())
            ++pollable_count;
    }

    // Early out: no pollable instances
    if (pollable_count == 0)
    {
        if (sPollSet)
        {
    ...
    // Rebuild only if pollset is missing
    if (!sPollSet)
    {
        apr_status_t status = apr_pollset_create(&sPollSet, pollable_count, gAPRPoolp, APR_POLLSET_NOCOPY);
        if (status != APR_SUCCESS)
        {
            sPollSet = nullptr;
            LL_WARNS("PluginPoll") << "Pollset creation failed, disabling polling" << LL_ENDL;
            return;
        }

        // Add instances to pollset
        for (auto& [id, instance] : sInstances)
        {
            if (instance->wantsPolling())
            {
                status = apr_pollset_add(sPollSet, &instance->mPollFD);
                if (status == APR_SUCCESS)
                {
                    instance->mPolledInput = true;
                }
                else
                {
                    LL_WARNS("PluginPoll") << "Pollset add failed for instance " << id << " (status " << status << ")" << LL_ENDL;
                }
            }
        }
    }
...
void LLPluginProcessParent::poll(F64 timeout)
{
    ...
    if (sPollsetNeedsRebuild)
    {
        sPollsetNeedsRebuild = false;
        updatePollset(); // existing function
    }
    ...
    apr_int32_t count = 0;
    const apr_pollfd_t* descriptors = nullptr;
    apr_status_t status = apr_pollset_poll(sPollSet, static_cast<apr_interval_time_t>(timeout * 1e6), &count, &descriptors);
    ...
    if (status == APR_SUCCESS)
    {
        for (int i = 0; i < count; ++i)
        {
            ...
            {
                LLCoros::LockType lock(*sInstancesMutex);
                mapInstances_t::iterator it = sInstances.find(thatId);
                if (it != sInstances.end())
                    that = it->second;
            }
    ...
    {
        // Descriptor likely closed — flag for rebuild
        sPollsetNeedsRebuild = true;
    }
    else
    {
        ...
        if (failCount == 1 || failCount % 500 == 0)
        {
            LL_WARNS("PluginPoll") << "apr_pollset_poll failed, status=" << status << ", count=" << failCount << LL_ENDL;
        }

        if (failCount > 5000)
        {
            LL_WARNS("PluginPoll") << "Excessive poll failures — triggering pollset rebuild" << LL_ENDL;
            dirtyPollSet(); // existing function, safely flags rebuild and unpauses thread
            return;
        }
    }
    ...
    {
        LLCoros::LockType lock(*sInstancesMutex);
        for (mapInstances_t::iterator it = sInstances.begin(); it != sInstances.end(); )
        {
            if (it->second->isDone())
...
void LLPluginProcessParent::servicePoll()
{
    if (!sUseReadThread || !sReadThread || sReadThread->isQuitting())
    {
        LL_WARNS("PluginPoll") << "ServicePoll aborted: read thread quitting" << LL_ENDL;
        return; // result remains untouched, and no descriptor changes happen
    }
