...
{
	// #5607: Force layout recalculation by briefly hiding/showing the web content
	// Used to clear black screen issues after resize
...
void LLPluginClassMedia::receivePluginMessage(const LLPluginMessage &message)
{
	...
	if(message_class == LLPLUGIN_MESSAGE_CLASS_MEDIA)
	{
		...
		// S24 - Streaming audio metadata from VLC plugin
		else if(message_name == "metadata_update")
		{
			mMetadataTitle = message.getValue("title");
			mMetadataArtist = message.getValue("artist");
			LL_INFOS("MediaPlugins") << "Received metadata - Title: " << mMetadataTitle << ", Artist: " << mMetadataArtist << LL_ENDL;
		}
...
//-----------------------------------------------------------------------
// S24 - Audio Equalizer Support
//-----------------------------------------------------------------------
void LLPluginClassMedia::initEqualizer()
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_MEDIA, "eq_init");
	sendMessage(message);
}

void LLPluginClassMedia::cleanupEqualizer()
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_MEDIA, "eq_cleanup");
	sendMessage(message);
}

void LLPluginClassMedia::updateEqualizer(bool enabled, float preamp, const float bands[10])
{
	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_MEDIA, "eq_update");
	message.setValueBoolean("enabled", enabled);
	message.setValueReal("preamp", preamp);

	// Pack 10 bands into LLSD array
	LLSD band_array;
	for (int i = 0; i < 10; ++i)
	{
		band_array.append(LLSD::Real(bands[i]));
	}
	message.setValueLLSD("bands", band_array);
