...
#ifndef OPJ_STATIC
#define OPJ_STATIC  // Force static linkage BEFORE any headers
#endif
...
// ============================================================================
// Embedded OpenJPEG Integration (Default Mode)
// ============================================================================
// This implementation uses embedded, optimized OpenJPEG code compiled directly
// into llimagej2coj.lib. No external DLL is required.
//
// IMPORTANT: Embedded OpenJPEG is MORE STRICT than the old vcpkg DLL
// --------------------------------------------------------------------
// This exposes a pre-existing issue with RAM texture cache containing partial/
// truncated J2K data from interrupted downloads. The old DLL was more lenient
// and would attempt to decode partial data. The embedded version correctly 
// fails with "Stream too short" errors.
//
// MITIGATION: Dual opj_decoder_set_strict_mode(OPJ_FALSE) calls reduce false
// failures, but truly incomplete streams will still fail (correct behavior).
//
// USER WORKAROUND: Periodically clear RAM cache if textures don't appear after
// teleports (Advanced > Performance Tools > Clear Caches > RAM Cache).
// ============================================================================

#include "openjpeg/opj_config.h"  // Configuration
#include "openjpeg/sl_optimized/sl_config.h"  // SL-specific optimizations
#include "openjpeg/sl_optimized/sl_types.h"
#include "openjpeg/core/openjpeg.h"  // Embedded OpenJPEG API (from openjpeg/core/openjpeg.h)

#pragma message("=== BUILDING WITH EMBEDDED OPENJPEG (SL-OPTIMIZED) ===")

// ============================================================================
// OpenJPEG Threading - Conservative Tuning for Large Textures
// ============================================================================
// STRATEGY: Only use OpenJPEG internal threading for LARGE textures (1024x1024+)
// where wavelet decode parallelism provides real benefit.
// 
// RATIONALE:
//   - Small/medium textures (<=512): Single-threaded (threading overhead > benefit)
//   - Large textures (1024+): 2-3 threads for wavelet parallelism
//   - Very conservative to avoid over-subscription with viewer's ImageDecode pool
//
// MATH: 
//   - Viewer: 32 ImageDecode threads handling different textures
//   - Typically only 3-5 large textures decoding simultaneously 
//   - 5 large textures × 2 threads = 10 extra threads
//   - Total: 32 + 10 = 42 threads on 16-core system = reasonable
// ============================================================================
static int opj_get_optimal_threads_for_large_textures(U32 width, U32 height)
{
    // Only enable threading for genuinely large textures where it helps
    // 1024x1024 and above benefit from parallel wavelet decode
    if (width >= 1024 && height >= 1024)
    {
        // Conservative: use 2 threads to avoid over-subscription
        // Testing showed diminishing returns beyond 2 threads for JPEG2000
        return 2;
    }

    // All other textures: single-threaded (fast path, no overhead)
    return 0;  // 0 = disable OpenJPEG threading
}
...
{
    return std::string("KV:" OPENJPEG_VERSION);
}
...
{
    // Error logging can be enabled for debugging by uncommenting below
    // Note: "Stream too short" errors are expected for incomplete cached textures
    // and are handled gracefully by the decoder's non-strict mode
    #if WANT_VERBOSE_OPJ_SPAM
    LL_WARNS("OpenJPEG") << "OpenJPEG ERROR: " << chomp(msg) << LL_ENDL;
    #endif
}
/**
...
{
#if WANT_VERBOSE_OPJ_SPAM
    LL_WARNS("OpenJPEG") << "OpenJPEG WARNING: " << chomp(msg) << LL_ENDL;
#endif
}
/**
...
public:
        ...
        S32& discard_level)
    {
        ...
        // Clean up any existing decoder/stream/image before creating new ones
        if (decoder)
        {
            opj_destroy_codec(decoder);
            decoder = nullptr;
    ...
    bool decode(U8* data, U32 dataSize, U32* channels, U8 discard_level)
    {
        ...
        // Enable decoding of partially loaded/truncated images BEFORE setup
        // This must be set early to initialize internal state correctly
        opj_decoder_set_strict_mode(decoder, OPJ_FALSE);
        ...
        // Redundant strict mode setting here as well (belt and suspenders)
        // to ensure partial/truncated textures can decode
        opj_decoder_set_strict_mode(decoder, OPJ_FALSE);
        ...
        // Conservative threading: ONLY enable for large textures (1024x1024+)
        // Small/medium textures stay single-threaded (fast path, no overhead)
        // Note: opj_codec_set_threads() failure is non-fatal - decoder continues single-threaded
        // TEMPORARILY DISABLED FOR TESTING - isolate fetch vs decode issue
        /*
        if (image && image->x1 >= 1024 && image->y1 >= 1024)
        {
            int num_threads = opj_get_optimal_threads_for_large_textures(image->x1, image->y1);
            if (num_threads > 0)
            {
                // If thread setup fails, decoder automatically falls back to single-threaded
...
bool LLImageJ2COJ::decodeImpl(LLImageJ2C &base, LLImageRaw &raw_image, F32 decode_time, S32 first_channel, S32 max_channel_count)
{
    ...
    if (!decoded)
    {
        // Decode failed - typically due to incomplete/truncated stream data in cache
        // 
        // ROOT CAUSE: RAM texture cache can contain partial/truncated J2K data from 
        // interrupted downloads. The embedded OpenJPEG is stricter than the old vcpkg 
        // DLL and correctly fails on "Stream too short, expected SOT" errors.
        //
        // SYMPTOMS: Invisible textures after teleport that appear after cache clear
        //
        // WORKAROUND: Users should periodically clear RAM cache (Advanced > Performance 
        // Tools > Clear Caches > RAM Cache) if textures fail to appear after teleports.
        //
        // FUTURE FIX: Need cache invalidation mechanism to automatically discard bad 
        // entries and re-fetch from server. This requires upstream changes to texture 
        // fetch/cache system to detect "incomplete data" decode failures vs "corrupt data".
        ...
        return true; // done (decode attempt complete, even though it failed)
    }
