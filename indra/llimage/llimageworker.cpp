...
#include <thread>  // S24: For std::thread::hardware_concurrency()
...
LLImageDecodeThread::LLImageDecodeThread(bool /*threaded*/)
	: mDecodeCount(0)
{
	// S24 PERFORMANCE: Texture decode thread count respects KVTweaks core settings
	// The thread count is determined by LLAppViewer initialization using KVmax_cores, 
	// KVmin_cores, and KVdefault_cores settings, then stored in ThreadPoolSizes["ImageDecode"]
	// If no override is set, falls back to a conservative default based on hardware

	unsigned int hw_cores = std::thread::hardware_concurrency();
	unsigned int default_threads = 8; // Standard fallback if detection fails

	if (hw_cores > 0)
	{
		// Aggressive formula: use 8 - 32 threads depending on cores
		// Reserve 4 cores for main thread, render, and HTTP workers
		// Water priority and network throttle fixes allow higher decode concurrency
		unsigned int available_cores = (hw_cores > 4) ? (hw_cores - 4) : hw_cores;
		default_threads = std::max(8u, std::min(32u, available_cores));
	}
	else
	{
		// Fallback to aggressive default if hardware detection fails
		default_threads = 16;
	}

	// ThreadPool constructor automatically checks ThreadPoolSizes["ImageDecode"] setting
	// and uses that value if present, otherwise uses default_threads as fallback
	mThreadPool = std::make_unique<LL::ThreadPool>("ImageDecode", default_threads);
	mThreadPool->start();
