...
protected:
	...
	// S24 we are after a U16 but APR read deals with S32's so DO NOT use U16's in the bitshift as this cause's Major fail KL
	U16 read_short()
	{
		char p[2];
		mInfile.read(&p[0],2);
	...
	// S24 just for completeness include a reverse read :)
	U16 read_reversed_short()
	{
		char p[2];
		mInfile.read(&p[0],2);
		S32 temp = (static_cast<S32>(p[1])      & 0x000000FF) |
				   ((static_cast<S32>(p[0]) << 8) & 0x0000FF00);
		return temp;
	}
