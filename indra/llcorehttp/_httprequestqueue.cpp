...
namespace LLCore
{
	...
	HttpRequestQueue::opPtr_t HttpRequestQueue::fetchOp(bool wait)
	{
		...
		{
			mQueueCV.wait(lock, [this]() { return !mQueue.empty() || mQueueStopped; });
		}

		if (mQueue.empty() || mQueueStopped)
		{
			return HttpOperation::ptr_t();
		}

		HttpOperation::ptr_t result = mQueue.front();
