...
void HttpPolicy::retryOp(const HttpOpRequest::ptr_t &op)
{
	...
	// S24 - Removed debug and trace logging from retry scheduling
	// Eliminated: LL_DEBUGS with delta calculation/formatting, nested tracing conditional,
	// LL_INFOS with handle/delta/retry count string construction.
	// Gains: Removes ~30-60 cycles per retry operation. Debug macros evaluate even when
	// disabled, string formatting is expensive.累積 savings in retry-heavy scenarios.
		...
		if (needed > 0)
		{
			...
			while (needed > 0 && ! retryq.empty())
			{
				...
				retryq.pop();
					...
					if (throttle_enabled)
					{
						...
						{
							// S24 - Removed throttle window refresh debug logging (retry queue path)
							// Eliminated: LL_DEBUGS with state variable formatting
							// Gains: ~20-40 cycles saved per throttle window refresh in retry path
			...
			while (needed > 0 && ! readyq.empty())
			{
				...
				if (throttle_enabled)
				{
					if (now >= state.mThrottleEnd)
					{
						// S24 - Removed throttle window refresh debug logging (ready queue path)
						// Eliminated: LL_DEBUGS with state variable formatting
						// Gains: ~20-40 cycles saved per throttle window refresh in ready path
						state.mThrottleLeft = state.mOptions.mThrottleRate;
						state.mThrottleEnd = now + HttpTime(1000000);
					}
					if (--state.mThrottleLeft <= 0)
...
bool HttpPolicy::stageAfterCompletion(const HttpOpRequest::ptr_t &op)
{
	...
	if (! op->mStatus)
	{
		// S24: Changed to LL_DEBUGS for all HTTP status codes - these are server responses, not viewer errors
		// HTTP 4xx (404, 403, etc.) occur normally during: region crossings, offline servers, missing content
		// HTTP 5xx (503, 500, etc.) are server errors, not viewer failures
		// KL "the only thing the end user cares for is connect and disconnect"
		// End users don't need HTTP protocol details - only connection state matters
		// A 404 could be: region crossing, offline server, deleted asset, cache miss - all NORMAL states
		// Potential gain: Eliminate HTTP status spam from region navigation and normal network operations
		// UX: Users care about "connected" vs "disconnected", not retry counts or status codes
		// Privacy: HTTP URLs may contain UUIDs or other PII, debug-only logging reduces exposure
		LL_DEBUGS(LOG_CORE) << "HTTP request " << op->getHandle()
							<< " completed with status " << op->mStatus.toTerseString()
							<< " after " << op->mPolicyRetries << " retries"
							<< " - Reason: " << op->mStatus.toString()
							<< LL_ENDL;
	}
