...
LLAvatarAppearance::LLAvatarXmlInfo::~LLAvatarXmlInfo()
{
    ...
    for (LLViewerVisualParamInfo* distortion : mSkeletalDistortionInfoList)
    {
        delete distortion;
    }
    mSkeletalDistortionInfoList.clear();

    for (LLAvatarAttachmentInfo* attachment : mAttachmentInfoList)
    {
        delete attachment;
    }
    ...
    for (LLTexLayerSetInfo* layer : mLayerInfoList)
    {
        delete layer;
    }
    mLayerInfoList.clear();

    for (LLDriverParamInfo* driver : mDriverInfoList)
    {
        delete driver;
    }
    mDriverInfoList.clear();

    for (LLAvatarMorphInfo* morph : mMorphMaskInfoList)
    {
        delete morph;
    }
    ...
    mWearableData(wearable_data)
{
    ...
    for (auto& baked_data : mBakedTextureDatas)
    {
        ...
        baked_data.mTextureIndex = sAvatarDictionary->bakedToLocalTextureIndex((LLAvatarAppearanceDefines::EBakedTextureIndex)i);
        ++i;
    }
}
...
//  LL_RECORD_BLOCK_TIME(FTM_LOAD_AVATAR);
    ...
    for (LLDriverParamInfo* info : sAvatarXmlInfo->mDriverInfoList)
    {
        ...
        if (driver_param->setInfo(info))
        {
            ...
            auto avatar_lambda = [this](S32 id) -> LLVisualParam*
                {
                    return getVisualParam(id);
                };

            if (!driver_param->linkDrivenParams(avatar_lambda, false))
            {
...
void LLAvatarAppearance::makeJointAliases(LLAvatarBoneInfo* bone_info)
{
    ...
    while (ss >> alias)
    {
        ...
        if (!inserted)
        {
            LL_WARNS() << "avatar skeleton:  Joint alias \"" << alias
                << "\" remapped from " << iter->second
                << " to " << bone_name << LL_ENDL;
            iter->second = bone_name;  // Update existing
        }
    }
