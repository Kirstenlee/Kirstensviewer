...
void LLWearable::createVisualParams(LLAvatarAppearance* avatarp)
{
    ...
    for (visual_param_index_map_t::value_type& param_pair : mVisualParamIndexMap)
    {
        ...
        auto wearable_lambda = [this](S32 id) -> LLVisualParam*
            {
                return this->getVisualParam(id);
            };

        auto avatar_lambda = [avatarp](S32 id) -> LLVisualParam*
            {
                return avatarp->getVisualParam(id);
            };

        param->resetDrivenParams();
        if (!param->linkDrivenParams(wearable_lambda, false))
        {
            if (!param->linkDrivenParams(avatar_lambda, true))
            {
