...
namespace LLInitParam
{
	...
	void Parser::parserWarning(const std::string& message)
	{
		...
		// S24: Changed to LL_DEBUGS - parserWarning fires for normal XML variations (missing optional params, defaults, type conversions)
		// Called from submitValue when parameters don't parse (line 196) - extremely common during XUI loading
		// Potential gain: Eliminate 100+ console writes during UI initialization, ~500ms I/O blocking during startup
		LL_DEBUGS("XUIParse") << message << LL_ENDL;
	}
	...
	bool BaseBlock::validateBlock(bool emit_errors) const
	{
		...
		for (const BlockDescriptor::param_validation_list_t::value_type& pair : block_data.mValidationList)
		{
			...
			if (!pair.second(param))
			{
				if (emit_errors)
				{
					// S24: Changed to LL_DEBUGS - validation failures often occur for optional params with defaults
					// Called during block validation when param values don't meet validation rules (common for UI widgets)
					// Potential gain: Eliminate spam during skin/XUI changes, ~50-100 console writes per UI initialization
					LL_DEBUGS("XUIParse") << "Invalid param \"" << getParamName(block_data, param) << "\"" << LL_ENDL;
				}
				return false;
			}
		}
