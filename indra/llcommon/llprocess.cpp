...
#include "llexception.h"
 ...
 // S24 for APR autokill behavior. See LLProcess::Params::autokill.
#include <windows.h>
#include <mutex>
...
public:
	...
	bool tick(const LLSD&)
	{
		...
		if (total)
		{
			...
			{
				// Modern Boost.Asio: const_buffer has data() and size() methods
				// instead of free functions buffer_cast and buffer_size
				const boost::asio::const_buffer& buffer = *bufi;
				const char* remainptr = static_cast<const char*>(buffer.data());
				std::size_t remainlen = buffer.size();
				while (remainlen)
				{
	...
	bool tick(const LLSD&)
	{
		...
		do
		{
			...
			for (auto bufi = boost::asio::buffer_sequence_begin(bufs),
				...
				bufi != bufend; ++bufi)
			{
				// Modern Boost.Asio: mutable_buffer has data() and size() methods
				// instead of free functions buffer_cast and buffer_size
				const boost::asio::mutable_buffer& buffer = *bufi;
				std::size_t toread(buffer.size());
	...
	mPool(NULL)
{
...
#if defined(APR_HAS_PROCATTR_CONSTRAIN_HANDLE_SET) && APR_HAS_PROCATTR_CONSTRAIN_HANDLE_SET
	// Compile-time-detected extension present: use it directly.
	chkapr(apr_procattr_constrain_handle_set(procattr, 1));
#else
	// No compile-time Linden APR support for apr_procattr_constrain_handle_set.
	// Because APR is statically linked into the EXE in our build, runtime
	// probing (GetModuleHandle/GetProcAddress) is not useful. Use a safe,
	// conservative Windows fallback: clear HANDLE_FLAG_INHERIT on the standard
	// handles so common console-handle inheritance problems are avoided.
	// S24 : Not useful we use vcpkg APR which is not patched with apr_procattr_constrain_handle_set, but we can still apply the conservative Windows fallback.
	LL_DEBUGS("LLProcess") << "apr_procattr_constrain_handle_set unavailable; applying conservative Windows fallback" << LL_ENDL;
	...
	for (HANDLE h : std_handles)
	{
		...
		{
			// Best-effort; warn only if it fails.
			if (!SetHandleInformation(h, HANDLE_FLAG_INHERIT, 0))
			{
				// Not fatal; avoid console spam. Leave detailed diagnosis to debug logs.
				LL_DEBUGS("LLProcess") << "SetHandleInformation() failed for std handle" << LL_ENDL;
			}
		}
	}
#endif
...
// S24 : On Windows, if autokill is requested, we'll put the child process in a Job Object
	if (mAutokill)
	{
#if defined(APR_HAS_PROCATTR_AUTOKILL_SET) && APR_HAS_PROCATTR_AUTOKILL_SET
		// Compile-time-detected extension present: use it directly.
		ll_apr_warn_status(apr_procattr_autokill_set(procattr, 1));
#else
		// No compile-time Linden APR autokill support; will emulate autokill on Windows
		// after creating the process (LLProcess::autokill()).
#endif
	}
	...
	LL_DEBUGS("LLProcess") << mDesc << ": launched " << params << LL_ENDL;
...
bool LLProcess::kill(const std::string& who)
{
	if (isRunning())
	{
		...
		// S24: Don't spam logs with "Access Denied" during shutdown - process may already be dead
		// apr_proc_kill() often fails harmlessly during cleanup, no need to warn about it
		apr_proc_kill(&mProcess, sig);
	}
...
/*****************************************************************************
*  S24 Handle autokill on Windows by associating child processes with a Job Object that has
*  the JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE flag set. This ensures that all child processes
*  are terminated when the parent process exits.
*****************************************************************************/

static HANDLE sAutokillJob = NULL;
static std::mutex sAutokillJobMutex;

static HANDLE get_autokill_job()
{
	std::lock_guard<std::mutex> lock(sAutokillJobMutex);
	if (sAutokillJob)
		return sAutokillJob;

	sAutokillJob = CreateJobObjectA(NULL, NULL);
	if (!sAutokillJob)
	{
		// real failure worth warning
		LL_WARNS("LLProcess") << "CreateJobObject() failed" << LL_ENDL;
		sAutokillJob = NULL;
		return NULL;
	}
	...
	if (!SetInformationJobObject(sAutokillJob, JobObjectExtendedLimitInformation, &jeli, sizeof(jeli)))
	{
		// real failure worth warning
		LL_WARNS("LLProcess") << "SetInformationJobObject() failed" << LL_ENDL;
		CloseHandle(sAutokillJob);
		sAutokillJob = NULL;
		return NULL;
	}
...
{
	// Ensure we have a job to assign the child to.
	HANDLE job = get_autokill_job();
	if (!job)
		return;

	// APR stores the native process handle in mProcess.hproc; treat it as a
	// Windows HANDLE. If it's null, nothing to do.
	HANDLE proc_handle = reinterpret_cast<HANDLE>(mProcess.hproc);
	if (!proc_handle)
	{
		// Possibly transient; don't spam console
		LL_DEBUGS("LLProcess") << "autokill(): no process handle available to assign to job" << LL_ENDL;
		return;
	}

	// Duplicate the handle into one we control with sufficient access.
	HANDLE dup = NULL;
	if (!DuplicateHandle(GetCurrentProcess(), proc_handle,
		GetCurrentProcess(), &dup,
		PROCESS_CREATE_PROCESS | PROCESS_QUERY_INFORMATION | PROCESS_TERMINATE | SYNCHRONIZE,
		FALSE, 0))
	{
		// If duplication fails, fall back to trying the original handle.
		dup = proc_handle;
	}

	// Try assigning to the job. If the process is already in a job, this may
	// fail (ERROR_ACCESS_DENIED); that's usually benign for autokill semantics.
	if (!AssignProcessToJobObject(job, dup))
	{
		DWORD err = GetLastError();
		if (err == ERROR_ACCESS_DENIED)
		{
			// Likely already in a job — benign for autokill semantics; debug-level only.
			LL_DEBUGS("LLProcess") << "AssignProcessToJobObject(): process is already in a job (ignored)" << LL_ENDL;
		}
		else
		{
			// Unexpected failure is worth warning once per occurrence.
			LL_WARNS("LLProcess") << "AssignProcessToJobObject() failed" << LL_ENDL;
		}
	}

	// If we duplicated, close the duplicate handle. If dup == proc_handle we must
	// not close it (APR owns it).
	if (dup && dup != proc_handle)
	{
		CloseHandle(dup);
	}
}
...
static LLProcess::Status interpret_status(int status)
{
	...
	// Copied from APR/Windows behavior: high-word 0xC000 indicates an exception
	// (access violation etc.) which we treat as KILLED; otherwise the child
	// exited normally and we treat as EXITED.
...
// S24: end of LLProcess implementation