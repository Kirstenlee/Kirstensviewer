...
{
    // Warning: Calling the thread destructor from within the thread itself
    // will result in undefined behavior and resource leaks.
    if (!mThreadp) return;
    ...
    if (!isStopped())
    {
        // Thread failed to stop within the timeout
        // Force terminate the thread as a last resort
        // WARNING: TerminateThread is dangerous and should only be used
        // when the thread is truly hung. It does NOT:
        // - Run C++ destructors for stack objects
        // - Release synchronization objects (mutexes, semaphores)
        // - Free thread-local storage
        // - Notify attached DLLs
        // This can leave the process in an inconsistent state.
        if (mNativeHandle)
        {
            TerminateThread(mNativeHandle, 0);
        }

        // Manually clean up resources since the thread was forcibly terminated
        delete mRecorder;
        mRecorder = nullptr;
        mStatus = STOPPED;
    ...
    // Thread stopped gracefully - clean up resources
    delete mThreadp;
    mThreadp = nullptr;
