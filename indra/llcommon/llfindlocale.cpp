...
{
    if (!l || !l->lang || !l->country) return;

    struct {
        const char* lang;
        const char* bad_country;
        const char* good_country;
    } fixes[] = {
        { "en", "UK", "GB" },
        { "ja", "JA", "JP" }
    };
    ...
    {
        if (strcmp(l->lang, fixes[i].lang) == 0 &&
            strcmp(l->country, fixes[i].bad_country) == 0)
        {
            free((void*)l->country);
            l->country = _strdup(fixes[i].good_country);
            break; // only one fix applies
        }
    }
}
