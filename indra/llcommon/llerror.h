...
/** Error Logging Facility
	...
	Lastly, logging is now very efficient (yeah right) in both compiled code and execution
...
namespace LLError
{
	...
	*/
    ...
    public:
        ...
        // title, message and error code to include in error marker file
...
// Set to 1 to disable LL_INFOS logging (reduces log noise and overhead)
#define LL_DISABLE_INFOS 1

#if LL_DISABLE_INFOS
// No-op implementation that compiles away cleanly
// This creates the same structure as lllog but with a constant false condition
#define LL_INFOS(...) \
	do { \
		if (false) { \
			static LLError::CallSite _site(LLError::LEVEL_INFO, __FILE__, __LINE__, \
				typeid(_LL_CLASS_TO_LOG), __FUNCTION__, false, nullptr, 0); \
			std::ostringstream _out; \
			_out
#else
// Active implementation for INFO level logging
#define LL_INFOS(...)	lllog(LLError::LEVEL_INFO, false, ##__VA_ARGS__)
#endif // LL_DISABLE_INFOS
