    ...
    std::string& value) const
{
    ...
    // Read the 4‑byte length safely
    if (!read(istr, reinterpret_cast<char*>(&value_nbo), sizeof(U32)))
        return false;

    S32 size = static_cast<S32>(ntohl(value_nbo));

    // Reject negative sizes (malformed or malicious input)
    if (size < 0)
        return false;

    // Enforce byte‑limit constraints
    if (mCheckLimits && size > mMaxBytesLeft)
        return false;

    // Zero‑length string: must clear the output
    if (size == 0)
    {
        value.clear();
        return true;
    ...
    // Read exactly 'size' bytes; fail if truncated
    const S32 bytes_read = fullread(istr, value.data(), size);
    if (bytes_read != size)
        return false;
	...
	case LLSD::TypeBinary:
	{
		...
		if (options & LLSDFormatter::OPTIONS_PRETTY_BINARY)
		{
			...
			if (!buffer.empty())
			{
				...
				// S24 Use a temporary buffer to avoid repeated stream operations
				std::string hex_output;
				hex_output.reserve(buffer.size() * 2); // Each byte → 2 hex chars

				for (U8 byte : buffer)
				{
					hex_output.push_back(hex_digits[(byte >> 4) & 0xF]);
					hex_output.push_back(hex_digits[byte & 0xF]);
				}
...
{
    llssize count = 0;
    static constexpr S32 BUF_LEN = 20;
    char buf[BUF_LEN]; // "(1234567890" fits easily

    // Read up to ')' or until buffer is full - 1
    istr.get(buf, BUF_LEN - 1, ')');
    const std::streamsize header_len = istr.gcount();
    count += header_len;

    // If we failed to read anything, or stream is bad, bail
    if (header_len <= 0 || !istr.good())
        return LLSDParser::PARSE_FAILURE;

    // We now expect ')' and then opening quote
    const int closing_paren = istr.get();
    const int quote_char    = istr.get();
    count += 2;

    if (closing_paren != ')' || (quote_char != '"' && quote_char != '\''))
        return LLSDParser::PARSE_FAILURE;
    ...
    // Parse length from buf+1, ensure it’s all digits and non-negative
    char* endptr = nullptr;
    errno = 0;
    const long len_long = std::strtol(buf + 1, &endptr, 10);

    // No digits, overflow/underflow, or trailing junk before '\0'
    if (endptr == buf + 1 || errno != 0 || *endptr != '\0' || len_long < 0)
        return LLSDParser::PARSE_FAILURE;

    const llssize len = static_cast<llssize>(len_long);

    // Enforce max_bytes limit if set
    ...
    if (len > 0)
    {
        ...
        const llssize bytes_read = fullread(istr, value.data(), len);
        count += bytes_read;
    ...
    {
        // Zero-length: must clear
        value.clear();
    }

    // Expect closing quote matching the opening one
    const int closing_quote = istr.get();
    ++count;
