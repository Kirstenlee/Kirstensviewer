...
{
    // S24 Boost replace: converted boost::bind to C++ lambda for listener setup
    LLEventPump& pump = LLEventPumps::instance().obtain(pumpname);
    mListener = [&pump](const LLSD& event) {
        return pump.post(event);
        };
}
...
{
    // S24 Boost replace: converted boost::bind to C++ lambda for listener setup (const char* overload)
    LLEventPump& pump = LLEventPumps::instance().obtain(pumpname);
    mListener = [&pump](const LLSD& event) {
        return pump.post(event);
        };
