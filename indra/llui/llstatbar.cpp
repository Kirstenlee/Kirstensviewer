...
// Disable C4756 for calc_tick_value - false positive from LTCG optimizer
// We clamp exponents to safe ranges but optimizer doesn't see it
#pragma warning(push)
...
F32 calc_tick_value(F32 min, F32 max)
{
    ...
    for (U32 divisor_idx = 0; divisor_idx < LL_ARRAY_SIZE(DIVISORS); divisor_idx++)
    {
        ...
        for (S32 digit_count = -(num_whole_digits - 1); digit_count < 6; digit_count++)
        {
            ...
            S32 safe_digit_count = llclamp(digit_count, -38, 38);
            F32 test_tick_value = min + (possible_tick_value * powf(10.0f, (F32)safe_digit_count));
...
// Disable C4756 for calc_auto_scale_range - false positive from LTCG optimizer
#pragma warning(push)
#pragma warning(disable: 4756)
...
void calc_auto_scale_range(F32& min, F32& max, F32& tick)
{
    ...
    // Clamp exponent to prevent overflow in powf (F32 range is ~1e-38 to 1e38)
    const S32 safe_exponent = llclamp(num_digits - 1, -38, 38);
    const F32 power_of_10 = powf(10.0f, (F32)safe_exponent);
        ...
        if (!llisnan(display_value) && frame_recording.getNumRecordedPeriods() != 0)
        {
            ...
            if (mDisplayHistory && mStat.valid)
            {
                ...
                for (S32 i = 1; i <= max_frame; i++)
                {
                    ...
                    if (mOrientation == HORIZONTAL)
                    {
                        gDX.vertex2f((F32)bar_rect.mRight - offset, max);
                        gDX.vertex2f((F32)bar_rect.mRight - offset, min);
                        gDX.vertex2f((F32)bar_rect.mRight - offset - 1, min);

                        gDX.vertex2f((F32)bar_rect.mRight - offset, max);
                        gDX.vertex2f((F32)bar_rect.mRight - offset - 1, min);
                        gDX.vertex2f((F32)bar_rect.mRight - offset - 1, max);
                    }
                    else
                    {
                        gDX.vertex2f(min, (F32)bar_rect.mBottom + offset + 1);
                        gDX.vertex2f(min, (F32)bar_rect.mBottom + offset);
                        gDX.vertex2f(max, (F32)bar_rect.mBottom + offset);
                        gDX.vertex2f(min, (F32)bar_rect.mBottom + offset + 1);
                        gDX.vertex2f(max, (F32)bar_rect.mBottom + offset);
                        gDX.vertex2f(max, (F32)bar_rect.mBottom + offset + 1 );
                    }
                }
                gDX.end();
            }
