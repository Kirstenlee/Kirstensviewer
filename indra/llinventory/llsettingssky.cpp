...
void LLSettingsSky::blend(LLSettingsBase::ptr_t& end, F64 blendf)
{
	...
	if (other)
	{
		...
		// ---------------------------------------------------------------------------
		// Haze parameter blending with legacy support.
		// This version avoids chained comparisons, division by zero, and silent mode
		// flips. All logic is explicit and self-contained.
		// ---------------------------------------------------------------------------

		// Independent haze lerps (always safe)
		// ---------------------------------------------------------------------------
		mHasLegacyHaze |= lerp_legacy_float(
			mHazeDensity, mLegacyHazeDensity,
			other->mHazeDensity, other->mLegacyHazeDensity,
			0.7f, (F32)blendf);

		mHasLegacyHaze |= lerp_legacy_float(
			mDistanceMultiplier, mLegacyDistanceMultiplier,
			other->mDistanceMultiplier, other->mLegacyDistanceMultiplier,
			0.8f, (F32)blendf);

		mHasLegacyHaze |= lerp_legacy_color(
			mAmbientColor, mLegacyAmbientColor,
			other->mAmbientColor, other->mLegacyAmbientColor,
			LLColor3(0.25f, 0.25f, 0.25f), (F32)blendf);

		mHasLegacyHaze |= lerp_legacy_color(
			mBlueDensity, mLegacyBlueDensity,
			other->mBlueDensity, other->mLegacyBlueDensity,
			LLColor3(0.2447f, 0.4487f, 0.7599f), (F32)blendf);

		// ---------------------------------------------------------------------------
		// Coupled haze parameters: HazeHorizon, DensityMultiplier, BlueHorizon
		// We only use the "fake level" path when all three legacy flags match.
		// ---------------------------------------------------------------------------

		// Explicit 3-way equality check with tolerance.
		// Avoids the C++ bug of (a == b == c).
		bool use_coupled_path = false;
		{
			const F32 eps = 1e-6f;
			bool eq_ab = (fabsf(mLegacyHazeHorizon - mLegacyDensityMultiplier) < eps);
			bool eq_bc = (fabsf(mLegacyDensityMultiplier - mLegacyBlueHorizon) < eps);
			use_coupled_path = (eq_ab && eq_bc);
		}

		if (use_coupled_path)
		{
			// Compute start and end "levels" for brightness smoothing.
			F32 start_level = mHazeHorizon * mDensityMultiplier * mBlueHorizon.length();
			F32 end_level = other->mHazeHorizon *
				other->mDensityMultiplier *
				other->mBlueHorizon.length();

			// Lerp the individual parameters first.
			mHasLegacyHaze |= lerp_legacy_float(
				mHazeHorizon, mLegacyHazeHorizon,
				other->mHazeHorizon, other->mLegacyHazeHorizon,
				0.19f, (F32)blendf);

			mHasLegacyHaze |= lerp_legacy_color(
				mBlueHorizon, mLegacyBlueHorizon,
				other->mBlueHorizon, other->mLegacyBlueHorizon,
				LLColor3(0.4954f, 0.4954f, 0.6399f), (F32)blendf);

			// Compute the new level.
			F32 new_level = lerp(start_level, end_level, (F32)blendf);

			// Compute a safe denominator.
			F32 denom = mHazeHorizon * mBlueHorizon.length();

			// Avoid division by zero or extremely small values.
			if (fabsf(denom) < 1e-6f)
			{
				denom = (denom < 0.f ? -1e-6f : 1e-6f);
			}

			// Recompute density multiplier safely.
			mDensityMultiplier = new_level / denom;
		}
		else
		{
			// Default independent lerps (old behavior).
			mHasLegacyHaze |= lerp_legacy_float(
				mHazeHorizon, mLegacyHazeHorizon,
				other->mHazeHorizon, other->mLegacyHazeHorizon,
				0.19f, (F32)blendf);
			...
			mHasLegacyHaze |= lerp_legacy_float(
				mDensityMultiplier, mLegacyDensityMultiplier,
				other->mDensityMultiplier, other->mLegacyDensityMultiplier,
				0.0001f, (F32)blendf);
		...
		// ---------------------------------------------------------------------------
		// Continue with parameter mapping
		// ---------------------------------------------------------------------------
...
LLSD LLSettingsSky::defaults(const LLSettingsBase::TrackPosition& position)
{
	...
	if (dfltsetting.size() == 0)
	{
		...
		dfltsetting[SETTING_CLOUD_SCALE] = LLSD::Real(0.4199f);
		dfltsetting[SETTING_CLOUD_SCROLL_RATE] = llsd::array(0.2f, 0.01f);
		dfltsetting[SETTING_CLOUD_SHADOW] = LLSD::Real(0.2699f);
		dfltsetting[SETTING_CLOUD_VARIANCE] = LLSD::Real(0.0f);

		dfltsetting[SETTING_DOME_OFFSET] = LLSD::Real(0.96f);
		dfltsetting[SETTING_DOME_RADIUS] = LLSD::Real(15000.f);
		dfltsetting[SETTING_GAMMA] = LLSD::Real(1.0f);
		dfltsetting[SETTING_GLOW] = LLColor4(5.000f, 0.0010f, -0.4799f, 1.0f).getValue();

		dfltsetting[SETTING_MAX_Y] = LLSD::Real(1605.0f);
		dfltsetting[SETTING_MOON_ROTATION] = moonquat.getValue();
		dfltsetting[SETTING_MOON_BRIGHTNESS] = LLSD::Real(0.5f);

		dfltsetting[SETTING_STAR_BRIGHTNESS] = LLSD::Real(250.0000f);
...
LLSD LLSettingsSky::translateLegacySettings(const LLSD& legacy)
{
	...
	if (legacy.has(SETTING_CLOUD_SCROLL_RATE))
	{
		...
		cloud_scroll -= LLVector2(10.0f, 10.0f);
...
LLColor3 LLSettingsSky::getLightAttenuation(F32 distance) const
{
	...
	LLColor3    light_atten = (blue_density * 1.0f + smear(haze_density * 0.25f)) * density_multiplier * distance;
	return light_atten;
}
