...
#include <boost/algorithm/string/replace.hpp>
 ...
 /*
	 Compatibility shim for deprecated libxml2 encoding helpers.

	 Collada DOM (via daeLIBXMLPlugin) still references the legacy
	 ISO‑8859‑1 <-> UTF‑8 conversion functions removed from modern
	 libxml2 builds. Rather than pinning the entire viewer to an
	 outdated XML stack, we provide minimal replacements here.

	 These functions satisfy Collada DOM’s link requirements and
	 preserve expected behaviour without modifying upstream code
	 or relying on deprecated libxml2 internals.

	 S24 — KL
 */

extern "C" {
	int UTF8Toisolat1(char* out, int* outlen, const unsigned char* in, int* inlen)
	{
		int o = 0, i = 0;
		while (i < *inlen && o < *outlen)
		{
			unsigned char c = in[i++];
			out[o++] = (c < 0x80) ? c : '?'; // ISO-8859-1 clamp
		}
		*outlen = o;
		*inlen = i;
		return 0;
	...
	int isolat1ToUTF8(unsigned char* out, int* outlen, const char* in, int* inlen)
	{
		int o = 0, i = 0;
		while (i < *inlen && o + 1 < *outlen)
		{
			unsigned char c = in[i++];
			if (c < 0x80)
			{
