...
S32 LLPrimitive::packTEField(U8 *cur_ptr, U8 *data_ptr, U8 data_size, U8 last_face_index, EMsgVariableType type) const
{
    ...
    for (face_index = last_face_index-1; face_index >= 0; face_index--)
    {
        ...
        if (!already_sent)
		{
			...
			// S24 - Refactored implementation for assigning exception faces to cur_ptr
			while (exception_faces >= ((U64)0x1 << 7)) {
				// Extract the lower 7 bits of exception_faces and add the continuation marker (0x80)
				*cur_ptr++ = (U8)((exception_faces & 0x7F) | 0x80);

				// Shift exception_faces right by 7 bits to process the next chunk of 7 bits
				exception_faces >>= 7;
			}

			// Process the last chunk: assign the remaining 7 bits without the continuation marker
...
bool LLPrimitive::getTESTAxes(const U8 face, U32* s_axis, U32* t_axis) {
	switch (face) {
	case 0:
	case 5: // Both face 0 and face 5 share the same axes
		*s_axis = VX; *t_axis = VY;
		return true;
	case 1:
	case 3: // Both face 1 and face 3 share the same axes
		*s_axis = VX; *t_axis = VZ;
		return true;
	case 2:
	case 4: // Both face 2 and face 4 share the same axes
		*s_axis = VY; *t_axis = VZ;
		return true;
	default:
		// Unknown face
