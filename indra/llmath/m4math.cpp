...
LLMatrix4::LLMatrix4(const F32* mat)
{
    for (int i = 0; i < 4; ++i)
    {
        // Load 4 elements of the row into an AVX register
        __m128 row = _mm_loadu_ps(mat + i * 4);
        // Store them directly into the matrix
        _mm_storeu_ps(mMatrix[i], row);
    }
}
...
{
    // Load the 3x3 matrix rows from LLMatrix3
    __m128 row0 = _mm_set_ps(0.f, mat.mMatrix[0][2], mat.mMatrix[0][1], mat.mMatrix[0][0]);
    __m128 row1 = _mm_set_ps(0.f, mat.mMatrix[1][2], mat.mMatrix[1][1], mat.mMatrix[1][0]);
    __m128 row2 = _mm_set_ps(0.f, mat.mMatrix[2][2], mat.mMatrix[2][1], mat.mMatrix[2][0]);

    // Load the vector into an AVX register and append 1.0 for the fourth column
    __m128 row3 = _mm_set_ps(1.f, vec.mV[2], vec.mV[1], vec.mV[0]);

    // Store the rows back into the 4x4 matrix
    _mm_storeu_ps(mMatrix[0], row0);
    _mm_storeu_ps(mMatrix[1], row1);
    _mm_storeu_ps(mMatrix[2], row2);
    _mm_storeu_ps(mMatrix[3], row3);
}

// S24 AVX
LLMatrix4::LLMatrix4(const LLMatrix3& mat)
{
    // Load the first three rows of LLMatrix3 into AVX registers and append the 0.f column
    __m128 row0 = _mm_set_ps(0.f, mat.mMatrix[0][2], mat.mMatrix[0][1], mat.mMatrix[0][0]);
    __m128 row1 = _mm_set_ps(0.f, mat.mMatrix[1][2], mat.mMatrix[1][1], mat.mMatrix[1][0]);
    __m128 row2 = _mm_set_ps(0.f, mat.mMatrix[2][2], mat.mMatrix[2][1], mat.mMatrix[2][0]);

    // Load the last row (0, 0, 0, 1) directly
    __m128 row3 = _mm_set_ps(1.f, 0.f, 0.f, 0.f);
...
{
    // Step 1: Load matrix rows into AVX registers
    __m128 row0 = _mm_loadu_ps(mMatrix[0]); // Load Row 0
    __m128 row1 = _mm_loadu_ps(mMatrix[1]); // Load Row 1
    __m128 row2 = _mm_loadu_ps(mMatrix[2]); // Load Row 2
    __m128 row3 = _mm_loadu_ps(mMatrix[3]); // Load Row 3

    // Step 2: Shuffle elements and compute sub-determinants
    __m128 temp1 = _mm_shuffle_ps(row1, row1, _MM_SHUFFLE(3, 0, 2, 1));
    __m128 temp2 = _mm_shuffle_ps(row2, row2, _MM_SHUFFLE(3, 1, 0, 2));
    __m128 temp3 = _mm_shuffle_ps(row3, row3, _MM_SHUFFLE(3, 2, 1, 0));

    __m128 term1 = _mm_mul_ps(temp1, _mm_mul_ps(temp2, temp3)); // temp1 * temp2 * temp3
    __m128 term2 = _mm_mul_ps(temp3, _mm_mul_ps(temp1, temp2)); // temp3 * temp1 * temp2
    __m128 term3 = _mm_mul_ps(temp2, _mm_mul_ps(temp3, temp1)); // temp2 * temp3 * temp1

    // Step 3: Sum and subtract results for determinant
    __m128 det = _mm_sub_ps(_mm_add_ps(term1, term2), term3);

    // Step 4: Horizontally add elements in the determinant vector
    __m128 result = _mm_hadd_ps(det, det);
    result = _mm_hadd_ps(result, result);

    // Step 5: Extract the scalar determinant value
    float determinant;
    _mm_store_ss(&determinant, result);
...
{
    // Load epsilon boundaries into an AVX register
    __m128 neg_epsilon = _mm_set1_ps(-FLT_EPSILON);
    __m128 pos_epsilon = _mm_set1_ps(FLT_EPSILON);
    __m128 zero = _mm_set1_ps(0.0f);

    for (int i = 0; i < 3; ++i) // Process only the first 3 rows
    {
        // Load the row into an AVX register (4 floats at a time)
        __m128 row = _mm_loadu_ps(mMatrix[i]);

        __m128 greater_than_neg_eps = _mm_cmpgt_ps(row, neg_epsilon);
        __m128 less_than_pos_eps = _mm_cmplt_ps(row, pos_epsilon);
        __m128 mask = _mm_and_ps(greater_than_neg_eps, less_than_pos_eps);

        // Set values within [-FLT_EPSILON, FLT_EPSILON] to 0.0f
        __m128 conditioned_row = _mm_blendv_ps(row, zero, mask);

        // Store the result back into the matrix
        _mm_storeu_ps(mMatrix[i], conditioned_row);
    }
}
...
{
    // Load the rows into AVX registers
    __m128 vec0 = _mm_loadu_ps(row0.mV); // Load 4 floats from row0
    __m128 vec1 = _mm_loadu_ps(row1.mV); // Load 4 floats from row1
    __m128 vec2 = _mm_loadu_ps(row2.mV); // Load 4 floats from row2
    __m128 vec3 = _mm_loadu_ps(row3.mV); // Load 4 floats from row3

    // Store the AVX registers into the matrix rows
    _mm_storeu_ps(mMatrix[0], vec0);
    _mm_storeu_ps(mMatrix[1], vec1);
    _mm_storeu_ps(mMatrix[2], vec2);
    _mm_storeu_ps(mMatrix[3], vec3);
}
