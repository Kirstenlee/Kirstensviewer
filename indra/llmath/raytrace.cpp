	...
	LLVector3& intersection, LLVector3& intersection_normal)
{
	...
	LLVector3   ray_to_cyl;          // from ray_point to cyl_bottom
	F32         shortest_distance;   // used for multiple purposes
	F32         cyl_length;
	...
	if (0.0f == shortest_distance)
	{
		...
		if (shortest_distance <= cyl_radius)
		{
			...
			if (shortest_distance > 0.0f)
			{
				...
				intersection.mV[0] = std::fma(-shortest_distance, cyl_axis.mV[0], ray_point.mV[0]);
				intersection.mV[1] = std::fma(-shortest_distance, cyl_axis.mV[1], ray_point.mV[1]);
				intersection.mV[2] = std::fma(-shortest_distance, cyl_axis.mV[2], ray_point.mV[2]);
			...
			else if (shortest_distance > -cyl_length)
			{
				...
				if (dot < 0.0f)
				{
					...
					F32 factor = cyl_length + shortest_distance;
					intersection.mV[0] = std::fma(-factor, cyl_axis.mV[0], ray_point.mV[0]);
					intersection.mV[1] = std::fma(-factor, cyl_axis.mV[1], ray_point.mV[1]);
					intersection.mV[2] = std::fma(-factor, cyl_axis.mV[2], ray_point.mV[2]);
					intersection_normal = -cyl_axis;
				}
				else
				{
			...
			else
			{
				...
				F32 factor = shortest_distance + cyl_length;
				intersection.mV[0] = std::fma(-factor, cyl_axis.mV[0], ray_point.mV[0]);
				intersection.mV[1] = std::fma(-factor, cyl_axis.mV[1], ray_point.mV[1]);
				intersection.mV[2] = std::fma(-factor, cyl_axis.mV[2], ray_point.mV[2]);
				intersection_normal = -cyl_axis;
	...
	if (shortest_distance <= cyl_radius)
	{
		F32 dist_to_closest_point;    // distance from ray_point to closest point on the infinite-cylinder surface
		F32 half_chord_length;        // half-length of the intersection chord
		F32 in, out;                  // distances to entering/exiting points along the ray
		...
		out = dist_to_closest_point + half_chord_length;    // distance to exiting point
		if (out < 0.0f)
		{
		...
		in = dist_to_closest_point - half_chord_length;     // distance to entering point
		if (in < 0.0f)
		{
			// ray_point is inside the cylinder;
			// store the exiting intersection point using FMA
			intersection.mV[0] = std::fma(out, ray_direction.mV[0], ray_point.mV[0]);
			intersection.mV[1] = std::fma(out, ray_direction.mV[1], ray_point.mV[1]);
			intersection.mV[2] = std::fma(out, ray_direction.mV[2], ray_point.mV[2]);
			shortest_distance = out;
		}
		else
		{
			// ray hit cylinder from outside;
			// store the entering intersection point using FMA
			intersection.mV[0] = std::fma(in, ray_direction.mV[0], ray_point.mV[0]);
			intersection.mV[1] = std::fma(in, ray_direction.mV[1], ray_point.mV[1]);
			intersection.mV[2] = std::fma(in, ray_direction.mV[2], ray_point.mV[2]);
		...
		if (line_plane(ray_point, ray_direction, cyl_top, -cyl_axis, temp_vector))
		{
			...
			line_plane(ray_point, ray_direction, cyl_bottom, cyl_axis, temp_vector);
			shortest_distance = (temp_vector - ray_point).magVec();
			if ((ray_direction * cyl_axis) < 0.0f)
			{
	...
	LLVector3& intersection)
{
	...
	{
		LLVector3 diff = point_a - intersection;
		// Compare squared distances to avoid an extra sqrt in magVec():
		if ((segment_length * segment_length) >= diff.magVecSquared())
		{
			return true;
		}
	}
	...
	LLVector3& intersection, LLVector3& intersection_normal)
{
	...
	{
		LLVector3 diff = point_a - intersection;
		// Compare the squared distance to avoid an expensive square-root operation.
		if ((segment_length * segment_length) >= diff.magVecSquared())
		{
			return true;
		}
	...
	LLVector3& intersection, LLVector3& intersection_normal)
{
	...
	LLVector3 diff = intersection - point_a;
	if ((segment_length * segment_length) < diff.magVecSquared())
	{
		return NO_SIDE;
	}
