...
LLColor4 LLColor4::white2(   1.f, 1.f, 1.f, 0.8f);
...
bool LLColor4::parseColor(const std::string& buf, LLColor4* color)
{
    ...
    // Single token = named color
    std::string color_name = tokens[0];

    if (tokens.size() > 1)
    {
        // Vector-style parsing
        LLColor4 v;
        LLStringUtil::convertToF32(color_name, v.mV[VRED]);
        LLStringUtil::convertToF32(tokens[1], v.mV[VGREEN]);
        v.mV[VBLUE] = 0.0f;
        v.mV[VALPHA] = 1.0f;

        if (tokens.size() > 2)
        {
            LLStringUtil::convertToF32(tokens[2], v.mV[VBLUE]);
            if (tokens.size() > 3)
            {
                LLStringUtil::convertToF32(tokens[3], v.mV[VALPHA]);
            }
        }
        else
        {
    ...
    {
    // Single value: handle named color
