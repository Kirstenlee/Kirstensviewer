...
#include <cfenv>    // for floating-point environment if needed
	...
	const LLVector2& w3)
{
	...
	// Use std::fma for a fused multiplication and addition in the calculations:
	float sdir_x = std::fma(t2, x1, -t1 * x2) * r;
	float sdir_y = std::fma(t2, y1, -t1 * y2) * r;
	float sdir_z = std::fma(t2, z1, -t1 * z2) * r;
	LLVector4a sdir(sdir_x, sdir_y, sdir_z);

	float tdir_x = std::fma(s1, x2, -s2 * x1) * r;
	float tdir_y = std::fma(s1, y2, -s2 * y1) * r;
	float tdir_z = std::fma(s1, z2, -s2 * z1) * r;
	LLVector4a tdir(tdir_x, tdir_y, tdir_z);

	// Copy the input normal and use sdir as the initial tangent candidate.
	LLVector4a current_normal = normal;
	LLVector4a initial_tangent = sdir;

	// Compute the cross product to later help determine handedness.
	LLVector4a cross_nt;
	...
	// Compute each component using std::fma:
	float cand_x = std::fma(-dot, current_normal.getF32ptr()[0], initial_tangent.getF32ptr()[0]);
	float cand_y = std::fma(-dot, current_normal.getF32ptr()[1], initial_tangent.getF32ptr()[1]);
	...
	if (tangent_candidate.dot3(tangent_candidate).getF32() > F_APPROXIMATELY_ZERO)
	{
		...
		// Calculate handedness: if dot(cross(normal, initial_tangent), tdir) is negative, flip.
		const F32 handedness = (cross_nt.dot3(tdir).getF32() < 0.f) ? -1.f : 1.f;
		tangent_candidate.getF32ptr()[3] = handedness;

		tangent_out = tangent_candidate;
	}
	else
	{
		// In degenerate cases, supply a default tangent.
...
// S24 a descriptive constant for the bevel parameter.
// 'NO_BEVEL' indicates that no bevel (filleted/chamfered corner) modifications should occur.
// Historically, a value of -1 was used to signal this condition, circa 2002!
constexpr float NO_BEVEL = -1.0f;

// S24 Performs a linear interpolation between two LLVector4a vectors using FMA for the first three components.
// Uses getF32ptr() to access vector components.
// Adjust this if LLVector4a requires a different interface.
LLVector4a lerpFMA(const LLVector4a& a, const LLVector4a& b, float f)
{
	const F32* a_ptr = a.getF32ptr();
	const F32* b_ptr = b.getF32ptr();

	F32 x = std::fma(b_ptr[0] - a_ptr[0], f, a_ptr[0]);
	F32 y = std::fma(b_ptr[1] - a_ptr[1], f, a_ptr[1]);
	F32 z = std::fma(b_ptr[2] - a_ptr[2], f, a_ptr[2]);

	LLVector4a result;
	result.set(x, y, z); // Assumption: This sets the vector's components.
	return result;
}
...
{
	// Generate an n-sided "circular" path.
	// 0 is (1,0), and we go counter-clockwise along a circular path from that point.
	// NOTE: The 'bevel' parameter is currently unused.
	// When bevel == NO_BEVEL, no filleting/chamfering is applied. This was intended for potential future support of
	...
	const F32 begin = params.getBegin();
	const F32 end = params.getEnd();

	t_step = 1.0f / sides;
    ang_step = 2.0f*F_PI*t_step*ang_scale;

	S32 total_sides = ll_round(sides / ang_scale);  // Total sides across the circle.
	if (total_sides < 8)
	{
	...
	if (t_fraction < 0.9999f)
	{
		...
		// (Assumes a helper function 'lerpFMA' that handles linear interpolation using FMA.)
		LLVector4a new_pt = lerpFMA(pt1, pt2, t_fraction);
		mProfile.push_back(new_pt);
	}

	// Generate points for integer steps across the profile.
	while (t < end)
	{
		pt1.set(cos(ang) * scale, sin(ang) * scale, t);

		// If there is at least one point already in mProfile, insert split (interpolated) points.
		if (mProfile.size() > 0)
		{
			LLVector4a p = mProfile[mProfile.size() - 1];
			for (S32 i = 0; i < split; i++)
			{
				float factor = (i + 1) / static_cast<float>(split + 1);
				LLVector4a new_pt = lerpFMA(p, pt1, factor);
				mProfile.push_back(new_pt);
			}
		}
	...
	// Process any fractional portion at the end of the profile.
	t_fraction = (end - (t - t_step)) * sides;
	pt2.set(cos(ang) * scale, sin(ang) * scale, t);
	if (t_fraction > 0.0001f)
	{
		...
		if (mProfile.size() > 0)
		{
			...
			{
				float factor = (i + 1) / static_cast<float>(split + 1);
				LLVector4a interp_pt = lerpFMA(p, new_pt, factor);
				mProfile.push_back(interp_pt);
			}
		}
		mProfile.push_back(new_pt);
	}

	// Determine if the profile is open (sliced) or closed.
	if ((end - begin) * ang_scale < 0.99f)
	{
		...
		if (params.getHollow() <= 0)
		{
			// For profiles that are not hollow, add the center point.
...
LLProfile::Face* LLProfile::addHole(const LLProfileParams& params, bool flat, F32 sides, F32 offset, F32 box_hollow, F32 ang_scale, S32 split)
{
	...
	// S24 see comments above NO_BEVEL is now used once upon a time( 2002 !) objects were going to use chamfered corners! KL
	genNGon(params, llfloor(sides), offset, NO_BEVEL, ang_scale, split);
