...
void LLStreamingAudio_FMODSTUDIO::update()
{
    ...
    if (mFMODInternetStreamChannelp)
    {
        ...
        if (mFMODInternetStreamChannelp->getCurrentSound(&sound) == FMOD_OK && sound)
        {
            ...
            {
                // Process tags if there are any dirty tags OR if we haven't read tags yet
                if (dirtytagcount > 0 || tagcount > 0)
                {
                    for (S32 i = 0; i < tagcount; ++i)
                    {
                        ...
                        // Extract metadata from streaming audio (Shoutcast/Icecast/ID3)
                        if (tag.datatype == FMOD_TAGDATATYPE_STRING && tag.data)
                        {
                            std::string tag_name = tag.name ? tag.name : "";
                            std::string tag_data = (const char*)tag.data;

                            // Shoutcast/Icecast metadata
                            if (tag.type == FMOD_TAGTYPE_SHOUTCAST || tag.type == FMOD_TAGTYPE_ICECAST)
                            {
                                // Ignore non-metadata tags (bitrate, URLs, station info, notices, etc.)
                                if (tag_name == "icy-br" || tag_name == "icy-url" || 
                                    tag_name == "icy-name" || tag_name == "icy-genre" ||
                                    tag_name == "icy-description" || tag_name == "icy-pub" ||
                                    tag_name == "icy-notice1" || tag_name == "icy-notice2" ||
                                    tag_data.empty())
                                {
                                    continue; // Skip these tags
                                }

                                // Handle ARTIST and TITLE tags separately
                                if (tag_name == "ARTIST")
                                {
                                    mLastMetadataArtist = tag_data;
                                }
                                else if (tag_name == "TITLE")
                                {
                                    mLastMetadataTitle = tag_data;
                                }
                                // StreamTitle contains "Artist - Title" combined
                                else if (tag_name == "StreamTitle")
                                {
                                    size_t separator = tag_data.find(" - ");
                                    if (separator != std::string::npos)
                                    {
                                        mLastMetadataArtist = tag_data.substr(0, separator);
                                        mLastMetadataTitle = tag_data.substr(separator + 3);
                                    }
                                    else
                                    {
                                        mLastMetadataTitle = tag_data;
                                        mLastMetadataArtist.clear();
                                    }
                                }
                            }
                            // ID3 tags (MP3 streams)
                            else if (tag.type == FMOD_TAGTYPE_ID3V1 || tag.type == FMOD_TAGTYPE_ID3V2)
                            {
                                if (tag_name == "TIT2" || tag_name == "TITLE")
                                {
                                    mLastMetadataTitle = tag_data;
                                }
                                else if (tag_name == "TPE1" || tag_name == "ARTIST")
                                {
                                    mLastMetadataArtist = tag_data;
                                }
                            }
            ...
            {
                static U32 tag_error_count = 0;
                if (++tag_error_count % 300 == 0) // Log every ~5 seconds at 60fps
                {
                    LL_WARNS("FMOD") << "Failed to get tag count from stream" << LL_ENDL;
                }
            }
...
bool LLStreamingAudio_FMODSTUDIO::getMetadata(std::string& title, std::string& artist)
{
    // Return current metadata (will be populated by update() when tags are found)
    title = mLastMetadataTitle;
    artist = mLastMetadataArtist;

    // Return true if we have any metadata
