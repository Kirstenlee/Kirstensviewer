...
bool LLAudioEngine_FMODSTUDIO::init(void* userdata, const std::string &app_title)
{
    ...
    result = mSystem->setSoftwareChannels(LL_MAX_AUDIO_CHANNELS);  // S24 - 32 set by default no need to +2
    ...
    result = mSystem->init(LL_MAX_AUDIO_CHANNELS, fmod_flags, 0); // S24 32 set by default
...
// S24 less verbose and cleaner version
void LLAudioEngine_FMODSTUDIO::shutdown()
{
    stopInternetStream();
    cleanupEqualizer(); // S24 - Clean up equalizer before shutdown
    ...
    LL_INFOS("FMOD") << "LLAudioEngine_FMODSTUDIO::shutdown() complete" << LL_ENDL;
...
//-----------------------------------------------------------------------
// S24 - 3-Band Equalizer Support
//-----------------------------------------------------------------------
bool LLAudioEngine_FMODSTUDIO::initEqualizer()
{
    if (!mSystem || mEQDSP)
        return false;

    // Create a 3-band EQ DSP
    FMOD_RESULT result = mSystem->createDSPByType(FMOD_DSP_TYPE_THREE_EQ, &mEQDSP);
    if (Check_FMOD_Error(result, "FMOD::System::createDSPByType(FMOD_DSP_TYPE_THREE_EQ)"))
        return false;

    // Add the EQ to the master channel group
    FMOD::ChannelGroup* master_group = NULL;
    if (!Check_FMOD_Error(mSystem->getMasterChannelGroup(&master_group), "FMOD::System::getMasterChannelGroup") && master_group)
    {
        result = master_group->addDSP(FMOD_CHANNELCONTROL_DSP_TAIL, mEQDSP);
        if (Check_FMOD_Error(result, "FMOD::ChannelGroup::addDSP"))
        {
            mEQDSP->release();
            mEQDSP = NULL;
            return false;
    ...
    // Set the EQ to active
    mEQDSP->setActive(true);

    LL_INFOS("FMOD") << "3-band equalizer initialized successfully" << LL_ENDL;
    return true;
}

void LLAudioEngine_FMODSTUDIO::cleanupEqualizer()
{
    if (mEQDSP)
    {
        FMOD::ChannelGroup* master_group = NULL;
        if (!Check_FMOD_Error(mSystem->getMasterChannelGroup(&master_group), "FMOD::System::getMasterChannelGroup")
            && master_group)
        {
            master_group->removeDSP(mEQDSP);
        }
        mEQDSP->release();
        mEQDSP = NULL;
        LL_INFOS("FMOD") << "3-band equalizer cleaned up" << LL_ENDL;
    }
}

void LLAudioEngine_FMODSTUDIO::updateEqualizer(bool enabled, F32 low_gain, F32 mid_gain, F32 high_gain)
{
    if (!mEQDSP)
        return;

    // Note: FMOD THREE_EQ uses these parameters:
    // FMOD_DSP_THREE_EQ_LOWGAIN  (Low frequency gain in dB, -80 to 10, default = 0)
    // FMOD_DSP_THREE_EQ_MIDGAIN  (Mid frequency gain in dB, -80 to 10, default = 0)
    // FMOD_DSP_THREE_EQ_HIGHGAIN (High frequency gain in dB, -80 to 10, default = 0)
    // FMOD_DSP_THREE_EQ_LOWCROSSOVER  (Low-to-mid crossover frequency, 10 to 22000, default = 400)
    // FMOD_DSP_THREE_EQ_HIGHCROSSOVER (Mid-to-high crossover frequency, 10 to 22000, default = 4000)

    // Enable/disable the EQ
    mEQDSP->setBypass(!enabled);

    if (enabled)
    {
        // Clamp values to FMOD's acceptable range (-30 to +10 dB for safety)
        F32 low_db = llclamp(low_gain, -30.0f, 10.0f);
        F32 mid_db = llclamp(mid_gain, -30.0f, 10.0f);
        F32 high_db = llclamp(high_gain, -30.0f, 10.0f);

        mEQDSP->setParameterFloat(FMOD_DSP_THREE_EQ_LOWGAIN, low_db);
        mEQDSP->setParameterFloat(FMOD_DSP_THREE_EQ_MIDGAIN, mid_db);
        mEQDSP->setParameterFloat(FMOD_DSP_THREE_EQ_HIGHGAIN, high_db);
    }
}
...
void LLAudioChannelFMODSTUDIO::play()
{
    if (!mChannelp)
    {
        //S24 no warn it just spams  the debug console!
        return;
    }
...
// S24 C++11 negates use of memset also don't delete user file as failure can be for other reasons
bool LLAudioBufferFMODSTUDIO::loadWAV(const std::string& filename)
{
    ...
    FMOD_RESULT result = getSystem()->createSound(filename.c_str(), FMOD_LOOP_NORMAL, &exinfo, &mSoundp);
    if (result != FMOD_OK)
    {
        LL_WARNS() << "Failed to load '" << filename << "': " << FMOD_ErrorString(result) << LL_ENDL;

        // KL Consider quarantine or user notification instead of deleting
