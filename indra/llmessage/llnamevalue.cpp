...
void LLNameValue::init(const char *name, const char *data, const char *type, const char *nvclass, const char *nvsendto)
{
    ...
    {
        mType = NVT_VEC3;
        F32 t1 = 0.f, t2 = 0.f, t3 = 0.f;

        int parsed = 0;
        if (strchr(data, '<'))
        {
            parsed = sscanf(data, "<%f, %f, %f>", &t1, &t2, &t3);
        }
        else
        {
            parsed = sscanf(data, "%f, %f, %f", &t1, &t2, &t3);
        ...
        if (parsed != 3 || !llfinite(t1) || !llfinite(t2) || !llfinite(t3))
        {
            LL_WARNS("LLNameValueParser") << "Invalid VEC3 format: '" << data << "'" << LL_ENDL;
            t1 = t2 = t3 = 0.f;
        }
...
{
    baseInit();
    char name[NV_BUFFER_LEN] = {};
    char type[NV_BUFFER_LEN] = {};
    char nvclass[NV_BUFFER_LEN] = {};
    char nvsendto[NV_BUFFER_LEN] = {};
    char nvdata[NV_BUFFER_LEN] = {};

    S32 character_count = 0;

    auto skipWhitespace = [&](const char* str, S32& offset) {
    ...
    auto parseToken = [&](const char* str, S32& offset, char* out) -> bool {
        skipWhitespace(str, offset);
        int parsed = sscanf(str + offset, "%2047s", out);
        if (parsed == 1)
        {
            offset += static_cast<S32>(strlen(out));
            return true;
        }
        return false;
        };

    // SECURITY: Enforce parsing gates and avoid silent memory corruption via character_count offset validation.
    //           Whitespace skipping and token extraction wrapped with binary guards for stability.

    // Parse name and type
    if (!parseToken(data, character_count, name) || !parseToken(data, character_count, type))
    {
        LL_WARNS("LLNameValue") << "Failed to parse name/type from: '" << data << "'" << LL_ENDL;
        init("", "", "", "", "");
        return;
    }

    // Parse optional nvclass
    S32 i;
    skipWhitespace(data, character_count);
    for (i = NVC_READ_ONLY; i < NVC_EOF; ++i)
    {
        if (!strncmp(NameValueClassStrings[i], data + character_count, strlen(NameValueClassStrings[i])))
        {
            parseToken(data, character_count, nvclass);
            break;
        }
    }
    if (i == NVC_EOF)
    ...
    // Copy remaining data
    skipWhitespace(data, character_count);
    strncpy(nvdata, data + character_count, NV_BUFFER_LEN - 1);
    nvdata[NV_BUFFER_LEN - 1] = '\0';
...
{
    if (mClass == NVC_READ_ONLY || mType != NVT_STRING)
    {
        return; // early exit for read-only or non-string types
    }

    delete[] mNameValueReference.string; // safe even if null

    if (a && a[0] != '\0')
    {
        size_t len = strlen(a);
        mNameValueReference.string = new char[len + 1];
        memcpy(mNameValueReference.string, a, len);
        mNameValueReference.string[len] = '\0';
    }
    else
    {
...
{
    if (mClass == NVC_READ_ONLY || mType != NVT_ASSET)
    {
        return; // early exit for read-only or incorrect type
    }
