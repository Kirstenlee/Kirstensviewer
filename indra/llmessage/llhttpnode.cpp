...
// virtual S24 FIX: Handle host or Host :)
void LLHTTPNode::options(ResponsePtr response, const LLSD& context) const
{
	LL_DEBUGS("LLHTTPNode") << "context: " << context << LL_ENDL;

	// SECURITY: Validates presence of Host header for proper URL construction.
	//           Supports casing variants ("Host" and "host") to avoid malformed request handling.
	std::string host = context[CONTEXT_REQUEST][CONTEXT_HEADERS]["Host"].asString();
	if (host.empty())
	{
		host = context[CONTEXT_REQUEST][CONTEXT_HEADERS]["host"].asString();
	}

	if (host.empty())
	{
		response->status(HTTP_BAD_REQUEST, "Bad Request -- missing Host header");
		return;
	}
