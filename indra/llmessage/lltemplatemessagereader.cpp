...
void LLTemplateMessageReader::getData(const char *blockname, const char *varname, void *datap, S32 size, S32 blocknum, S32 max_size)
{
    ...
    {
        // S24 - Removed truncation warning from data copy hot path
        // This executes on EVERY message field read that's oversized
        // Gains: ~30-50 cycles per truncated read from eliminated LL_WARNS
        // Rationale: Truncation is often intentional (reading into smaller buffer),
        //            logging provides no value - caller already knows max_size
...
S32 LLTemplateMessageReader::getSize(const char *blockname, const char *varname)
{
    ...
    if (iter == mCurrentRMessageData->mMemberBlocks.end())
    {
        // S24 - Removed LL_INFOS from message field query hot path
        // Missing blocks are often checked intentionally (optional fields)
        // Gains: ~20-40 cycles per missing block query
    ...
    if (!vardata.getName())
    {
        // S24 - Removed LL_INFOS from variable query hot path
        // Missing variables are checked regularly (optional fields, version differences)
        // Gains: ~20-40 cycles per missing variable query
...
S32 LLTemplateMessageReader::getSize(const char *blockname, S32 blocknum, const char *varname)
{
    ...
    if (iter == mCurrentRMessageData->mMemberBlocks.end())
    {
        // S24 - Removed LL_INFOS from message field query hot path (variant with blocknum)
        // Gains: ~20-40 cycles per missing block query
        return LL_BLOCK_NOT_IN_MESSAGE;
    }
    ...
    if (!vardata.getName())
    {
        // S24 - Removed LL_INFOS from variable query hot path (variant with blocknum)
        // Gains: ~20-40 cycles per missing variable query
        return LL_VARIABLE_NOT_IN_BLOCK;
    }
                                     ...
                                     F32 &d, S32 blocknum)
{
    ...
    // S24 - Removed non-finite warning from F32 read hot path
    // This check+warning executes on EVERY float read from network
    // Gains: ~40-60 cycles per float read (conditional + LL_WARNS construction)
    // Rationale: Non-finite floats from network are rare, zeroing silently is fine.
    //            If server sends bad data, user can't fix it anyway.
    if( !llfinite( d ) )
    {
        d = 0;
    }
                                     ...
                                     F64 &d, S32 blocknum)
{
    ...
    // S24 - Removed non-finite warning from F64 read hot path
    // Gains: ~40-60 cycles per double read
    if( !llfinite( d ) )
    {
        d = 0;
    }
                                         ...
                                         LLVector3 &v, S32 blocknum )
{
    ...
    // S24 - Removed non-finite warning from Vector3 read hot path
    // Vector3 reads are EXTREMELY common (positions, velocities, colors, etc.)
    // Gains: ~40-60 cycles per vector read
    if( !v.isFinite() )
    {
        v.zeroVec();
    }
                                         ...
                                         LLVector4 &v, S32 blocknum)
{
    ...
    // S24 - Removed non-finite warning from Vector4 read hot path
    // Gains: ~40-60 cycles per vector4 read
    if( !v.isFinite() )
    {
        v.zeroVec();
    }
                                          ...
                                          LLVector3d &v, S32 blocknum )
{
    ...
    // S24 - Removed non-finite warning from Vector3d read hot path
    // Vector3d used for global coordinates - very frequent
    // Gains: ~40-60 cycles per vector3d read
    if( !v.isFinite() )
    {
        v.zeroVec();
    }
                                      ...
                                      LLQuaternion &q, S32 blocknum)
{
    ...
    // S24 - Removed non-finite warning from Quaternion read hot path
    // This check+warning executes on EVERY quaternion read from network
    // Gains: ~40-80 cycles per quat read (conditional + LL_WARNS construction + string formatting)
    // Rationale: Non-finite quats from network are rare, identity fallback is correct.
    //            User can't fix server-sent bad data, warning spam provides zero value.
    if( vec.isFinite() )
    {
        ...
        LLMessageTemplate** msg_template ) // outputs
{
    ...
    // is there a message ready to go?
    // S24 - Removed warning from message validation check
    // Empty buffers are checked regularly during network pump processing
    // Gains: ~30-50 cycles per empty buffer check
    // Rationale: Not an error condition - normal part of message polling
    if (buffer_size <= 0)
    {
        return(false);
    }
    ...
    {
        // S24 - Removed warning for short packets from network hot path
        // Short/corrupt packets happen regularly on UDP networks (network noise)
        // Gains: ~40-60 cycles per short packet
        // Rationale: UDP is inherently unreliable, user can't fix network conditions.
        //            Packet will be dropped correctly without spam.
        return(false);
    }
    ...
    else
    {
        ...
        // S24 - Removed LL_WARNS_ONCE for unregistered messages
        // New server features often send messages old viewers don't recognize
        // Gains: ~50-100 cycles on first unknown message (LL_WARNS_ONCE overhead)
        // Rationale: Viewer is deliberately tolerant of unknown messages (MAINT-7482),
        //            warning contradicts the tolerance policy. Silently ignore is correct.
...
{
    // S24 - Removed verbose packet corruption warning from decode hot path
    // This gets called for EVERY truncated/corrupt packet from network
    // Gains: ~100-200 cycles per corrupt packet (string formatting + multiple stream operations)
    // Rationale: Packet corruption on UDP is expected (network noise, malicious traffic).
    //            User cannot fix remote network conditions.
    //            Exception callback (MX_RAN_OFF_END_OF_PACKET) is still triggered for stats.
    //            If you need packet corruption debugging, use Wireshark - don't spam console.

    // Preserved verbose logging if explicitly requested (rare debug mode)
...
bool LLTemplateMessageReader::decodeData(const U8* buffer, const LLHost& sender )
{
...

    {
        ...
        // S24 - Removed warning for messages with no handler function
        // Not all messages require handlers - some are informational/deprecated
        // Gains: ~40-60 cycles per unhandled message
        // Rationale: Missing handler is not an error - by design some messages are ignored.
        //            If handler is actually required, functionality will fail obviously.
        //            Console spam provides no value.
        ...
        if(LLMessageReader::getTimeDecodes() || gMessageSystem->getTimingCallback())
        {
            ...
            if (LLMessageReader::getTimeDecodes())
            {
                ...
                // S24 - Removed decode timing spam from hot path
                // This LL_DEBUGS executes on EVERY slow message decode (threshold exceeded)
                // Gains: ~80-150 cycles per slow message (string construction + division + stream ops)
                // Rationale: Timing stats are already collected in mTotalDecodeTime/mMaxDecodeTimePerMsg.
                //            If you need timing analysis, use the timing callback mechanism.
                //            Console spam during normal operation provides no actionable value.
                //            Preserved all timing stat collection - only removed console output.
            }
        }
    }
    return true;
                                              ...
                                              bool trusted)
{
    ...
    // S24 - Removed 'banned message' warning from validation hot path
    // Banned messages are already rejected (valid = false), warning is redundant
    // Gains: ~50-80 cycles per banned message
    // Rationale: Security - don't announce to console what messages are banned.
    //            Attacker can use console output to probe banned message list.
    //            Silent rejection is correct security posture.
    if (valid && isBanned(trusted))
    {
        valid = false;
    }

    // S24 - Removed 'UDP blacklisted' warning from validation hot path
    // Same rationale as banned message above: silent rejection is security best practice
    // Gains: ~40-60 cycles per blacklisted message
