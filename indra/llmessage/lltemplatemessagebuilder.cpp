...
void LLTemplateMessageBuilder::newMessage(const char *name)
{
    ...
    if (mMessageTemplates.count(namep) > 0)
    {
        ...
        // S24 - Removed deprecated message warning from message creation hot path
        // Deprecation warnings spam logs and provide no value to users
        // Gains: ~30-50 cycles per deprecated message send
        // Rationale: If we're sending it, we know it's deprecated. Server handles compatibility.
    ...
    if (mCurrentSBlockName)
    {
            ...
            &&(mCurrentSMessageTemplate))
        {
            if (mCurrentSMessageData->mMemberBlocks[mCurrentSBlockName]->mBlockNumber >= 1)
            {
                ...
                if (num_blocks <= 1)
                {
                    // S24 - Removed warning from block removal edge case
                    // This is a programming error check, not a user-actionable warning
                    // Gains: ~20-30 cycles when attempting to remove last block
...
void LLTemplateMessageBuilder::addData(const char *varname, const void *data, EMsgVariableType type, S32 size)
{
    ...
    {
        // S24 - Removed Variable 1 truncation warning from data add hot path  
        // Variable size clamping occurs, logging provides no value
        // Gains: ~30-50 cycles per oversized variable add
        // Rationale: Truncation is intentional when it happens, caller knows size limit
        // Variable 1 can only store 255 bytes, clamp if necessary
...
static S32 buildBlock(U8* buffer, S32 buffer_size, const LLMessageBlock* template_data, LLMsgData* message_data)
{
    ...
    // S24 Defensive: if the message data doesn't contain this block, warn and skip it
    if (block_iter == message_data->mMemberBlocks.end())
    {
        LL_WARNS() << "buildBlock: message data missing block '" << template_data->mName
                   << "'. Skipping block instead of crashing." << LL_ENDL;
        return 0;
    }
    ...
    if (template_data->mType == MBT_VARIABLE)
    {
        ...
        else
        {
            ...
            LL_WARNS() << "buildBlock failed. Message excedding "
    ...
    while(block_count > 0)
    {
             ...
             iter != mbci->mMemberVarData.end(); iter++)
        {
            ...
            if (mvci.getSize() == -1)
            {
                // new (non-fatal): warn and return error up the stack so we don't abort the process
                LL_WARNS() << "The variable " << mvci.getName() << " in block "
                           << mbci->mName << " of message "
                           << template_data->mName
                           << " wasn't set prior to buildMessage call. Aborting message build." << LL_ENDL;
            ...
            else
            {
                ...
                if(data_size > 0)
                {
                    ...
                    case 4:
                        ...
                        break;
                    default:
                        LL_WARNS() << "Attempting to build variable field with unknown size of " << size << LL_ENDL;
                        break;
                    }
    ...
    U8 offset_to_data)
{
    ...
    if (!mCurrentSMessageTemplate)
    {
        LL_WARNS() << "newMessage not called prior to buildMessage" << LL_ENDL;
        return 0;
    }
...
// new, independant way
    ...
    {
        LL_WARNS() << "unexpected message frequency in buildMessage" << LL_ENDL;
        return 0;
    }
    ...
    for(LLMessageTemplate::message_block_map_t::const_iterator
        ...
        ++iter)
    {
        S32 block_result = buildBlock(buffer + result, buffer_size - result, *iter, mCurrentSMessageData);
        if (block_result < 0)
        {
            LL_WARNS() << "buildMessage: aborting build due to missing/invalid data for block '"
                       << (*iter)->mName << "' in message '" << mCurrentSMessageTemplate->mName << "'" << LL_ENDL;
            return 0; // fail gracefully (no crash)
        }
        result += block_result;
    }
    mbSBuilt = true;
