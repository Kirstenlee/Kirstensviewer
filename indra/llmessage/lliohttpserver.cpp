...
// Use constexpr for compile-time constant - no runtime initialization
static constexpr const char HTTP_VERSION_STR[] = "HTTP/1.0";

static LLIOHTTPServer::timing_callback_t sTimingCallback = NULL;
static void* sTimingCallbackData = NULL;

// =============================================================================
// Helper function to compare std::string with std::string_view efficiently
// This avoids creating temporary std::string objects for comparison
// =============================================================================
[[nodiscard]] inline bool streq(const std::string& str, std::string_view sv) noexcept
{
    ...
    LLPumpIO* pump)
{
    ...
    if (mState == STATE_INVOKE)
    {
        ...
        // Get verb as string for comparison with string_view constants
        std::string verb = context[CONTEXT_REQUEST][CONTEXT_VERB];
        
        // Use helper function to compare std::string with string_view constants
        // This is more efficient than creating temporary std::string objects
        if (streq(verb, HTTP_VERB_GET))
        {
            mNode.get(LLHTTPNode::ResponsePtr(mResponse), context);
        }
        else if (streq(verb, HTTP_VERB_PUT))
        {
            LLSD input;
        ...
        else if (streq(verb, HTTP_VERB_POST))
        {
            LLSD input;
            if (mNode.getContentType() == LLHTTPNode::CONTENT_TYPE_LLSD)
            {
        ...
        else if (streq(verb, HTTP_VERB_DELETE))
        {
            mNode.del(LLHTTPNode::ResponsePtr(mResponse), context);
        }
        else if (streq(verb, HTTP_VERB_OPTIONS))
        {
    ...
    switch (mState)
    {
        ...
        case STATE_GOOD_RESULT:
        {
            ...
            // Convert string_view to std::string for LLSD map key
            headers[std::string{HTTP_OUT_HEADER_CONTENT_TYPE}] = std::string{HTTP_CONTENT_LLSD_XML};
        ...
        case STATE_STATUS_RESULT:
        {
            LLSD headers = mHeaders;
            // Convert string_view to std::string for LLSD map key
            headers[std::string{HTTP_OUT_HEADER_CONTENT_TYPE}] = std::string{HTTP_CONTENT_TEXT_PLAIN};
            context[CONTEXT_RESPONSE][CONTEXT_HEADERS] = headers;
    ...
    LLPumpIO* pump)
{
    ...
    if(eos)
    {
        ...
        if(0 < content_length)
        {
            // Output header name from string_view constant - streams handle string_view correctly
    ...
    if((STATE_NOTHING == mState) || (STATE_READING_HEADERS == mState))
    {
        ...
        if(readHeaderLine(channels, buffer, (U8*)buf, len))
        {
            ...
            if(mVerb.empty())
            {
                ...
                // Compare mVerb (std::string) with string_view constants using helper
                if (streq(mVerb, HTTP_VERB_GET)
                   || streq(mVerb, HTTP_VERB_POST)
                   || streq(mVerb, HTTP_VERB_PUT)
                   || streq(mVerb, HTTP_VERB_DELETE)
                   || streq(mVerb, HTTP_VERB_OPTIONS))
                {
            ...
            if(parse_all)
            {
                ...
                while(keep_parsing)
                {
                    ...
                    // Compare with string_view constant - name is already lowercase
                    if (streq(name, HTTP_IN_HEADER_CONTENT_LENGTH))
                    {
