...
void LLHttpSDHandler::onCompleted(LLCore::HttpHandle handle, LLCore::HttpResponse * response)
{
    ...
    else
    {
        ...
        if (!parsed)
        {
            ...
            // HttpHeaders::find() takes const std::string& so convert string_view
            // Returns const std::string* which can be compared directly with string_view
            const std::string* contentType = (headers) 
                ? headers->find(std::string{HTTP_IN_HEADER_CONTENT_TYPE}) 
                : nullptr;

            // Compare std::string with string_view - works directly in C++17+
            if (contentType && (*contentType == HTTP_CONTENT_LLSD_XML))
            {
                std::string thebody = LLCoreHttpUtil::responseToString(response);

                LL_WARNS() << "Failed to deserialize . " << response->getRequestURL() 
