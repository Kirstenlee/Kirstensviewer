...
void LLAvatarNameCache::handleAvNameCacheSuccess(const LLSD &data, const LLSD &httpResult)
{
    ...
    if (num_unresolved > 0)
    {
        // S24: Removed warning spam from hot path - unresolved IDs are handled by handleAgentError()
        // Network response contains ID details if debugging needed (check LLSD dumps)
        for (LLSD::array_const_iterator it = unresolved_agents.beginArray(); 
             it != unresolved_agents.endArray(); ++it)
        {
...
bool LLAvatarNameCache::importFile(std::istream& istr)
{
    ...
    // S24: Changed to LL_DEBUGS - cache load success is normal operation, not user-facing information
    // Users don't care how many avatar names loaded from cache - this is internal detail
    // Success is expected, only failures matter to end users
    // Potential gain: Eliminate cache statistics spam from startup logs
    LL_DEBUGS("AvNameCache") << "LLAvatarNameCache loaded " << mCache.size() << " entries" << LL_ENDL;
...
void LLAvatarNameCache::exportFile(std::ostream& ostr)
{
    ...
    // S24: Changed to LL_DEBUGS - cache export is normal shutdown operation
    // Users don't care about cache statistics during viewer exit
    // This is internal bookkeeping, not user-facing information
    // Potential gain: Cleaner shutdown logs, focus on actual errors
    LL_DEBUGS("AvNameCache") << "LLAvatarNameCache at exit cache has " << mCache.size() << " entries" << LL_ENDL;
    ...
    // S24: Changed to LL_DEBUGS - cache export count is internal detail, not user information
    // Exporting avatar name cache is normal shutdown behavior
    // Users don't need to know how many names were exported
    // Potential gain: Reduce noise in shutdown logs
    LL_DEBUGS("AvNameCache") << "LLAvatarNameCache exporting " << agents.size() << " valid entries" << LL_ENDL;
    LLSD data;
    data["agents"] = agents;
...
bool max_age_from_cache_control(const std::string& cache_control, S32* max_age)
{
    ...
    {
        LLStringUtil::trim(directive);
        if (directive.compare(0, MAX_AGE.size(), MAX_AGE) == 0)
        {
            // Split "max-age=1234" into ["max-age", "1234"]
            std::istringstream subtokens(directive);
            std::string key, value;

            if (!std::getline(subtokens, key, '=')) return false;
            LLStringUtil::trim(key);
            if (key != MAX_AGE) return false;

            if (!std::getline(subtokens, value)) return false;
