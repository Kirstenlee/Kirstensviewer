...
// Use inline constexpr for zero-overhead constant - no heap allocation
inline constexpr F32 HTTP_REQUEST_EXPIRY_SECS = 60.0f;

namespace
{
    // Local constant using string_view for consistency with updated llhttpconstants.h
    inline constexpr std::string_view HTTP_LOGBODY_KEY{"HTTPLogBodyOnError"};

    BoolSettingQuery_t  mBoolSettingGet;
    BoolSettingUpdate_t mBoolSettingPut;

    // Helper to convert string_view to std::string for APIs that require it
    [[nodiscard]] inline bool getBoolSetting(std::string_view keyname)
    {
        if (mBoolSettingGet == nullptr)
            return false;
        // Convert to std::string for the callback interface
        return mBoolSettingGet(std::string{HTTP_LOGBODY_KEY});
    }
...
void setPropertyMethods(BoolSettingQuery_t queryfn, BoolSettingUpdate_t updatefn)
{
    ...
    {
        // Convert string_view to std::string for the callback
        mBoolSettingPut(std::string{HTTP_LOGBODY_KEY}, false, "Log the entire HTTP body in the case of an HTTP error.");
    }
}
...
{
    // Use string_view for the static empty string - no heap allocation
...
//    const bool emit_parse_errors = false;
    ...
    if (!success)
    {
        ...
        const std::string *contentType = (headers) ? headers->find(std::string{HTTP_IN_HEADER_CONTENT_TYPE}) : nullptr;

        // Compare std::string* with string_view using operator== (string_view is comparable to std::string)
        if (contentType && (*contentType == HTTP_CONTENT_LLSD_XML))
        {
...
LLSD HttpCoroRawHandler::handleSuccess(LLCore::HttpResponse * response, LLCore::HttpStatus &status) const
{
    ...
    // Reserve and read binary data efficiently
    LLSD::Binary data;
    data.reserve(size);
    bas >> std::noskipws;
...
//========================================================================
// Static member constants - using inline constexpr string_view for zero-overhead
// These are defined in the header as inline constexpr, but if they must remain
// as std::string for ABI compatibility, keep them here:
    ...
    LLCore::HttpOptions::ptr_t options, LLCore::HttpHeaders::ptr_t headers)
{
    ...
    // Convert string_view to std::string for append() - HttpHeaders::append takes const std::string&
    headers->append(std::string{HTTP_OUT_HEADER_DESTINATION}, dest);
    ...
    LLCore::HttpOptions::ptr_t options, LLCore::HttpHeaders::ptr_t headers)
{
    ...
    // Convert string_view to std::string for append()
    headers->append(std::string{HTTP_OUT_HEADER_DESTINATION}, dest);
...
void HttpCoroutineAdapter::checkDefaultHeaders(LLCore::HttpHeaders::ptr_t &headers)
{
    ...
    // Convert string_view constants to std::string for HttpHeaders API
    // HttpHeaders::find() and append() accept const std::string&
    if (!headers->find(std::string{HTTP_OUT_HEADER_ACCEPT}))
    {
        headers->append(std::string{HTTP_OUT_HEADER_ACCEPT}, std::string{HTTP_CONTENT_LLSD_XML});
    }
    if (!headers->find(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}))
    {
        headers->append(std::string{HTTP_OUT_HEADER_CONTENT_TYPE}, std::string{HTTP_CONTENT_LLSD_XML});
    }
