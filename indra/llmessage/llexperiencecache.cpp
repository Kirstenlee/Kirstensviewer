...
void LLExperienceCache::requestExperiences()
{
    ...
    {
        // Capability query method not available during early login, region transitions, or shutdown.
        // This is expected - experiences will be fetched once region capabilities are established.
        LL_DEBUGS("ExperienceCache") << "Capability query method not set (region not fully connected)." << LL_ENDL;
        return;
    }
    ...
    if (urlBase.empty())
    {
        // GetExperienceInfo capability not available yet - normal during login/region crossing/shutdown.
        // Experiences are fetched automatically once region capabilities are fully established.
        LL_DEBUGS("ExperienceCache") << "GetExperienceInfo capability not available (pending region connection)." << LL_ENDL;
        return;
    }
