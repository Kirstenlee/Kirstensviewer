...
LLFontGL* LLFontRegistry::createFont(const LLFontDescriptor& desc)
{
		...
		++font_file_it)
	{
		...
		if (!is_font_loaded)
		{
			...
			// S24 avoid 10 times console spam since certain fonts are only availble when installed on a native system!!
			if (language == "ja" || language == "ko" || language == "zh")
			{
				LL_INFOS_ONCE("LLFontRegistry") << "Couldn't load font "
					<< font_file_it->FileName
					<< LL_ENDL;
			}
