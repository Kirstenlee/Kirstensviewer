...
:   mGlyphIndex(index),
    ...
    mLsbDelta(0),       // FreeType subpixel left side bearing delta
    mRsbDelta(0),       // FreeType subpixel right side bearing delta
...
void LLFontFreetype::renderGlyph(EFontGlyphType bitmap_type, U32 glyph_index, llwchar wch) const
{
    ...
    // --- Load flags ---------------------------------------------------------
    FT_Int32 load_flags = static_cast<FT_Int32>(mHinting);
    ...
    if (error != FT_Err_Ok)
    {
        // Hard fail on OOM — correct behaviour
        if (error == FT_Err_Out_Of_Memory)
        {
            LLError::LLUserWarningMsg::showOutOfMemory();
            LL_ERRS() << "OOM loading glyph U+" << llformat("%xu", U32(wch)) << LL_ENDL;
        }

        // Retry without FT_LOAD_COLOR (safe, deterministic)
        FT_Int32 retry_flags = load_flags & ~FT_LOAD_COLOR;
        error = FT_Load_Glyph(mFTFace, glyph_index, retry_flags);

        // If still bad, fallback to glyph 0 (missing glyph)
        if (error != FT_Err_Ok)
        {
            error = FT_Load_Glyph(mFTFace, 0, FT_LOAD_FORCE_AUTOHINT);
            llassert_always_msg(error == FT_Err_Ok,
                "Failed to load fallback glyph 0");
        }
    }

    // --- Glyph slot must exist ----------------------------------------------
    llassert_always(mFTFace->glyph != nullptr);

    // --- Render only if needed ----------------------------------------------
    // Color/SVG/bitmap glyphs may already have a bitmap buffer.
    if (!mFTFace->glyph->bitmap.buffer)
    {
        error = FT_Render_Glyph(mFTFace->glyph, gFontRenderMode);

        // If render mode fails (LCD/subpixel), retry NORMAL.
        if (error != FT_Err_Ok && gFontRenderMode != FT_RENDER_MODE_NORMAL)
        {
            error = FT_Render_Glyph(mFTFace->glyph, FT_RENDER_MODE_NORMAL);
        }

        llassert_always_msg(error == FT_Err_Ok,
            "Glyph render failed in all modes");
    }
