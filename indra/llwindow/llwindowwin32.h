...
// S24 Define custom message for async hostname resolution
constexpr UINT LL_WM_HOST_RESOLVED = WM_APP + 1;

// S24 Function pointer type for message callbacks
using LLW32MsgCallback = void(*)(const MSG&);

// S24 Function prototype for handling window messages
void processWindowMessage(const MSG& msg, LLW32MsgCallback callback);
...
public:
	...
	bool getBorderless() override { return mBorderless; } // S24: Return borderless mode status
...
protected:
	LLWindowWin32(LLWindowCallbacks* callbacks,
		...
		bool ignore_pixel_depth, U32 fsaa_samples, U32 max_cores, F32 max_gl_version, bool borderless = false);
	~LLWindowWin32();
	...
	// #5611: Discrete GPU selection for hybrid graphics systems
	static void selectHighPerformanceAdapter();
...
protected:
	...
	bool mBorderless { false }; // S24: Borderless window mode

	// S24: Borderless window maximize/restore
	void maximizeBorderless();
	void restoreBorderless();
	RECT mRestoreRect { 0, 0, 0, 0 }; // Saved position for restoring from maximized
	bool mBorderlessMaximized { false }; // Track if borderless window is "maximized"

	HANDLE      mMMCSSHandle = nullptr; // MMCSS registration for the main/render thread; reverted in destructor
