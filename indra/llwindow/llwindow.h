...
#define LL_LLWINDOW_H
 ...
 // Ensure Windows headers are properly configured before Boost includes
#ifdef _WIN32
...
public:
	...
	virtual bool getBorderless() { return false; }; // S24: Check if window is in borderless mode
...
// return a platform-specific window reference (HWND on Windows)
	virtual void *getPlatformWindow() = 0;

// return the platform-specific window reference we use to initialize llmozlib (HWND on Windows)
