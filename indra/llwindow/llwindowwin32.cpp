 ...
 *
 * !!! Furthur Enhanced and Modernised S24 - Kirstenlee Cinquetti !!!
 *
...
#define WIN32_LEAN_AND_MEAN  // Prevents extra headers, including old Winsock
...
// Private WM_USER sub-messages used for inter-thread signalling
const UINT WM_DUMMY_         = WM_USER + 0x0017; // wake window thread out of GetMessage()
const UINT WM_POST_FUNCTION_ = WM_USER + 0x0018; // dispatch a posted std::function<void()>
...
namespace
{
	LPDIRECTINPUT8 gDirectInput8 = nullptr; // DirectInput8 interface, initialised in LLWindowWin32 ctor
}

// #5611: D3D11 discrete GPU selection globals
namespace
{
	HMODULE gD3D11Library = nullptr;
	ID3D11Device* gD3D11Device = nullptr;
	ID3D11DeviceContext* gD3D11Context = nullptr;
	LUID gExpectedAdapterLUID = { 0, 0 };
}
...
//
// Helper: Detect preferred cores (P-cores on Intel, V-Cache CCD on AMD)
//
static std::vector<DWORD> detectPreferredCores()
{
	std::vector<DWORD> preferred_cores;
	preferred_cores.reserve(16); // Reserve space to avoid reallocs

	DWORD buffer_size = 0;
	GetLogicalProcessorInformationEx(RelationProcessorCore, nullptr, &buffer_size);
	if (buffer_size == 0) return preferred_cores;
	...
	DWORD offset = 0;
	DWORD logical_processor_index = 0;
	...
	while (offset < buffer_size)
	{
		...
		{
			// EfficiencyClass: 0 = P-core/V-Cache, 1 = E-core
			BYTE efficiency_class = current->Processor.EfficiencyClass;
				...
				for (int bit = 0; bit < 64; bit++)
				{
					if (mask & (1ULL << bit))
					{
						if (efficiency_class == 0)
							preferred_cores.push_back(logical_processor_index);
	...
	mBorderless(borderless)
{
	...
	// #5611: Select discrete GPU early (before window creation)
	...
	if (mMaxCores != 0)
	{
		...
		// Build affinity mask for the first mMaxCores logical processors.
		DWORD_PTR mask = 0;
		...
		// Apply process affinity (safe, reversible). This tends to prevent
		// excessive core parking by keeping the process active on more cores.
		if (!SetProcessAffinityMask(hProcess, mask))
		{
			LL_WARNS("Window") << "SetProcessAffinityMask failed: 0x" << std::hex << GetLastError() << LL_ENDL;
		}
		else
		{
			LL_INFOS("Window") << "Set process affinity to first " << mMaxCores << " logical processors." << LL_ENDL;
		}

		// Hint the main thread to prefer core 0 (doesn't _force_ it, just hints).
		SetThreadIdealProcessor(GetCurrentThread(), 0);

		// Detect preferred cores (P-cores/V-Cache) and hint scheduler
		std::vector<DWORD> preferred_cores = detectPreferredCores();
		if (!preferred_cores.empty())
		{
			SetThreadIdealProcessor(GetCurrentThread(), preferred_cores[0]);
		}

		// Conservative per-thread tweaks are opt-in via environment variable
		// LL_PERF_TWEAKS=1. This avoids surprising behaviour for normal users.
		const char* env = getenv("LL_PERF_TWEAKS");
		if (env && env[0] == '1')
		{
			// Raise process class one step to HIGH (NOT REALTIME) to reduce the chance
			// of being preempted under load while avoiding system lock-ups.
			if (!SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
			{
				LL_WARNS("Window") << "SetPriorityClass(HIGH) failed: 0x" << std::hex << GetLastError() << LL_ENDL;
			}
			else
			{
				LL_INFOS("Window") << "Process priority set to HIGH_PRIORITY_CLASS (opt-in)" << LL_ENDL;
			}

			// Raise current thread a notch (HIGHEST). Avoid THREAD_PRIORITY_TIME_CRITICAL
			// which can starve other system threads.
			if (!SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_HIGHEST))
			{
				LL_WARNS("Window") << "SetThreadPriority(HIGHEST) failed: 0x" << std::hex << GetLastError() << LL_ENDL;
			}
			else
			{
				LL_INFOS("Window") << "Main thread priority set to THREAD_PRIORITY_HIGHEST (opt-in)" << LL_ENDL;
			}

			// Disable dynamic priority boosting: prevents the scheduler from
			// unexpectedly demoting this thread after a brief high-priority burst.
			if (!SetThreadPriorityBoost(GetCurrentThread(), TRUE))
			{
				LL_WARNS("Window") << "SetThreadPriorityBoost(TRUE) failed: 0x" << std::hex << GetLastError() << LL_ENDL;
			}

			// Hint ideal processor to keep the render thread on core 0 where cache locality is best.
			SetThreadIdealProcessor(GetCurrentThread(), 0);

			// Optional: register with MMCSS or disable power throttling — kept in the
			// advanced block below so you can enable/experiment as needed.
		}
	}

#if 0
	// Advanced / potentially risky tweaks kept disabled by default.
	// Enable only after thorough testing and with explicit opt-in.
	if (env && env[0] == '1')
	{
		// Example: register render/main thread with MMCSS (Pro Audio) for better latency.
		// Requires including <avrt.h> and linking avrt.lib.
		DWORD mmcssTaskIndex = 0;
		HANDLE mmcssHandle = AvSetMmThreadCharacteristicsA("Pro Audio", &mmcssTaskIndex);
		if (mmcssHandle)
		{
			LL_INFOS("Window") << "MMCSS registered (taskIndex=" << mmcssTaskIndex << ")" << LL_ENDL;
			// Store mmcssHandle to revert via AvRevertMmThreadCharacteristics on shutdown.
		}

		// Example: attempt to disable power-throttling for the process.
		// This is system-dependent and can require elevated privileges on some OS versions.
		// PROCESS_POWER_THROTTLING_STATE pptState = { sizeof(pptState), PROCESS_POWER_THROTTLING_EXECUTION_SPEED, TRUE };
		// SetProcessInformation(GetCurrentProcess(), ProcessPowerThrottling, &pptState, sizeof(pptState));

		// Example: pin to P‑cores on hybrid CPUs (requires GetLogicalProcessorInformationEx parsing)
		// and/or use SetThreadGroupAffinity. Implement only if you detect CPU topology and validate.
	}
#endif

	// Register main/render thread with MMCSS for consistent low-latency scheduling.
	// Use "Games" task for render thread (lighter than "Pro Audio").
	DWORD mmcssTaskIndex = 0;
	mMMCSSHandle = AvSetMmThreadCharacteristicsA("Games", &mmcssTaskIndex);
	if (!mMMCSSHandle)
	{
	...
	if (!borderless && !fullscreen)
	{
		...
		S32 titlebar_height = GetSystemMetrics(SM_CYCAPTION) + GetSystemMetrics(SM_CYFRAME);
		S32 min_visible_y = workArea.top + titlebar_height;

		// Ensure window isn't positioned offscreen or with titlebar above visible area
		if (x < workArea.left) x = workArea.left;
		if (y < min_visible_y) y = min_visible_y;

		// Ensure window isn't too far right or bottom (allow some overhang but keep usable)
		if (x + 100 > workArea.right) x = workArea.right - 100;
		if (y > workArea.bottom - titlebar_height) y = workArea.bottom - titlebar_height;

		// Sanity check window size doesn't exceed work area
		...
		LL_INFOS("Window") << "S24: Window position sanitized to x=" << x << " y=" << y 
			<< " (work area: " << workArea.left << "," << workArea.top 
			<< " to " << workArea.right << "," << workArea.bottom << ")" << LL_ENDL;
	}
	else
	{
		// Original sanitization for borderless/fullscreen
	...
	// S24: If borderless mode, start maximized to fill screen like Fortnite
...
{
	// Restore main thread priority before reverting MMCSS
	setThreadPriorityNormal();

	// Revert MMCSS registration for main thread
	if (mMMCSSHandle)
	{
		AvRevertMmThreadCharacteristics(mMMCSSHandle);
...
// S24: Maximize borderless window to fill entire monitor (over taskbar like Fortnite)
void LLWindowWin32::maximizeBorderless()
{
	if (!mWindowHandle || !mBorderless) return;

	// Save current window position for restore
	if (!mBorderlessMaximized)
	{
		GetWindowRect(mWindowHandle, &mRestoreRect);
	}
	...
	{
		// Use full monitor (rcMonitor) not work area - covers taskbar like true borderless fullscreen
		...
		mBorderlessMaximized = true;
		LL_INFOS("Window") << "S24: Borderless window maximized to full monitor "
			<< (screen.right - screen.left) << "x" << (screen.bottom - screen.top) << LL_ENDL;
	}
}

// S24: Restore borderless window to previous size/position
void LLWindowWin32::restoreBorderless()
{
	if (!mWindowHandle || !mBorderless || !mBorderlessMaximized) return;

	SetWindowPos(mWindowHandle, HWND_TOP,
		mRestoreRect.left, mRestoreRect.top,
		mRestoreRect.right - mRestoreRect.left, mRestoreRect.bottom - mRestoreRect.top,
		SWP_FRAMECHANGED);

	mBorderlessMaximized = false;
	LL_INFOS("Window") << "S24: Borderless window restored" << LL_ENDL;
}
...
bool LLWindowWin32::switchContext(bool fullscreen, const LLCoordScreen& size, bool enable_vsync, const LLCoordScreen* const posp)
{
	...
	else
	{
		...
		{
			// Window with an edge (normal windowed mode)
	...
	PFD_TYPE_RGBA,  // Ensure RGBA format
            BITS_PER_PIXEL,
	0, 0, 0, 0, 0, 0, // Unused RGB bits
	8,              // Alpha bits
	0,              // Alpha shift
	0,              // Accumulation buffer bits
	0, 0, 0, 0,     // Accumulation RGBA
	24,             // Depth bits
	8,              // Stencil bits (explicitly retained)
	0,
	PFD_MAIN_PLANE, // No layering needed
	0,              // Reserved
	0, 0, 0         // Unused layer masks
	};
	...
	{
		// Dynamically determine the number of attributes needed
		...
		// Choose best pixel format using updated list
		...
		if (!result)
		{
			...
			close(); // Safely close context-related resources
            show_window_creation_error("Error after wglChoosePixelFormatARB 32-bit");
			return false; // Ensure graceful failure
		}
...
const   S32   max_format  = (S32)num_formats - 1;
		...
		// Iterate through available pixel formats to find a valid swap method
        // Spec:
        ...
        if (mhDC != 0)                                          // Does The Window Have A Device Context?
		{
			...
			{
				wglDeleteContext(mhRC); // Properly release the rendering context
                mhRC = 0;                                       // Zero The Rendering Context
			}
		}

		// Apply selected pixel format and recreate the window
		recreateWindow(window_rect, dw_ex_style, dw_style);
...
void LLWindowWin32::recreateWindow(RECT window_rect, DWORD dw_ex_style, DWORD dw_style)
{
	...
	// Pre-clear local handles so fallback message loop kicks in
    mWindowHandle = NULL;
    mhDC = NULL;
	...
	// 🚚 Dispatch window_work to window thread via appropriate method
...
void LLWindowWin32::toggleVSync(bool enable_vsync)
{
	...
	if (!mhDC || !mhRC)
	{
		LL_WARNS("Window") << "toggleVSync(): no active GL context" << LL_ENDL;
		return;
	}

	using PFNWGLSWAPINTERVALEXTPROC = BOOL(WINAPI*)(int);

	static PFNWGLSWAPINTERVALEXTPROC pSwapInterval = nullptr;
	static bool probed = false;

	if (!probed)
	{
		// Try to get function pointer (prefer wglGetProcAddress)
		pSwapInterval = (PFNWGLSWAPINTERVALEXTPROC)LLWindowWin32::getProcAddress("wglSwapIntervalEXT");

		probed = true;
		LL_INFOS("Window") << "VSync probe: wglSwapIntervalEXT " << (pSwapInterval ? "present" : "missing") << LL_ENDL;
	}

	if (!pSwapInterval)
	{
		LL_INFOS("Window") << "VSync: wglSwapIntervalEXT not available, cannot change vsync" << LL_ENDL;
		return;
	}

	int interval = enable_vsync ? 1 : 0;

	// Call on the current context; don't change context here.
	if (!pSwapInterval(interval))
	{
		LL_WARNS("Window") << "wglSwapIntervalEXT(" << interval << ") failed" << LL_ENDL;
	}
	else
	{
		if (interval == 1)
			LL_INFOS("Window") << "VSync enabled (swap interval = 1)" << LL_ENDL;
		else
			LL_INFOS("Window") << "VSync disabled (swap interval = 0)" << LL_ENDL;
	}
}
...
LRESULT CALLBACK LLWindowWin32::mainWindowProc(HWND h_wnd, UINT u_msg, WPARAM w_param, LPARAM l_param)
{
	...
	if (NULL != window_imp)
	{
		...
		case WM_SYSCOMMAND:
		{
			...
			// S24: Handle maximize/restore for borderless windows
			case SC_MAXIMIZE:
				if (window_imp->mBorderless)
				{
					window_imp->post([=]() { window_imp->maximizeBorderless(); });
					return 0;
				}
				break;
			...
			case SC_RESTORE:
				if (window_imp->mBorderless)
				{
					window_imp->post([=]() { window_imp->restoreBorderless(); });
					return 0;
				}
				break;
			}
	...
	case WM_CTLCOLORSTATIC:
		{
			...
			// Return dark brush for dialog background
...
// #5611: Select discrete GPU when possible (hybrid graphics systems)
void LLWindowWin32::selectHighPerformanceAdapter()
{
	gD3D11Library = LoadLibrary(L"d3d11.dll");
	if (gD3D11Library)
	{
		...
		if (pD3D11CreateDevice)
		{
			...
			if (adapterSelected)
			{
				...
				if (!SUCCEEDED(hr))
				{
					LL_WARNS("Window") << "D3D11 failed to use preferred adapter " << selected_descr << LL_ENDL;
					gExpectedAdapterLUID = { 0, 0 };
	...
	if (SUCCEEDED(hr) && pDXGIDevice)
	{
		...
		if (SUCCEEDED(hr) && pCurrentAdapter)
		{
				...
				desc.AdapterLuid.HighPart != gExpectedAdapterLUID.HighPart)
			{
				changed = true;
				std::string description = ll_convert_wide_to_string(description_w);
				LL_WARNS("Window") << "GPU change detected. Expected LUID: "
					<< gExpectedAdapterLUID.HighPart << "_" << gExpectedAdapterLUID.LowPart
					<< ", Current: " << desc.AdapterLuid.HighPart << "_" << desc.AdapterLuid.LowPart
					<< " (" << description << ")" << LL_ENDL;
			}
...
void LLWindowWin32::LLWindowWin32Thread::run()
{
	...
	// Register window thread with MMCSS for consistent scheduling
	DWORD mmcssTaskIndex = 0;
	HANDLE mmcssHandle = AvSetMmThreadCharacteristicsA("Games", &mmcssTaskIndex);
	if (!mmcssHandle)
	{
		LL_WARNS("Window") << "Failed to register window thread with MMCSS: " << GetLastError() << LL_ENDL;
	}

	// Raise window thread priority to HIGHEST for responsive input handling
	if (!SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_HIGHEST))
	{
		LL_WARNS("Window") << "SetThreadPriority(HIGHEST) failed for window thread: 0x" << std::hex << GetLastError() << LL_ENDL;
	}

	// Hint window thread to prefer a performance core (P-core/V-Cache)
	std::vector<DWORD> preferred_cores = detectPreferredCores();
	if (preferred_cores.size() > 1)
	{
		SetThreadIdealProcessor(GetCurrentThread(), preferred_cores[1]);
	}
	else if (!preferred_cores.empty())
	{
		SetThreadIdealProcessor(GetCurrentThread(), preferred_cores[0]);
	}
	...
	// Restore window thread priority before reverting MMCSS
	SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_NORMAL);

	// Revert MMCSS registration before destroying window
	if (mmcssHandle)
	{
