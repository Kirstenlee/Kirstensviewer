...
private:
	...
	libvlc_equalizer_t* mLibVLCEqualizer; // S24 - 10-band equalizer
	...
	// S24 - Metadata tracking
	std::string mLastMetadataTitle;
	std::string mLastMetadataArtist;
...
MediaPluginBase(host_send_func, host_user_data)
{
	...
	// S24 - Initialize metadata
	mLastMetadataTitle = std::string();
	mLastMetadataArtist = std::string();
}
...
{
	// S24 - Clean up equalizer
	if (mLibVLCEqualizer)
	{
		libvlc_audio_equalizer_release(mLibVLCEqualizer);
		mLibVLCEqualizer = NULL;
	}
}
...
void MediaPluginLibVLC::eventCallbacks(const libvlc_event_t* event, void* ptr)
{
	...
	case libvlc_MediaPlayerTimeChanged:
		...
		// S24 - Poll metadata periodically for streaming audio (every ~5 seconds)
		// VLC doesn't always fire TitleChanged for Shoutcast/Icecast metadata updates
		{
			static int poll_counter = 0;
			if (++poll_counter >= 50) // ~5 seconds at 10 fps time updates
			{
				poll_counter = 0;

				char* title = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_Title);
				char* artist = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_Artist);
				char* nowplaying = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_NowPlaying);

				bool metadata_changed = false;
				std::string new_title, new_artist;

				// Priority 1: NowPlaying (dynamic Shoutcast/Icecast metadata)
				if (nowplaying && strlen(nowplaying) > 0)
				{
					std::string nowplaying_str = nowplaying;
					size_t separator = nowplaying_str.find(" - ");
					if (separator != std::string::npos)
					{
						new_artist = nowplaying_str.substr(0, separator);
						new_title = nowplaying_str.substr(separator + 3);
					}
					else
					{
						new_title = nowplaying_str;
						new_artist.clear();
					}

					// Only send update if metadata actually changed
					if (new_title != parent->mLastMetadataTitle || new_artist != parent->mLastMetadataArtist)
					{
						parent->mLastMetadataTitle = new_title;
						parent->mLastMetadataArtist = new_artist;
						metadata_changed = true;
					}
				}
				// Priority 2: Static Title/Artist as fallback
				else
				{
					...
					if ((new_title != parent->mLastMetadataTitle || new_artist != parent->mLastMetadataArtist) &&
						(!new_title.empty() || !new_artist.empty()))
					{
						parent->mLastMetadataTitle = new_title;
						parent->mLastMetadataArtist = new_artist;
						metadata_changed = true;
				...
				if (metadata_changed)
				{
					parent->sendMetadataUpdate();
				}

				if (title) libvlc_free(title);
				if (artist) libvlc_free(artist);
				if (nowplaying) libvlc_free(nowplaying);
			}
		}
		break;
	...
	// S24 - MediaMetaChanged fires when Shoutcast/Icecast sends new metadata
	case libvlc_MediaMetaChanged:
	{
		// Extract metadata - Title often contains "Artist - Track" for streaming audio
		char* title = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_Title);
		char* artist = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_Artist);
		char* nowplaying = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_NowPlaying);
		...
		// Priority 1: NowPlaying field (if populated)
		if (nowplaying && strlen(nowplaying) > 0)
		{
			std::string nowplaying_str = nowplaying;
			size_t separator = nowplaying_str.find(" - ");
		...
		// Priority 2: Title field (often contains "Artist - Track" for Shoutcast)
		else if (title && strlen(title) > 0)
		{
			std::string title_str = title;
			size_t separator = title_str.find(" - ");
			if (separator != std::string::npos)
			{
				new_artist = title_str.substr(0, separator);
				new_title = title_str.substr(separator + 3);
			}
			else
			{
				new_title = title_str;
		...
		// Priority 3: Separate Artist field (fallback)
		if (artist && strlen(artist) > 0 && new_artist.empty())
		{
			new_artist = artist;
			metadata_found = true;
		}
		...
		// Only send update if metadata actually changed
		if (metadata_found && (new_title != parent->mLastMetadataTitle || new_artist != parent->mLastMetadataArtist))
		{
			parent->mLastMetadataTitle = new_title;
			parent->mLastMetadataArtist = new_artist;
			parent->sendMetadataUpdate();
	...
	{
		// S24 - Also check here as backup (MediaMetaChanged is primary for streaming)
		char* title = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_Title);
		char* artist = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_Artist);
		char* nowplaying = libvlc_media_get_meta(parent->mLibVLCMedia, libvlc_meta_NowPlaying);
		...
		// Priority 2: Parse Title field for "Artist - Track"
		else if (title && strlen(title) > 0)
		{
			std::string title_str = title;
			size_t separator = title_str.find(" - ");
		...
		// Priority 3: Separate Artist field
		if (artist && strlen(artist) > 0 && new_artist.empty())
		{
			new_artist = artist;
			metadata_changed = true;
		...
		// Only send update if metadata actually changed
		if (metadata_changed && (new_title != parent->mLastMetadataTitle || new_artist != parent->mLastMetadataArtist))
		{
			parent->mLastMetadataTitle = new_title;
			parent->mLastMetadataArtist = new_artist;
			parent->sendMetadataUpdate();
...
void MediaPluginLibVLC::playMedia()
{
	...
	// S24 - Parse metadata for network streams (required for Shoutcast/Icecast)
	// Note: metadata will be null until VLC receives first ICY block during playback
	libvlc_media_parse_with_options(mLibVLCMedia, libvlc_media_parse_network, -1);
	...
	// S24 - Listen for metadata changes (Shoutcast/Icecast sends metadata every icy-metaint bytes)
	libvlc_event_manager_t* media_em = libvlc_media_event_manager(mLibVLCMedia);
	if (media_em)
	{
		libvlc_event_attach(media_em, libvlc_MediaMetaChanged, eventCallbacks, this);
	}
	...
	// S24 - Initialize 10-band equalizer if not already created
	if (!mLibVLCEqualizer)
	{
		mLibVLCEqualizer = libvlc_audio_equalizer_new();
		if (mLibVLCEqualizer)
		{
			// VLC 10-band frequencies: 60, 170, 310, 600, 1K, 3K, 6K, 12K, 14K, 16K Hz
			// Initialize all bands to 0 dB (flat) and preamp to 0
			for (unsigned i = 0; i < 10; ++i)
			{
				libvlc_audio_equalizer_set_amp_at_index(mLibVLCEqualizer, 0.0f, i);
			}
			libvlc_audio_equalizer_set_preamp(mLibVLCEqualizer, 0.0f);
		}
	}
...
////////////////////////////////////////////////////////////////////////////////
// S24 - Send metadata update to viewer
void MediaPluginLibVLC::sendMetadataUpdate()
{
	if (mLastMetadataTitle.empty() && mLastMetadataArtist.empty())
		return;

	LLPluginMessage message(LLPLUGIN_MESSAGE_CLASS_MEDIA, "metadata_update");
	message.setValue("title", mLastMetadataTitle);
	message.setValue("artist", mLastMetadataArtist);
	sendMessage(message);
}
...
void MediaPluginLibVLC::receiveMessage(const char* message_string)
{
	...
	if (message_in.parse(message_string) >= 0)
	{
		...
		else if (message_class == LLPLUGIN_MESSAGE_CLASS_MEDIA)
		{
			...
			// S24 - Metadata request
			else if (message_name == "get_metadata")
			{
				if (mLibVLCMedia)
				{
					...
					LLPluginMessage response(LLPLUGIN_MESSAGE_CLASS_MEDIA, "metadata_update");
					if (title && strlen(title) > 0)
					{
						response.setValue("title", std::string(title));
						libvlc_free(title);
					}
					if (artist && strlen(artist) > 0)
					{
						response.setValue("artist", std::string(artist));
						libvlc_free(artist);
					}
			...
			else if (message_name == "eq_init")
			{
				if (!mLibVLCEqualizer)
				{
					...
					{
						// VLC 10-band frequencies: 60, 170, 310, 600, 1K, 3K, 6K, 12K, 14K, 16K Hz
						// Initialize all bands to 0 dB (flat)
						for (unsigned i = 0; i < 10; ++i)
						{
							libvlc_audio_equalizer_set_amp_at_index(mLibVLCEqualizer, 0.0f, i);
						}
			...
			// S24 - Equalizer cleanup
			else if (message_name == "eq_cleanup")
			{
				if (mLibVLCEqualizer)
				{
					if (mLibVLCMediaPlayer)
					{
						libvlc_media_player_set_equalizer(mLibVLCMediaPlayer, NULL);
					}
					libvlc_audio_equalizer_release(mLibVLCEqualizer);
					mLibVLCEqualizer = NULL;
				}
			}
			// S24 - Equalizer update
			else if (message_name == "eq_update")
			{
				if (mLibVLCEqualizer && mLibVLCMediaPlayer)
				{
					bool enabled = message_in.getValueBoolean("enabled");
					F32 preamp = (F32)message_in.getValueReal("preamp");
					LLSD bands = message_in.getValueLLSD("bands");

					// Clamp preamp to VLC range
					preamp = llclamp(preamp, -20.0f, 20.0f);
					libvlc_audio_equalizer_set_preamp(mLibVLCEqualizer, preamp);

					// Update 10 bands
					for (unsigned i = 0; i < 10 && i < (unsigned)bands.size(); ++i)
					{
						F32 gain = llclamp((F32)bands[i].asReal(), -20.0f, 20.0f);
						libvlc_audio_equalizer_set_amp_at_index(mLibVLCEqualizer, gain, i);
					}

					// Apply or disable EQ
					if (enabled)
					{
						libvlc_media_player_set_equalizer(mLibVLCMediaPlayer, mLibVLCEqualizer);
					}
					else
					{
						libvlc_media_player_set_equalizer(mLibVLCMediaPlayer, NULL);
